select jsonb_build_object(
 ...
 'instances',(select jsonb_agg(x order by x.operation_type) from (
   ...
   where o.outlet_id='e804c48d-6343-4bf8-99d7-9893c473948f' and o.business_date=current_date group by o.id
 ) x),
 'daily_tasks',(select jsonb_agg(jsonb_build_object('title',title,'priority',priority,'status',status,'exception_reason',exception_reason,'note',note) order by created_at) from public.crew_daily_tasks where outlet_id='e804c48d-6343-4bf8-99d7-9893c473948f' and task_date=current_date),
