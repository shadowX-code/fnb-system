...
- Stock Check mobile-first card UI is an experiment behind `VITE_ENABLE_MOBILE_STOCKCHECK_EXPERIMENT=true` / `ENABLE_MOBILE_STOCKCHECK_EXPERIMENT=true`. Staging enables this through `.env.staging`; local/dev and production remain off unless explicitly configured. When disabled, all users keep the existing desktop/table implementation. When enabled, only screens `<= 768px` render the separate `StockCheckMobileView`; it receives the same rows, validation, save, submit, skip, and photo preview handlers and does not change persistence or workflow logic. The temporary label `Mobile Stock Check Experimental` appears at the top of the mobile layout for verification.
