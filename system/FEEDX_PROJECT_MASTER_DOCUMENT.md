...
- Add/Edit Recipe validates `recipe_code`, `recipe_name_en`, `recipe_name_cn`, positive Selling Price, and ingredient rows inline with touched/submit-attempt timing. Fresh blank ingredient rows do not show red errors until the quantity field is touched or Save Recipe is clicked. Duplicate `recipe_code` is checked on blur and again before submit; duplicate codes block saving and show `Recipe code already exists.`
...
- Recipe Intelligence shows a Recipe Mapping Health card with Mapped, Unmapped, Coverage %, and a progress bar so operators can see Product Analytics ↔ Recipe Mapping completeness at a glance.
- Recipe Intelligence keeps Top Margin Products with Margin % and Profit per serving, shows Highest Cost Ingredients as the top five ingredient cost contributors sorted descending by total cost contribution, adds Top Revenue Recipes ranked by Product Analytics revenue, and keeps Lowest Margin Recipes sorted ascending by Margin % with English and Chinese recipe names plus Profit per serving. Recipe Cost Composition is not used. POS/sales-dependent charts show `Coming Soon` guidance when Product Analytics ↔ Recipe Mapping is unavailable; no fake sales data is generated.
