...
- Add/Edit Recipe validates `recipe_code`, `recipe_name_en`, `recipe_name_cn`, positive Selling Price, and ingredient rows inline before save. Duplicate `recipe_code` is checked on blur against loaded recipes and disables Save with `Recipe code already exists.` while unresolved.
