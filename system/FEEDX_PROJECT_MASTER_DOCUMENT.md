...
- Login, Forgot Password, Setup Password, and Reset Password share the FeedX auth visual system: desktop uses a minimal dark futuristic layout with a left brand/intelligence panel, a central holographic operations portal image asset (`public/design-homepage/holographic-ring.webp`) with subtle CSS/SVG motion overlays, and a right dark glassmorphism auth card. Mobile uses a compact brand header and keeps the form visible without excessive scrolling; the holographic visual may be hidden on small screens.
