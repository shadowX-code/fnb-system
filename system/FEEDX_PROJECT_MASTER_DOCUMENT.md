...
- Recipe Intelligence appears below Recipe BOM Setup in a strategic management dashboard layout. Default analysis period is Last 3 Months, with options for Current Month, Last 3 Months, Last 6 Months, and Last 12 Months.
- Recipe Mapping Health uses a wide card with Coverage %, Mapped Recipes, Unmapped Products, Products / Recipes count, guidance copy, and a progress bar. It should guide operators to map more products before relying on menu insights.
...
- Recipe Intelligence includes a Recipe Insights panel beside the matrix when reliable mapped data exists. Insights classify recipes as Star, Workhorse, Puzzle, or Dog and include the recipe name, classification badge, reason, recommended action, and impact level. Low-coverage or unavailable data shows guidance rather than fake recommendations.
...
- Recipes & Usage includes a Product ↔ Recipe Mapping section that lists unmapped Product Analytics products, suggests recipe matches using `recipe_code`, then `recipe_name_en`, then `recipe_name_cn`, shows confidence, and lets operators manually map a product to a recipe. Saved mappings write to `product_recipe_mappings`; ignored products are session-level only until a persisted ignore/status field is added.
- Recipe Intelligence keeps compact table cards for Top Margin Products with Margin % and Profit per serving, Highest Cost Ingredients as the top five ingredient cost contributors sorted descending by total cost contribution, Top Revenue Recipes ranked by Product Analytics revenue, and Lowest Margin Products sorted ascending by Margin % with English and Chinese recipe names plus Profit per serving. Recipe Cost Composition is not used. POS/sales-dependent charts show empty or locked guidance when Product Analytics ↔ Recipe Mapping is unavailable; no fake sales data is generated.
