...
Deno.serve(async (request) => {
  ...
  let token = "", outletId = "", requestId = "", assetId = "", details: Record<string, unknown> = {}, original: File | null = null, display: File | null = null, thumbnail: File | null = null;
  try { const form = await request.formData(); token = String(form.get("token") || ""); outletId = String(form.get("outlet_id") || ""); requestId = String(form.get("request_id") || ""); assetId = String(form.get("asset_id") || ""); details = JSON.parse(String(form.get("details") || "{}")); original = form.get("original") instanceof File ? form.get("original") as File : null; display = form.get("display") instanceof File ? form.get("display") as File : null; thumbnail = form.get("thumbnail") instanceof File ? form.get("thumbnail") as File : null; } catch { return reply({ error: "The Asset details could not be read. Please try again." }, 400); }
  if (!token || !uuid.test(requestId) || !uuid.test(assetId) || (outletId && !uuid.test(outletId)) || (Boolean(original) !== Boolean(display) || Boolean(display) !== Boolean(thumbnail))) return reply({ error: "Asset details are incomplete. Please try again." }, 400);
  ...
  try {
    ...
    const { data, error } = await crew.rpc(outletId ? "crew_management_asset_update_details" : "crew_asset_update_details", { p_token: token, p_request_id: requestId, p_asset_id: assetId, p_details: details, p_original_image_url: urls[0], p_image_url: urls[1], p_thumbnail_url: urls[2], ...(outletId ? { p_outlet_id: outletId } : {}) });
