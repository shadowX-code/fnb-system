-- Read-only MeSTI projection of canonical completed-Production Finished Goods batches.
-- This creates no MeSTI ledger or inventory records.
...
begin
  ...
  select balance.id, production.id, job.id, job.job_order_no, production.production_no,
    ...
    balance.opening_qty, coalesce(nullif(btrim(finished_good.uom), ''), 'packs'), balance.storage_location_id,
...
create or replace function public.factory_mesti_finished_product_storage_control_filter_options()
...
begin
  ...
  return jsonb_build_object(
    'finished_goods', coalesce((select jsonb_agg(jsonb_build_object('id', source.id, 'name', source.name) order by source.name, source.id) from (
      select distinct product_family.id, coalesce(product_family.name_en, finished_good.product_name_en, finished_good.product_name) as name
      from public.factory_finished_good_batch_balances balance join public.factory_productions production on production.id = balance.production_id
      join public.factory_finished_goods finished_good on finished_good.id = balance.finished_good_id
      left join public.factory_product_families product_family on product_family.id = finished_good.product_family_id
      where balance.source_type = 'production' and lower(coalesce(production.status, '')) = 'completed'
    ) source where source.id is not null), '[]'::jsonb),
    'packaging_skus', coalesce((select jsonb_agg(jsonb_build_object('id', source.id, 'code', source.code, 'name', source.name) order by source.code, source.name, source.id) from (
      select distinct finished_good.id, finished_good.product_code as code, coalesce(finished_good.product_name_en, finished_good.product_name) as name
      from public.factory_finished_good_batch_balances balance join public.factory_productions production on production.id = balance.production_id
      join public.factory_finished_goods finished_good on finished_good.id = balance.finished_good_id
      where balance.source_type = 'production' and lower(coalesce(production.status, '')) = 'completed'
    ) source), '[]'::jsonb),
    'storage_locations', coalesce((select jsonb_agg(jsonb_build_object('id', source.id, 'name', source.name) order by source.name, source.id) from (
      select distinct balance.storage_location_id as id, coalesce(location.location_name, balance.storage_location) as name
      from public.factory_finished_good_batch_balances balance join public.factory_productions production on production.id = balance.production_id
      left join public.factory_storage_locations location on location.id = balance.storage_location_id
      where balance.source_type = 'production' and lower(coalesce(production.status, '')) = 'completed'
    ) source where source.id is not null), '[]'::jsonb)
...
revoke all on function public.factory_mesti_finished_product_storage_control_filter_options() from public, anon;
grant execute on function public.factory_mesti_finished_product_storage_control_filter_options() to authenticated;

comment on function public.factory_mesti_finished_product_storage_control(date, date, uuid, uuid, uuid, text)
is 'Read-only MeSTI projection: exactly one authoritative Finished Goods batch row per completed Production.';
