-- Factory management dashboard monthly analytics.
-- Read-only, permission-scoped aggregates are returned from one statement so
-- the dashboard does not depend on browser preload limits or mix incompatible UOMs.
...
begin
  ...
  receiving_materials as (
    select
      ...
      bool_and(unit_cost > 0 and total_cost > 0) as cost_complete,
      case when bool_and(unit_cost > 0 and total_cost > 0) then sum(total_cost) / nullif(sum(received_qty), 0) end as average_unit_cost,
      case when bool_and(unit_cost > 0 and total_cost > 0) then sum(total_cost) end as total_cost
    from receiving_scope
