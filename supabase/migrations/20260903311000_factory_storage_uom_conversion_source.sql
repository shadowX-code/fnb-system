...
declare
  v_storage text := public.factory_normalize_uom(new.uom);
  v_base text := public.factory_normalize_uom(new.conversion_base_uom);
begin
  if v_storage = '' then
    raise exception 'Storage UOM is required.';
  end if;
  if new.conversion_package_quantity is null and new.conversion_base_uom is null then
    return new;
  end if;
  if new.conversion_package_quantity is null or new.conversion_base_uom is null then
    raise exception 'Package conversion requires quantity and base UOM.';
  end if;
...
before insert or update of uom, conversion_package_quantity, conversion_base_uom
on public.factory_raw_materials
for each row execute function public.factory_validate_raw_material_uom_conversion();

alter table public.factory_raw_materials drop column if exists conversion_package_uom;
...
begin
  ...
  v_canonical := case
    when coalesce(v_material.conversion_package_quantity, 0) > 0
      and public.factory_normalize_uom(v_material.uom) <> ''
...
begin
  ...
  for v_item in select value from jsonb_array_elements(p_bom_items) loop
    ...
    if found
      and v_existing_item.raw_material_id = v_material.id
      and coalesce((v_item->>'update_recipe_usage_uom')::boolean, false) is not true then
    ...
    v_resolved_items := v_resolved_items || jsonb_build_array((v_item - 'id' - 'update_recipe_usage_uom') || jsonb_build_object('recipe_usage_uom', v_usage_uom, 'uom', v_usage_uom));
  end loop;
