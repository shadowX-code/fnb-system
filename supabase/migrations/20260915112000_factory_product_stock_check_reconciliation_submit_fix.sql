-- The save function returns a table with an `id` output variable. Qualify the
-- status-update predicate so PostgreSQL never resolves it ambiguously.
...
begin
  ...
  if v_submitting then
    update public.factory_product_stock_checks stock_check
