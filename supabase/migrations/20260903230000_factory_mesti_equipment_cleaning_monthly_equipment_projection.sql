-- Monthly Cleaning of Equipment is an Equipment-centric compliance projection.
-- It preserves occurrence-level evidence for the drill-down while aggregating
-- every scheduled and After Production obligation into one Equipment/date cell.
...
begin
  ...
  return coalesce((
    with occurrence_rows as (
      select
        occurrence.*,
        occurrence.requirement_snapshot->>'task_name' as task_name,
      ...
      select
        equipment_id,
        min(equipment_code) as equipment_code,
        min(equipment_name) as equipment_name,
        ...
        case
          when count(*) filter (where status = 'unsatisfactory') > 0 then 'unsatisfactory'
          when count(*) filter (where status = 'missed') > 0 then 'missed'
          when count(*) filter (where status = 'verified') = count(*) then 'verified'
          when count(*) filter (where status = 'verified') > 0 then 'mixed'
        ...
        ) order by created_at, id) as occurrences
      ...
      group by equipment_id, due_date
    ), equipment_rows as (
      select
        equipment_id,
        min(equipment_code) as equipment_code,
        ...
        min(location_name) as location_name,
        count(*)::integer as total_count,
        coalesce(sum(verified_count), 0)::integer as verified_count,
        coalesce(sum(completed_count), 0)::integer as completed_count,
        coalesce(sum(unsatisfactory_count), 0)::integer as unsatisfactory_count,
        coalesce(sum(missed_count), 0)::integer as missed_count,
        coalesce(sum(pending_count), 0)::integer as pending_count,
        jsonb_agg(jsonb_build_object(
          'due_date', due_date,
          'status', status,
    ...
    select jsonb_agg(jsonb_build_object(
      ...
      'days', days
    ) order by equipment_code, equipment_name)
    from equipment_rows
  ), '[]'::jsonb);
end;
$$;
