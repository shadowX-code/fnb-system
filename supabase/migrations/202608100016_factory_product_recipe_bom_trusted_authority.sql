-- Atomic, request-idempotent Factory Product Recipe/BOM structural saves.
create table if not exists public.factory_product_recipe_requests (
  request_id uuid primary key,
  operation text not null check (operation in ('create_product_recipe', 'update_product_recipe')),
  actor_auth_user_id uuid not null references auth.users(id),
  recipe_id uuid references public.factory_product_recipes(id) on delete restrict,
  payload_fingerprint text not null,
  canonical_result jsonb,
  created_at timestamptz not null default now(),
  completed_at timestamptz
);
alter table public.factory_product_recipe_requests enable row level security;
revoke all on public.factory_product_recipe_requests from anon, authenticated;
