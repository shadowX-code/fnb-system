-- Default arguments execute in caller context. Keep the date helper private and
-- use the built-in timezone expression directly for anon Crew calls.
...
create or replace function public.crew_operations_admin_data(p_outlet_id uuid,p_business_date date default timezone('Asia/Kuala_Lumpur',now())::date)
returns jsonb language plpgsql volatile security definer set search_path=public as $$
