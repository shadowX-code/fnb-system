-- Extend the canonical Reporting product-sales projection with category-level
-- contributions. Product Analytics remains the authority for all quantities
-- and sales values consumed by the Monthly poster.
...
begin
  ...
  if not found then return jsonb_build_object('period', jsonb_build_object('year', p_year, 'month', p_month), 'product_data_status', 'unavailable', 'top_products', '[]'::jsonb, 'lowest_products', '[]'::jsonb, 'category_contributions', '[]'::jsonb, 'total_product_sales_revenue', null); end if;
  if v_report.status <> 'completed' or not exists (select 1 from public.product_sales_items where report_id = v_report.id) then return jsonb_build_object('period', jsonb_build_object('year', p_year, 'month', p_month), 'product_data_status', 'incomplete', 'top_products', '[]'::jsonb, 'lowest_products', '[]'::jsonb, 'category_contributions', '[]'::jsonb, 'total_product_sales_revenue', null); end if;
  ...
  with products as (
    select i.category_name, i.product_name, coalesce(i.variant_name, '') as variant_name, sum(i.quantity) as quantity, sum(i.nett_sales) as sales_revenue
    from public.product_sales_items i where i.report_id = v_report.id
  ...
  ) select coalesce(jsonb_agg(jsonb_build_object('category_name', category_name, 'sales_revenue', sales_revenue) order by sales_revenue desc, category_name), '[]'::jsonb)
