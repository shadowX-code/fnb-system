-- KWSP publishes a rounded aggregate but not the allocation of fractional
-- residuals between employee deduction and employer contribution. Keep exact
-- published total evidence while refusing a Ready result until allocation is
-- governed and independently reconciled.
...
begin
  ...
  if v_epf_line->>'applicable'='true' then
    v_category_issue:=case when nullif(v_category,'') is null then 'epf_category_unreviewed'
      else public.payroll_statutory_category_issue('epf',v_category,
        v_employee.nationality,v_employee.birthday,v_period.period_start,v_period.period_end) end;
    ...
    if v_category='malaysian_under_60' and v_base>5000
      and v_ordinary-v_unclassified<=5000 and v_missing_class then
    ...
    if v_category_issue is null and v_base>=0
      and not (v_category='malaysian_under_60' and v_base>5000
        and v_ordinary-v_unclassified<=5000 and v_missing_class) then
      ...
      if v_schedule.id is null then
        ...
        if v_remittance_rounding<>0 then
          v_issues:=array_append(v_issues,'epf_remittance_rounding_allocation_unapproved');
        end if;
