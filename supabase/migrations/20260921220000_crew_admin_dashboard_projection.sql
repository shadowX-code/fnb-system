-- Crew Dashboard is a permission-aware operational read projection. It never
-- owns attendance, roster, leave, compliance, Performance, or Crew Access state.
...
begin
  ...
  with active_crew as materialized (
    select e.id, e.full_name, e.position, e.birthday, ca.access_state
  ...
  if v_can_leave then
    ...
    if v_count>0 then
      v_attention:=v_attention || jsonb_build_array(jsonb_build_object(
        'key', 'leave_requests', 'count', v_count,
        'title', 'Pending leave requests',
        'detail', 'New requests need review.'
      ));
    end if;
  end if;
  ...
  if v_can_attendance then
    ...
    if v_count>0 then
      v_attention:=v_attention || jsonb_build_array(jsonb_build_object(
        'key', 'attendance_exceptions', 'count', v_count,
        'title', 'Attendance exceptions',
        'detail', 'Location evidence needs review.'
      ));
    end if;
  end if;
  ...
  if v_can_compliance then
    ...
    select
      count(*) filter (where state->>'effective_status'='pending_verification'),
    ...
    if v_pending_compliance>0 then
      v_attention:=v_attention || jsonb_build_array(jsonb_build_object(
        'key', 'compliance_review', 'count', v_pending_compliance,
        'title', 'Compliance review',
        'detail', 'Documents are waiting for verification.'
      ));
    end if;
    if v_compliance_risk>0 then
      v_attention:=v_attention || jsonb_build_array(jsonb_build_object(
        'key', 'compliance_status', 'count', v_compliance_risk,
        'title', 'Compliance follow-up',
        'detail', 'Documents are missing, expiring, or expired.'
      ));
    end if;
  end if;

  if v_can_performance then
    v_performance:=public.crew_performance_admin_data(p_outlet_id, v_period);
    select count(*) into v_count
    from jsonb_array_elements(coalesce(v_performance->'crew', '[]'::jsonb)) row
    where row->'result'->'components'->'service'->>'status'<>'reviewed'
       or row->'result'->'components'->'conduct'->>'status'<>'reviewed';
    if v_count>0 then
      v_attention:=v_attention || jsonb_build_array(jsonb_build_object(
        'key', 'performance_reviews', 'count', v_count,
        'title', 'Performance reviews',
        'detail', 'Service Standards or Conduct is still pending.'
      ));
    end if;
  end if;
