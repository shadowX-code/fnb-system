...
create table if not exists public.factory_production_sop_step_qc_checks (
  ...
  constraint factory_production_sop_step_qc_checks_template_type check (qc_type = 'checklist' or checklist_template_id is null),
...
insert into public.factory_qc_checklist_templates (name, category, description)
values
  ('Raw material condition acceptable', 'Material', 'Confirm raw material condition is acceptable for production.'),
  ('Equipment cleaned', 'Equipment', 'Confirm production equipment has been cleaned.'),
  ('Temperature checked', 'Process', 'Confirm the required process temperature was checked.'),
  ('Texture acceptable', 'Product', 'Confirm finished product texture is acceptable.'),
  ('Colour acceptable', 'Product', 'Confirm finished product colour is acceptable.'),
  ('No foreign matter', 'Food Safety', 'Confirm no foreign matter is present.'),
  ('Packaging seal complete', 'Packaging', 'Confirm the packaging seal is complete.'),
  ('Label correct', 'Packaging', 'Confirm the correct label is applied.'),
  ('Weight checked', 'Packaging', 'Confirm packaged weight was checked.'),
  ('No leakage', 'Packaging', 'Confirm packaging has no leakage.')
...
begin
  ...
  loop
    ...
    loop
      v_qc_no := v_qc_no + 1;
      v_qc_type := lower(btrim(coalesce(v_qc_check->>'qc_type', '')));
      if v_qc_type not in ('checklist', 'remarks') then
        raise exception 'Step % QC % requires a valid Type.', v_step_no, v_qc_no;
      end if;
      if nullif(btrim(coalesce(v_qc_check->>'qc_name', '')), '') is null then
        raise exception 'Step % QC % requires a QC Name.', v_step_no, v_qc_no;
      end if;

      v_qc_template_id := nullif(v_qc_check->>'checklist_template_id', '')::uuid;
      v_qc_template_name := null;
      if v_qc_template_id is not null then
        select template.name, case when template.is_active then template.id else null end
        into v_qc_template_name, v_qc_template_id
        from public.factory_qc_checklist_templates template
        where template.id = nullif(v_qc_check->>'checklist_template_id', '')::uuid;
        if not found then
          raise exception 'Step % QC % uses an unavailable checklist preset.', v_step_no, v_qc_no;
        end if;
      end if;
      ...
      insert into public.factory_production_sop_step_qc_checks (
        ...
        case when v_qc_type = 'checklist' then v_qc_template_id else null end,
        btrim(v_qc_check->>'qc_name'),
