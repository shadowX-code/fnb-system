-- Forward fix for reading requirement flags from jsonb evidence rows.
...
begin
  ...
  select
    count(*) filter(where coalesce((e->>'required')::boolean,false)),
    count(*) filter(where coalesce((e->>'required')::boolean,false) and coalesce((e->>'completed')::boolean,false)),
    count(*) filter(where coalesce((e->>'required')::boolean,false) and e->>'type' not in ('practical','manual','performance')),
    count(*) filter(where coalesce((e->>'required')::boolean,false) and e->>'type' not in ('practical','manual','performance') and coalesce((e->>'completed')::boolean,false)),
    bool_or(coalesce((e->>'required')::boolean,false) and e->>'type'='practical'),
    bool_or(coalesce((e->>'required')::boolean,false) and e->>'type'='manual')
  into total,done,automated_total,automated_done,practical_required,manual_required
  from jsonb_array_elements(requirements) e;
