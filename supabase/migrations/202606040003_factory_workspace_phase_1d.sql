-- Factory workspace Phase 1D: Production SOP, QC checkpoints,
-- SOP attachment to production and batch traceability foundation.
...
alter table public.factory_production_sop_steps
  ...
  add column if not exists is_qc_checkpoint boolean not null default false;

update public.factory_production_sop_steps
set process_name = coalesce(process_name, instruction),
    description = coalesce(description, instruction),
    estimated_time_minutes = coalesce(estimated_time_minutes, expected_duration_minutes)
where process_name is null
   or description is null
   or estimated_time_minutes is null;

alter table public.factory_productions
  add column if not exists production_sop_id uuid references public.factory_production_sops(id),
  add column if not exists sop_version text;

alter table public.factory_production_material_usage
  add column if not exists raw_material_receiving_id uuid references public.factory_raw_material_receivings(id),
  add column if not exists raw_material_lot_no text;

create table if not exists public.factory_production_qc_checkpoints (
...
grant select, insert, update, delete on public.factory_production_qc_checkpoints to authenticated;

alter table public.factory_production_qc_checkpoints enable row level security;
...
drop policy if exists "factory production qc checkpoints manage" on public.factory_production_qc_checkpoints;
create policy "factory production qc checkpoints manage" on public.factory_production_qc_checkpoints for all to authenticated
using (
