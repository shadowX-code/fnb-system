-- Transactional, request-idempotent authority for one complete Duty Roster week.

create table if not exists public.duty_roster_lifecycle_requests (
...
create index if not exists duty_roster_lifecycle_requests_scope_idx
  on public.duty_roster_lifecycle_requests (operation, outlet_id, week_start_date);

alter table public.duty_roster_lifecycle_requests enable row level security;
revoke all on table public.duty_roster_lifecycle_requests from anon, authenticated;
...
begin
  ...
  if exists (
    ...
    having r.employee_id is null or r.roster_date is null or r.shift_template_id is null or count(*) > 1
...
revoke all on function public.save_roster_week_snapshot(uuid, uuid, date, jsonb) from public;
