-- Workplace identifies workforce belonging. Role authority and Restaurant outlet
-- scope remain separate, and Factory has no outlet-scope analogue.

alter table public.permissions
  add column if not exists requires_restaurant_outlet_scope boolean not null default true;

-- Factory, Crew, People, System, and Guest permissions do not consume a
-- Restaurant outlet relationship. New permissions default to scoped until their
-- owning migration classifies them deliberately.
update public.permissions
set requires_restaurant_outlet_scope = false
where code like 'factory_%'
   or code like 'crew_%'
   or code like 'guest_ai.%'
   or code like 'employees.%'
   or code like 'employee_%'
   or code like 'employment_contract_templates.%'
   or code like 'legal_entities.%'
   or code like 'departments.%'
   or code like 'job_positions.%'
   or code like 'roles.%'
   or code like 'roles_permissions.%'
   or code like 'audit_logs.%';
...
alter table public.roles
  ...
  check (outlet_access_type in ('all', 'selected', 'none'));

-- Factory-only and other non-Restaurant roles retain their permissions but no
-- longer imply access to Restaurant outlets. No employee or historical record
-- is changed.
update public.roles role
set outlet_access_type = 'none'
where not exists (
  select 1
...
delete from public.role_outlets role_outlet
using public.roles role
where role.id = role_outlet.role_id
  and role.outlet_access_type = 'none';
...
begin
  ...
  if not v_protected and exists (
    select 1 from unnest(v_permissions) code
    where not exists(select 1 from public.permissions permission where permission.code=code and public.current_user_can_assign_permission(permission.id))
  ...
  if exists(select 1 from unnest(v_permissions) code where not exists(select 1 from public.permissions permission where permission.code=code))
    ...
    raise exception 'Unknown permission or outlet.';
  end if;
...
-- Factory workforce selectors use workplace belonging only. Management authority
-- or Factory permissions never add an employee to this operational population.
...
language sql stable security definer set search_path = public, pg_temp as $$
  select e.id, coalesce(e.nickname, e.full_name), e.position, e.workplace
  from public.employees e
  where e.is_active
    and coalesce(e.employment_status, 'active') = 'active'
    and lower(btrim(coalesce(e.workplace, ''))) = 'factory'
