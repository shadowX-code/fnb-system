-- Allow an employee to be scheduled at different outlets by date without
-- weakening scope: the manager must access both the target roster outlet and
-- the employee's established home outlet.

drop policy if exists "crew roster viewers can view schedulable employees" on public.employees;
create policy "crew roster viewers can view schedulable employees"
...
using (
  ...
  and exists (
    ...
    where public.current_user_can_access_outlet(o.id)
      and (lower(o.name)=lower(coalesce(employees.workplace,'')) or lower(coalesce(o.code,''))=lower(coalesce(employees.workplace,'')) or o.id::text=employees.workplace)
  )
);
...
declare
  ...
  has_inserts boolean; has_updates boolean; has_deletes boolean; period_exists boolean; result jsonb;
begin
  ...
  if exists(
    select 1 from jsonb_to_recordset(rows_json) r(employee_id uuid,roster_date date,shift_template_id uuid,remark text)
    left join public.employees e on e.id=r.employee_id
    where e.id is null or not coalesce(e.is_active,true) or coalesce(e.employment_status,'')<>'active'
  ...
  select jsonb_build_object('period',to_jsonb(period_row),'rows',coalesce(jsonb_agg(to_jsonb(d) order by d.roster_date,d.employee_id),'[]'::jsonb)) into result from public.duty_rosters d where d.outlet_id=p_outlet_id and d.roster_date between p_week_start_date and week_end;
  update public.duty_roster_lifecycle_requests set result=result,completed_at=now() where request_id=p_request_id;
  return result;
end $$;
