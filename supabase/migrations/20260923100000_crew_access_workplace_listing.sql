-- Crew Access provisioning follows Employee Workplace, not the employee's Role outlet visibility.
...
begin
  ...
  with source as (
    select e.*,ca.employee_id as access_employee_id,ca.mobile_number as crew_mobile_number,
      ...
      case when r.is_active and r.outlet_access_type='selected' then
        (select count(*) from public.role_outlets ro join public.outlets o on o.id=ro.outlet_id
  ...
  select
    ...
    (select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'role_outlet_access',jsonb_build_object('type',coalesce(role_outlet_type,'none'),'count',role_outlet_count),
