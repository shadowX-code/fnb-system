...
const data = {
  ...
  operationsHistory: () => ({ from: "2026-08-02", to: date, history_window_days: 30, tasks: tasks().filter((task) => task.status === "completed") }),
  operationDetail: () => ({ id: "qa-task-0", name: longCopy(), status: "not_started", task_type: "checklist", can_act: true, allow_exception: true, assignment: { kind: "individual", employee_id: "qa-employee", employee_name: longCopy(), is_current_employee: true }, blocks: [{ id: "qa-block", title: longCopy(), block_type: "yes_no", required: true, status: "pending", config: { no_requires_issue: true } }] }),
  ...
  assetsMobile: () => ({ outlet: outlet(), assets: [], categories: [], inspection_drafts: [], movement_history: [], inspection_history: [], can_add_assets: false, can_adjust_assets: true, can_perform_asset_inspections: true }),
