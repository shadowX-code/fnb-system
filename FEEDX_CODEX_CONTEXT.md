...
FeedX Staging Vercel deployments target only `fnb-system-staging` (`prj_t6uJtKPDu9GuyefG6IqAfxh5YoIi`); Production targets only `fnb-system`. Project ID/name, not Vercel's internal `target` label, define the FeedX business environment: a `target=production` deployment inside the verified Staging project is still Staging.

Normal canonical Staging delivery is `origin/dev` → Vercel Git Integration → `fnb-system-staging` → authenticated QA. Canonical Staging is permitted only from a clean `dev` worktree whose local `HEAD` exactly equals current `origin/dev`; the Git Integration deployment SHA must be that same SHA and be `READY` on the verified Staging project/alias. Run `npm run verify:staging-vercel-target` before checking a canonical Staging deployment. Do not create manual Vercel deployments, Preview deployments, or alias promotions when Git Integration has already produced the required `dev` deployment. Feature and Guest AI worktrees may use `npm run deploy:preview` only for explicitly requested isolated QA; they must never use `vercel --prod`, the real `fnb-system` Production project, or a direct promotion to alter `fnb-system-staging.vercel.app`. Never implicitly run `vercel link` or create another FeedX Vercel project.

The canonical workspace must be explicitly linked only to verified Staging Supabase `fnb-system-staging` (`ujkzdaaadnvcfayuldmh`) before Staging migration or database QA. Confirm that ref immediately before a mutation. Never link or push `fnb-system` Production (`oyfobxdoyfuzsodogpgs`) without explicit Production authorization.
