...
The application still shares an initial Admin/Crew entry; this is **not** a claim that Crew has an isolated 3.7 MB bundle or that the entire application is now small. The build manifest places Growth/Performance (30.44 kB), Reward (13.54 kB), Learn (23.39 kB), Cash Checkout (28.75 kB), and Leave (10.01 kB) exclusively behind dynamic imports. The entry has no static imports of these chunks. Shared lazy-only help is 1.10 kB. Home, session/bootstrap, navigation and CSS remain eager. Tasks still shares code with Admin Preview.
