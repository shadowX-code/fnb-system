...
- Home/task and navigation presentation: one grouped task surface with row dividers, shared SOP-density title typography, untruncated title/status coexistence, and full-row reachability. All five bottom-nav destinations retain consistent line-icon geometry, active page semantics and shared token colors without shifting the nav on route changes. These checks run across the full width/locale matrix.
- Shared overlays: BottomSheet, HelpSheet and MobileModal portal ownership, one active focus trap, trigger restoration, Escape/backdrop close, nested scroll-lock retention and reduced motion. BottomSheet follows `window.visualViewport` when present so its scroll body and footer remain inside the usable mobile viewport; blank body taps dismiss a focused keyboard and the handle supports dismissal. PerformanceDetailSheet is exercised through the real Performance route. Clock reason selection must replace, not nest, its confirmation sheet.
- Input flows: Cash Count, Leave dates/reason, Clock exception reason, Task issue note and Assets. Editable fields are at least 16 CSS px. Short-height viewport checks verify that key actions remain reachable without clicking Submit/Confirm; return hints advance or dismiss only through the shared bottom-sheet behavior.
...
- Async CSS: Cash, Growth/Performance, Reward and Leave styles are absent from Home's stylesheet requests, arrive with first feature entry, and preserve representative computed styles on history re-entry and direct refresh. The Performance evidence sheet checks feature content styling inside the shared portal shell. These four checks run at every width/locale, alongside the existing layout/interaction matrix. `verifyAppBundle.mjs` separately checks real production CSS ownership; the local renderer server is not a production chunk-delivery test.
