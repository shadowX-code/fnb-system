...
All Factory MeSTI surfaces reuse those frozen foundations according to their workflow: Cleaning, Operator Hygiene, and Waste Disposal use Operational Daily and Monthly Matrix; Calibration setup and requirements use the List Foundation composition; Raw Material Control and Food Processing Control are read-only evidence reports; Health Declaration is an evidence history with declaration actions in the page header. This is presentation-only and does not create a second MeSTI source of truth.
