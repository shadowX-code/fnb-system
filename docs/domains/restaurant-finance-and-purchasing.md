...
Product Analytics remains a separate authority: completed product-sales reports/items provide the Monthly poster's product ranking, product quantities, category contribution projection, and period total product sales revenue only, never Financial Revenue. Rankings preserve category/product/variant identity; Top 10 is sales revenue descending, and the Reporting product-sales projection also returns category sales totals sorted by sales revenue. Product and category contribution percentages use that canonical product-sales total as their denominator. Missing or incomplete Product Analytics data does not block a financial report.

The Restaurant Admin `Reports` module is the current Reporting consumer. It uses `reports.view` and generates a Monthly P&L or Yearly/YTD P&L poster preview only when the Admin selects Generate. Its scope selector supports an individual accessible outlet or `All Outlets`. `All Outlets` is a server-derived set of only the caller's authorized outlets, never a browser loop or an unrestricted outlet set. The Reporting scope RPCs consolidate financial totals and product/category rows server-side; a financial metric remains missing when any scoped outlet lacks that source period, and Product Analytics remains unavailable/incomplete until every scoped outlet has an eligible completed source report. Outlet-list visibility for Reports is separately RLS-scoped by the same `reports.view` plus server outlet-scope predicate as the Reporting RPCs. The fixed-ratio React/HTML posters consume the `reportingService` dataset without direct Supabase queries or financial recomputation. Incomplete financial inputs and unavailable product data remain visible states, rather than being coerced to RM0 or blocking financial output.
...
Monthly and Yearly posters share a fixed-canvas editorial visual system: presentation primitives own the brand-neutral poster typography, canonical outlet identity treatment, financial story, ranking, trend, table, and status while the Admin page continues to own controls and preview containment. An Admin with `reports.export` can download the currently generated poster as a high-resolution PNG or single-page PDF. Both Monthly Profit and Yearly P&L use one canonical A4 portrait composition across preview and export: PDF is a single 210 × 297 mm page and PNG is 2480 × 3508 px. Both formats capture that same fixed logical React/HTML poster canvas and its current `reportingService` dataset; export neither re-queries Reporting nor recalculates finance. Report history/snapshots, sharing, scheduling, and AI insights remain deferred.
