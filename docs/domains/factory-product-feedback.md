...
- Before responses exist, Form Builder changes save in place. After responses exist, helper copy, translations, and other presentation-only localization can update the active version in place; structural or answer-semantic edits remain a local draft until an explicit new form version is confirmed. The authenticated form read model exposes compact active and historical version metadata; existing response snapshots and version links are never rewritten.
