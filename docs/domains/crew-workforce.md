...
For fixed-outlet Crew, Special Access remains on the employee's active Crew Access record and applies only to the canonical workplace outlet. For Management Crew, Special Access is stored per employee and authorized outlet; the employee's current Role outlet set limits where an Admin can grant it and where a Crew action can execute. Role access alone grants no Cash Handover, Asset, Stock Check or PO mutation. Independent grants cover Cash Handover initiation; Asset creation, details, adjustment and inspection; and Restaurant Inventory scheduled Stock Check, Audit Stock Check, PO creation/management and PO receiving. Every Management mutation revalidates the opaque session, current Role outlet membership, and the grant for the explicit outlet, then uses the owning domain's lifecycle authority. Historical employee-global Management flags do not grant actions at any outlet. Grant changes record actor, outlet, before/after state, and timestamp in audit evidence. Asset creation remains limited to a new outlet asset and initial photo.

Management Crew keeps identity, context and eligibility distinct. `crew_outlet_scope` derives the authorized active-outlet set from the current employee Role and projects each outlet's live Special Access. The client remembers a selected outlet per employee, revalidates it on startup/focus/switch, and invalidates only outlet-scoped reads when it changes. Home task context uses a non-materializing read-only projection; selected-outlet SOP Library is published-reference-only and creates no assignment or acknowledgement. Assets remain read-only where no outlet Asset grant exists; the relevant Asset actions appear only for an outlet with its own grants. The Cash surface for Management exposes deposit and handover only where that outlet's Handover grant exists; Cash Checkout remains unavailable. Personal roster, Attendance, Leave, Onboarding progress, Growth, Performance, Reward, Notifications and People records remain employee-global. Management Leave reads existing employee-owned history and balances without creating an entitlement from operational context; a new application requires an actual employment outlet. Growth/Performance/Reward retain their existing employment attribution and may be unavailable without one. Clock In/Out still requires an actual published working shift, not an outlet selection. Existing fixed-outlet Crew behavior remains unchanged. Notification destination outlet is resolved from the live source record, never from its snapshot.
...
Crew Access, Leave Requests, and grouped Leave Balances use separate server-paged Admin projections. Crew Access provisioning lists employees by Employee Master Workplace: an outlet page contains only that outlet's employees, while the explicit Management scope contains only Management employees and requires all-outlet Admin access. Management rows show the existing active Role's outlet access as read-only context, never as a second credential or outlet assignment. Search and employment-status filters execute before the authoritative count and page; balance paging does not change the canonical entitlement or calculation authority.
...
Admins enable/reset/disable Crew Access and configure Crew capabilities through the separate Special Access workflow; those capabilities are not Admin roles. A Management employee retains one Crew Access identity and passcode, while the Admin editor selects one of that employee's currently authorized Role outlets before changing its grants. Fixed-outlet Crew keep the one-outlet editor. Admins also prepare and publish rosters, review attendance evidence, configure leave policy/entitlement, and decide leave requests.
