...
Factory List UI Foundation V1 begins with Batch Traceability. Its shared filter rail, compact table treatment, status badge mapping, date display helper, row-detail action, pagination, and list states are presentation-only primitives: they must preserve each listing's existing query, service, permission, and detail authority contracts. The primitives resolve all surfaces, controls, table states, and floating layers through FeedX theme tokens in light and dark modes. Other Factory pages adopt the foundation only through deliberate follow-up migrations.
