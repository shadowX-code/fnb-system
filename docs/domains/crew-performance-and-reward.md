...
Crew trend presentation is a read-only comparison of the current score with the prior finalized comparable period. A shared presentation formatter rounds its magnitude to at most one decimal, normalizes floating-point no-change residue to zero, and never changes the server score, period selection, or Reward authority.
