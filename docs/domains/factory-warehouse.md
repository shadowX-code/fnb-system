...
Positive Raw Material Stock Check variance is represented on approval by an active, source-linked `stock_check_adjustment` reconciliation batch when no existing valid batch is available. Its Malaysia-business-date reference is `ADJ-RMSC<YYMMDD>-<NN>` and it is eligible for the established FEFO allocation authority without impersonating Receiving. Historical adjustment batches are repaired only where the approved Stock Check, linked movement, active storage Location, and aggregate-to-batch reconciliation prove ownership exactly; repair activates the existing batch without changing inventory or adding movement evidence.
