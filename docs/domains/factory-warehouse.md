...
Transfers, issues, returns, adjustments, and stock checks use controlled lifecycle paths. Stock Check drafts support shared Count All and Skip All actions; a skipped row is excluded from the count snapshot, is never treated as a zero physical count, and may retain an optional operator note. Counted variance rows still require a reason on submission. A positive Finished Goods Stock Check variance may credit a valid existing batch or, where no batch exists, create one traceable `adjustment` reconciliation batch only on approval. The reconciliation batch is linked to the Stock Check and SKU; it is never represented as Factory Production. Draft and submitted checks preserve only the proposed reconciliation intent and do not change inventory. Approval posts exactly one ledger movement per variance item through the canonical Finished Goods balance authority.
Positive Raw Material Stock Check variance is represented on approval by an active, source-linked `stock_check_adjustment` reconciliation batch when no existing valid batch is available. Its Malaysia-business-date reference is `ADJ-RMSC<YYMMDD>-<NN>` and it is eligible for the established FEFO allocation authority without impersonating Receiving. Legacy adjustment-batch repair is fail-closed: each forward repair must explicitly whitelist immutable batch IDs and assert approved Stock Check provenance, one linked movement, active storage, and aggregate-to-batch reconciliation before it can activate existing batch authority without changing inventory or adding movement evidence.
Raw Material Movement list and detail display project the immutable `raw_material_batch_balance_id` when present, so Receiving, Production, Stock Check Adjustment, and Transfer evidence use the same canonical batch identity without creating or inferring a new batch.
Internal Transfers move only positive, exact Raw Material or Finished Good batch balances between different active, storage-enabled Factory Locations. Each completed, immutable `TR<YYMMDD>-<NN>` document is request-id idempotent and atomically writes paired `Transfer Out` and `Transfer In` movement evidence. A destination balance is a provenance-linked location projection of the original batch: item/SKU identity, lot/batch number, manufacture and expiry data remain intact; no Production, Receiving, or Stock Check adjustment batch is created, and aggregate stock never changes. Source-balance locks prevent concurrent over-transfer.
...
References use the Malaysia business date and a transaction-locked daily `PC<YYMMDD>-<NN>` sequence. Expense Categories are active/inactive master records with retained historical references. Receipts are optional private image/PDF objects exposed through short-lived signed URLs to users with Petty Cash view access. Image receipts are signature-validated; reasonably sized images remain unchanged, while materially oversized images are resized once to an approximately 2000px long edge with readability-first encoding. PDFs remain unchanged. Superseded or deleted Draft evidence is removed only after it is no longer referenced; Posted evidence remains immutable through reversal. All mutations use permission-checked trusted RPCs and write audit evidence.
