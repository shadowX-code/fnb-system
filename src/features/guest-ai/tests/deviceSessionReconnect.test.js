...
describe("Guest AI device reconnect session", () => {
  ...
  it("retains non-sensitive end-of-speech evidence from the completed capture", () => {
    ...
    session.receive(JSON.stringify({ protocol_version: "1.0", message_id: "complete", type: "audio_capture_complete", sent_at: "now", payload: { turn_id: "turn-eos", byte_count: 4, duration_ms: 1000, completion: "completed", speech_detected: true, auto_stop: true, current_rms: 20, noise_floor_rms: 20, speech_threshold_rms: 450, estimated_speech_end_elapsed_ms: 280, post_speech_silence_ms: 720, time_saved_ms: 5000 } }));
    expect(session.snapshot.audio).toMatchObject({ state: "COMPLETE", auto_stop: true, time_saved_ms: 5000, captureDiagnostics: { audio_capture_speech_detected: { elapsed_ms: 260 } } });
  });
