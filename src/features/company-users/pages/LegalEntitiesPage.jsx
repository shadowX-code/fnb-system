...
export default function LegalEntitiesPage({ ui, auth }) {
  ...
  const [actionEntityId, setActionEntityId] = useState(null);
  const [busy, setBusy] = useState(false);
  ...
  const columns = [
    ...
    { key: "actions", header: "", align: "right", width: "80px", render: (row) => canManage ? <ActionMenu open={actionEntityId === row.id} onOpenChange={(open) => setActionEntityId(open ? row.id : null)} trigger={({ toggle, ariaLabel }) => <button className="icon-btn" type="button" aria-label={ariaLabel} onClick={toggle}><MoreHorizontal size={18} /></button>}><button className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left font-semibold hover:bg-slate-50" type="button" onClick={() => { setFormEntity(row); setActionEntityId(null); }}><Edit3 size={15} /> Edit</button><button className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left font-semibold hover:bg-slate-50" type="button" onClick={() => { setTemplatesEntity(row); setActionEntityId(null); }}><FileText size={15} /> Contract templates</button>{row.is_active ? <button className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left font-semibold text-rose-700 hover:bg-rose-50" type="button" onClick={() => { setDeactivateEntity(row); setActionEntityId(null); }}><Power size={15} /> Deactivate</button> : <button className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left font-semibold text-emerald-800 hover:bg-emerald-50" type="button" onClick={() => { setFormEntity({ ...row, is_active: true }); setActionEntityId(null); }}><Power size={15} /> Activate</button>}</ActionMenu> : <span className="text-text-muted">—</span> },
  ];
  ...
  return <div className="space-y-4">
    ...
    <Card>{loading ? <div className="p-8 text-center text-sm font-semibold text-text-secondary">Loading Legal Entities...</div> : error ? <div className="p-8 text-center text-sm font-semibold text-rose-700">{error}</div> : filteredEntities.length ? <DataTable columns={columns} rows={filteredEntities} getRowKey={(row) => row.id} density="compact" tableClassName="min-w-[860px]" onRowClick={canManage ? (row) => setFormEntity(row) : undefined} /> : <div className="p-8 text-center"><div className="text-sm font-bold text-text-primary">No Legal Entities found.</div><p className="mt-1 text-sm text-text-secondary">Add a legal employer before assigning one to an employee.</p></div>}</Card>
