...
export default function CrewPurchaseOrdersMobile({ token, outletId, grants, initialTarget, onBack, onFlowChange }) {
  ...
  if (mode === "suggestions") return <section className="crew-inventory-page"><CrewMobileDetailHeader title={t("inventory.reviewPurchaseSuggestions")} subtitle={source?.check_name} onBack={goBack} />
    ...
    {Object.entries(sourceGroups).map(([id, group]) => <section className="crew-inventory-supplier-group" key={id}>
      ...
      {group.map((item) => <article className="crew-inventory-suggestion" key={item.stock_check_item_id}>
        <label className="crew-inventory-suggestion-name"><input type="checkbox" checked={item.included} disabled={!item.suppliers.length} onChange={(event) => updateSourceItem(item.stock_check_item_id, { included: event.target.checked })} /><strong>{item.item_name}</strong></label>
