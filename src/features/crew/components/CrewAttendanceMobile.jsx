...
export function CrewClockDialogs({ clock, context, navigate }) {
  ...
  return <>
    ...
    {clockSuccess && <CrewMobileModal title={t("home.clockedInSuccess")} onClose={() => setClockSuccess(null)} contentClassName="crew-home-success-content" footer={<><button type="button" className="crew-mobile-primary" onClick={() => { setClockSuccess(null); navigate("home"); }}>{t("home.goHome")}</button><button type="button" className="crew-mobile-secondary" onClick={() => { setClockSuccess(null); navigate("attendance"); }}>{t("home.viewAttendance")}</button></>}><span className="crew-ui-icon-container"><Check size={28} /></span><dl><div><dt>{t("home.clockInTime")}</dt><dd>{formatTime(clockSuccess.time)}</dd></div><div><dt>{t("common.outlet")}</dt><dd>{clockSuccess.outlet}</dd></div><div><dt>{t("common.role")}</dt><dd>{clockSuccess.role}</dd></div></dl></CrewMobileModal>}
  </>;
}
