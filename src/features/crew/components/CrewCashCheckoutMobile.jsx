...
import CrewMoneyField from "./CrewMoneyField.jsx";
import CrewPersonPicker from "./CrewPersonPicker.jsx";
...
const safeCashActor = (value) => String(value || "").includes("@") ? "Admin" : value;
...
function RecentActivityRow({ row, onOpen, interactive = true }) {
  ...
  const actor = row.handover_from && row.handover_to ? `${safeCashActor(row.handover_from)} → ${safeCashActor(row.handover_to)}` : safeCashActor(row.receiver_name || row.recorded_by || row.employee_name);
  ...
  const activity = checkout ? t("cash.ledgerCheckout") : row.activity === "Cash Handover" ? t("cash.handOverCash") : row.activity === "Cash Collection" ? t("cash.ledgerCollection") : row.activity || t("cash.ledgerCollection");
...
function CollectionSheet({ data, token, onClose, onSaved }) {
  ...
  const amountValid = Number.isFinite(amount) && amount >= 0.05 && amount <= Number(balance || 0);
