  ...
  return <section className="crew-cash-mobile crew-cash-flow">
    ...
    {!data?.can_perform ? <CrewEmptyState title={t("cash.notAssigned")} body={t("cash.notAssignedBody")} /> : <>
      ...
      <footer className="crew-cash-actions">
        ...
        {step === "confirm" && <button className="crew-mobile-primary" type="button" onClick={() => onSave(requiresReview ? "submit" : "complete")} disabled={saving || (requiresReview && data?.checkout?.status === "submitted")}>{saving ? t("common.saving") : requiresReview && data?.checkout?.status === "submitted" ? t("cash.awaitingReview") : requiresReview ? t("cash.submitForReview") : t("cash.completeCheckout")}</button>}
      </footer>
    </>}
  </section>;
}
...
function CountStep({ data, draft, setDraft, counted, expectedOpening, floating, previousCarry }) { const { t } = useTranslation(); const hasActualOpening = draft.actual_opening_cash !== "" && draft.actual_opening_cash != null; return <section className="crew-cash-card"><header><Banknote size={20} /><div><h2>{t("cash.countCash")}</h2><p>{t("cash.countHelp")}</p></div></header><div className="crew-cash-opening-context"><div><small>{t("cash.floatToKeep")}</small><strong>{money(floating)}</strong></div><div><small>{t("cash.carryForward")}</small><strong>{money(previousCarry)}</strong></div></div><div className="crew-cash-opening"><label>{t("cash.expectedOpening")}<strong>{money(expectedOpening)}</strong></label><label>{t("cash.actualOpening")}<input type="number" min="0" step="0.05" value={draft.actual_opening_cash} onChange={(event) => setDraft({ ...draft, actual_opening_cash: event.target.value })} /></label></div>{hasActualOpening && Number(draft.actual_opening_cash) !== expectedOpening && <label className="crew-cash-field">{t("cash.openingReason")}<textarea required value={draft.opening_variance_reason} onChange={(event) => setDraft({ ...draft, opening_variance_reason: event.target.value })} /></label>}<div className="crew-cash-denominations">{DENOMINATIONS.map((denomination) => { const key = denominationKey(denomination); const quantity = Number(draft.denomination_counts[key] || 0); return <label key={key}><b>RM {denomination.toFixed(denomination < 1 ? 2 : 0)}</b><input aria-label={`RM ${denomination}`} inputMode="numeric" type="number" min="0" step="1" value={draft.denomination_counts[key] || ""} onChange={(event) => setDraft({ ...draft, denomination_counts: { ...draft.denomination_counts, [key]: event.target.value } })} /><small>{money(denomination * quantity)}</small></label>; })}</div><div className="crew-cash-total"><span>{t("cash.countedCash")}</span><strong>{money(counted)}</strong></div><label className="crew-cash-field">{t("cash.posExpected")}<input type="number" min="0" step="0.05" value={draft.pos_expected_cash} onChange={(event) => setDraft({ ...draft, pos_expected_cash: event.target.value })} /></label></section>; }
...
function ConfirmStep({ data, draft, setDraft, counted, variance, deposit, floating, previousCarry }) { const { t } = useTranslation(); const requiresReason = Math.max(0, floating - counted) > 0 || (variance != null && Math.abs(variance) > Number(data.settings.variance_tolerance || 0)); return <section className="crew-cash-card"><header><ShieldCheck size={20} /><div><h2>{t("cash.reviewCheckout")}</h2><p>{t("cash.serverCalculated")}</p></div></header><dl className="crew-cash-breakdown"><div><dt>{t("cash.floatToKeep")}</dt><dd>{money(floating)}</dd></div><div><dt>{t("cash.carryForward")}</dt><dd>{money(previousCarry)}</dd></div><div><dt>{t("cash.countedCash")}</dt><dd>{money(counted)}</dd></div><div><dt>{t("cash.posExpected")}</dt><dd>{money(draft.pos_expected_cash)}</dd></div><div><dt>{t("cash.variance")}</dt><dd className={variance === 0 ? "is-balanced" : "is-warning"}>{variance > 0 ? "+" : ""}{money(variance)}</dd></div><div><dt>{t("cash.carryForward")}</dt><dd>{money(draft.carry_forward)}</dd></div><div className="is-total"><dt>{t("cash.forDeposit")}</dt><dd>{money(deposit)}</dd></div><div><dt>{t("cash.depositBalance")}</dt><dd>{money(data?.deposit?.current_balance)}</dd></div></dl>{requiresReason && <><p className="crew-cash-warning">{data.checkout?.review_required ? t("cash.managerReviewHelp") : t("cash.reasonRequired")}</p><label className="crew-cash-field">{t("cash.varianceReason")}<textarea required value={draft.variance_reason} onChange={(event) => setDraft({ ...draft, variance_reason: event.target.value })} /></label></>}</section>; }

function LedgerRow({ row }) { const { t } = useTranslation(); const isOut = Number(row.signed_amount) < 0; const checkout = row.entry_type === "checkout_due" || !isOut; const actor = row.receiver_name || row.recorded_by || row.employee_name; return <article><span><strong>{checkout ? t("cash.ledgerCheckout") : t("cash.ledgerCollection")}</strong><small>{formatCrewOperationalDateTime(row.occurred_at)}</small>{actor && <small>{formatCrewEmployee(actor)}</small>}{row.status === "pending_receipt" && <CrewStatusBadge tone="warning">{t("cash.pendingReceipt")}</CrewStatusBadge>}<small>{t("cash.ledgerBalance", { amount: money(row.balance_after) })}</small></span><em className={isOut ? "is-out" : "is-in"}>{isOut ? "−" : "+"}{money(Math.abs(row.signed_amount))}</em></article>; }
