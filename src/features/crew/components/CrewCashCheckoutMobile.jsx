...
export default function CrewCashCheckoutMobile({ token, onBack }) {
  ...
  if (error && !data) return <section className="crew-cash-mobile"><CrewMobileDetailHeader title={t("cash.title")} onBack={onBack} variant="workflow" /><CrewEmptyState title={t("cash.unableLoad")} body={error} /><button className="crew-cash-primary" type="button" onClick={load}><RefreshCw size={17} />{t("common.retry")}</button></section>;
