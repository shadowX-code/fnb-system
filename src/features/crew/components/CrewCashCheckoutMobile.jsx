...
function RecentActivityRow({ row, onOpen, interactive = true }) {
  ...
  return <article className="crew-cash-activity-row"><span className={`crew-ui-icon-container crew-ui-icon-container--round crew-cash-activity-icon${isOut ? " is-danger" : ""}`}>{isOut ? <ArrowDown size={20} /> : <ArrowUp size={20} />}</span><div className="crew-cash-activity-copy"><strong>{checkout ? t("cash.ledgerCheckout") : t("cash.ledgerCollection")}</strong><small>{formatCrewOperationalDateTime(row.occurred_at)}</small>{actor && <small>{formatCrewEmployee(actor)}</small>}{confirmation && <CrewStatusBadge tone={confirmation === "pending_confirmation" ? "warning" : confirmation === "confirmed" ? "success" : "neutral"}>{t(`cash.confirmation.${confirmation}`)}</CrewStatusBadge>}</div><div className="crew-cash-activity-amount"><em className={isOut ? "is-out" : "is-in"}>{isOut ? "−" : "+"}{money(Math.abs(row.signed_amount))}</em><small>{t("cash.ledgerBalance", { amount: money(row.balance_after) })}</small></div>{interactive && <button type="button" aria-label={t("cash.openLedgerEntry", { activity: checkout ? t("cash.ledgerCheckout") : t("cash.ledgerCollection") })} onClick={onOpen}><ChevronRight size={18} /></button>}</article>;
}
...
function CollectionSheet({ data, token, onClose, onSaved }) { const { t } = useTranslation(); const balance = data.deposit.current_balance; const [form, setForm] = useState({ request_id: crypto.randomUUID(), receiver_type: "internal", receiver_employee_id: "", receiver_name: "", amount: "", purpose: "Cash deposit collection", note: "" }); const [saving, setSaving] = useState(false); const [error, setError] = useState(""); async function submit(event) { event.preventDefault(); setSaving(true); setError(""); try { await crewService.recordCashCollection(token, form); await onSaved(); } catch (cause) { setError(cause.message); } finally { setSaving(false); } } return <div className="crew-cash-sheet-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><form className="crew-cash-sheet" onSubmit={submit}><header><h2>{t("cash.recordCollection")}</h2><p>{t("cash.cashDepositBalance")}: {money(balance)}</p></header><SelectField label={t("cash.receiverType")} ariaLabel={t("cash.receiverType")} value={form.receiver_type} onChange={(receiver_type) => setForm({ ...form, receiver_type, receiver_employee_id: "", receiver_name: "" })} options={[{ value: "internal", label: t("cash.internalCrew") }, { value: "external", label: t("cash.externalReceiver") }]} />{form.receiver_type === "internal" ? <SelectField label={t("cash.receiver")} ariaLabel={t("cash.receiver")} value={form.receiver_employee_id} placeholder={t("cash.chooseReceiver")} onChange={(receiver_employee_id) => setForm({ ...form, receiver_employee_id })} options={data.receivers.map((row) => ({ value: row.id, label: `${row.name} · ${row.position}` }))} searchable /> : <label>{t("cash.receiverName")}<input required value={form.receiver_name} onChange={(event) => setForm({ ...form, receiver_name: event.target.value })} /></label>}<label>{t("cash.amount")}<input required type="number" min="0.05" max={Number(balance)} step="0.05" value={form.amount} onChange={(event) => setForm({ ...form, amount: event.target.value })} /></label><label>{t("cash.purpose")}<input required value={form.purpose} onChange={(event) => setForm({ ...form, purpose: event.target.value })} /></label><label>{t("cash.noteOptional")}<textarea value={form.note} onChange={(event) => setForm({ ...form, note: event.target.value })} /></label>{error && <p className="crew-cash-error">{error}</p>}<footer><button type="button" className="crew-mobile-secondary" onClick={onClose}>{t("common.cancel")}</button><button className="crew-mobile-primary" disabled={saving || (form.receiver_type === "internal" && !form.receiver_employee_id)}>{saving ? t("common.saving") : t("cash.recordCollection")}</button></footer></form></div>; }
