...
function RecentActivityRow({ row, onOpen }) {
  ...
  return <article className="crew-cash-activity-row"><span className={`crew-ui-icon-container crew-ui-icon-container--round crew-cash-activity-icon ${isOut ? "is-danger" : "is-selected"}`}>{isOut ? <ArrowDown size={20} /> : <ArrowUp size={20} />}</span><div className="crew-cash-activity-copy"><strong>{checkout ? t("cash.ledgerCheckout") : t("cash.ledgerCollection")}</strong><small>{formatCrewOperationalDateTime(row.occurred_at)}</small>{actor && <small>{formatCrewEmployee(actor)}{row.status ? ` · ${t(`cash.status.${row.status}`)}` : ""}</small>}</div><div className="crew-cash-activity-amount"><em className={isOut ? "is-out" : "is-in"}>{isOut ? "−" : "+"}{money(Math.abs(row.signed_amount))}</em><small>{t("cash.ledgerBalance", { amount: money(row.balance_after) })}</small></div><button type="button" aria-label={t("cash.openLedgerEntry", { activity: checkout ? t("cash.ledgerCheckout") : t("cash.ledgerCollection") })} onClick={onOpen}><ChevronRight size={18} /></button></article>;
}
