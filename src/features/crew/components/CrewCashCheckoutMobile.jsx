...
const cashSummaryDate = (value, translate) => {
  if (!value) return translate("cash.today");
...
function PendingReceipts({ rows, token, onChanged }) { const { t } = useTranslation(); const [confirming, setConfirming] = useState(null); const [saving, setSaving] = useState(false); async function confirm() { setSaving(true); try { await crewService.confirmCashCollection(token, confirming.id, confirming.amount); await onChanged(); setConfirming(null); } finally { setSaving(false); } } return <section className="crew-cash-receipts">{rows.length ? <div>{rows.map((row) => <article key={row.id}><span className="crew-ui-icon-container crew-ui-icon-container--small"><HandCoins size={16} /></span><div className="crew-cash-receipt-copy"><strong>{money(row.amount)}</strong>{row.sender && <small>{t("cash.handedOverBy", { name: formatCrewEmployee(row.sender) })}</small>}{row.outlet_name && <small>{row.outlet_name}</small>}{row.occurred_at && <small>{formatCrewOperationalDateTime(row.occurred_at)}</small>}{row.note && <small>{row.note}</small>}</div><div className="crew-cash-receipt-action"><CrewStatusBadge tone="warning">{t("cash.confirmation.pending_confirmation")}</CrewStatusBadge><button className="crew-mobile-primary" type="button" onClick={() => setConfirming(row)}>{t("cash.confirmReceived")}</button></div></article>)}</div> : <CrewEmptyState title={t("cash.noPendingConfirmations")} />}{confirming && <CrewMobileModal title={t("cash.confirmCashReceived")} description={`${money(confirming.amount)} · ${formatCrewEmployee(confirming.sender)}`} onClose={() => setConfirming(null)} closeDisabled={saving} footer={<><button className="crew-mobile-secondary" type="button" disabled={saving} onClick={() => setConfirming(null)}>{t("common.cancel")}</button><button className="crew-mobile-primary" type="button" disabled={saving} onClick={confirm}>{saving ? t("common.saving") : t("cash.confirmReceived")}</button></>} />}</section>; }
...
function CollectionSheet({ data, token, onClose, onSaved }) {
  ...
  const matchingReceivers = data.receivers.filter((row) => `${row.name} ${row.position}`.toLowerCase().includes(receiverSearch.trim().toLowerCase()));
  ...
  return <CrewMobileModal title={t("cash.handOverCash")} description={`${t("cash.cashDepositBalance")}: ${money(balance)}`} onClose={onClose} closeDisabled={saving} className="crew-cash-collection-modal" onSubmit={(event) => { event.preventDefault(); review ? submit() : setReview(true); }} footer={<><button type="button" className="crew-mobile-secondary" disabled={saving} onClick={review ? () => setReview(false) : onClose}>{review ? t("common.back") : t("common.cancel")}</button><button className="crew-mobile-primary" disabled={saving || !form.receiver_employee_id || !form.amount}>{saving ? t("common.saving") : review ? t("cash.confirmHandoverAction") : t("common.continue")}</button></>}>
        {review ? <dl className="crew-cash-handover-review"><div className="is-amount"><dt>{t("cash.amount")}</dt><dd>{money(form.amount)}</dd></div><div><dt>{t("cash.from")}</dt><dd>{data.initiator_name || "—"}</dd></div><div><dt>{t("cash.to")}</dt><dd>{receiver?.name || "—"}</dd></div>{form.note && <div><dt>{t("cash.noteOptional")}</dt><dd>{form.note}</dd></div>}</dl> : <><div className="crew-cash-handover-meta"><span>{t("cash.receiverType")}</span><strong>{outletCrew}</strong></div><div className="crew-ui-form-field"><span>{t("cash.receiver")}</span><button className="crew-ui-field" type="button" aria-label={t("cash.receiver")} aria-haspopup="dialog" aria-expanded={receiverOpen} onClick={() => { setReceiverSearch(""); setReceiverOpen(true); }}>{receiver ? `${receiver.name} · ${receiver.position}` : t("cash.chooseReceiver")}</button></div><label className="crew-ui-form-field"><span>{t("cash.amount")}</span><span className="crew-cash-currency-field"><b>RM</b><input required aria-label={t("cash.amount")} inputMode="decimal" type="number" min="0.05" max={Number(balance)} step="0.05" value={form.amount} onChange={(event) => setForm({ ...form, amount: event.target.value })} /></span></label><label className="crew-ui-form-field"><span>{t("cash.noteOptional")}</span><textarea rows="3" value={form.note} onChange={(event) => setForm({ ...form, note: event.target.value })} /></label></>}
        {error && <p className="crew-cash-error" role="alert">{error}</p>}
        {receiverOpen && <CrewBottomSheet title={t("cash.chooseReceiver")} onClose={() => setReceiverOpen(false)}><div className="crew-ui-form-field"><input className="crew-ui-field" aria-label={t("cash.receiver")} placeholder={t("cash.receiver")} value={receiverSearch} onChange={(event) => setReceiverSearch(event.target.value)} /></div><div className="crew-ui-choice-list crew-ui-choice-list--mint">{matchingReceivers.map((row) => <button key={row.id} type="button" className={row.id === form.receiver_employee_id ? "is-selected" : ""} aria-pressed={row.id === form.receiver_employee_id} onClick={() => { setForm({ ...form, receiver_employee_id: row.id }); setReceiverOpen(false); }}>{row.name} · {row.position}</button>)}{!matchingReceivers.length && <p role="status">No options found</p>}</div></CrewBottomSheet>}
  </CrewMobileModal>;
}
