  ...
  return <section className="crew-cash-mobile">
    ...
    <div id="crew-cash-checkout-flow">{!data?.can_perform ? <CrewEmptyState title={t("cash.notAssigned")} body={t("cash.notAssignedBody")} /> : completed ? <p className="crew-cash-complete-note"><Check size={16} />{t("cash.completedAt", { time: time(data.checkout.completed_at) })}</p> : <>
      ...
      <footer className="crew-cash-actions">
        ...
        {step === "count" && <><button type="button" className="crew-mobile-secondary" onClick={() => save("draft")} disabled={saving}>{t("cash.saveDraft")}</button><button className="crew-mobile-primary" type="button" onClick={() => save("reconcile")} disabled={saving || posExpected == null}>{t("cash.reconcile")}<ArrowRight size={16} /></button></>}
        {step === "allocate" && <button className="crew-mobile-primary" type="button" onClick={() => setStep("confirm")} disabled={saving}>{t("cash.reviewSubmit")}<ArrowRight size={16} /></button>}
        {step === "confirm" && <button className="crew-mobile-primary" type="button" onClick={() => save(requiresReview ? "submit" : "complete")} disabled={saving || (requiresReview && data?.checkout?.status === "submitted")}>{saving ? t("common.saving") : requiresReview && data?.checkout?.status === "submitted" ? t("cash.awaitingReview") : requiresReview ? t("cash.submitForReview") : t("cash.completeCheckout")}</button>}
      </footer>
    </>}</div>
