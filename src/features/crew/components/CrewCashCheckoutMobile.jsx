...
function DetailSection({ title, rows }) { return <section className="crew-cash-detail-card"><h3>{title}</h3><dl className="crew-cash-breakdown">{rows.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value == null ? "—" : money(value)}</dd></div>)}</dl></section>; }
