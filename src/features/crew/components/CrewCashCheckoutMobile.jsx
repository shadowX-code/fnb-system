...
function CollectionSheet({ data, token, onClose, onSaved }) { const { t } = useTranslation(); const balance = data.deposit.current_balance; const [review, setReview] = useState(false); const [form, setForm] = useState({ request_id: crypto.randomUUID(), receiver_employee_id: "", amount: "", purpose: "Cash deposit collection", note: "" }); const [saving, setSaving] = useState(false); const [error, setError] = useState(""); const receiver = data.receivers.find((row) => row.id === form.receiver_employee_id); async function submit() { setSaving(true); setError(""); try { await crewService.recordCashCollection(token, form); await onSaved(); } catch (cause) { setError(cause.message); } finally { setSaving(false); } } return <div className="crew-cash-sheet-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><form className="crew-cash-sheet" onSubmit={(event) => { event.preventDefault(); review ? submit() : setReview(true); }}><header><h2>{t("cash.handOverCash")}</h2><p>{t("cash.cashDepositBalance")}: {money(balance)}</p></header>{review ? <><p>{t("cash.amount")}: {money(form.amount)}<br />{t("cash.from")}: {data.initiator_name || "—"}<br />{t("cash.to")}: {receiver?.name}<br />{t("cash.purpose")}: {form.purpose}</p></> : <><SelectField label={t("cash.receiver")} ariaLabel={t("cash.receiver")} value={form.receiver_employee_id} placeholder={t("cash.chooseReceiver")} onChange={(receiver_employee_id) => setForm({ ...form, receiver_employee_id })} options={data.receivers.map((row) => ({ value: row.id, label: `${row.name} · ${row.position}` }))} searchable /><label>{t("cash.amount")}<input required type="number" min="0.05" max={Number(balance)} step="0.05" value={form.amount} onChange={(event) => setForm({ ...form, amount: event.target.value })} /></label><label>{t("cash.purpose")}<input required value={form.purpose} onChange={(event) => setForm({ ...form, purpose: event.target.value })} /></label><label>{t("cash.noteOptional")}<textarea value={form.note} onChange={(event) => setForm({ ...form, note: event.target.value })} /></label></>}{error && <p className="crew-cash-error">{error}</p>}<footer><button type="button" className="crew-mobile-secondary" onClick={review ? () => setReview(false) : onClose}>{review ? t("common.back") : t("common.cancel")}</button><button className="crew-mobile-primary" disabled={saving || !form.receiver_employee_id || !form.amount}>{saving ? t("common.saving") : review ? t("cash.confirmHandover", { amount: money(form.amount) }) : t("common.continue")}</button></footer></form></div>; }
