...
const time = (value) => value ? new Intl.DateTimeFormat("en-MY", { hour: "numeric", minute: "2-digit", timeZone: "Asia/Kuala_Lumpur" }).format(new Date(value)) : "—";
...
export default function CrewCashCheckoutMobile({ token, onBack }) {
  ...
  return <section className="crew-cash-mobile">
    ...
    <section className="crew-cash-summary">
      <article className="crew-cash-today-summary"><WalletCards size={21} /><span><small>{t("cash.todayCheckout")}</small><strong>{data?.checkout ? t(`cash.status.${data.checkout.status}`) : t("cash.notStarted")}</strong></span><CrewStatusBadge tone={completed ? "success" : data?.checkout?.review_required ? "warning" : "neutral"}>{data?.checkout?.review_required ? t("cash.reviewRequired") : completed ? t("status.completed") : t("cash.today")}</CrewStatusBadge><dl><div><dt>{t("cash.expectedOpening")}</dt><dd>{money(data?.checkout?.expected_opening_cash ?? floating)}</dd></div><div><dt>{t("cash.countedCash")}</dt><dd>{data?.checkout ? money(data.checkout.counted_cash) : "—"}</dd></div><div><dt>{t("cash.variance")}</dt><dd>{data?.checkout?.variance == null ? "—" : money(data.checkout.variance)}</dd></div><div><dt>{t("cash.forDeposit")}</dt><dd>{data?.checkout ? money(data.checkout.amount_for_deposit) : "—"}</dd></div></dl><button type="button" onClick={() => document.getElementById("crew-cash-checkout-flow")?.scrollIntoView({ behavior: "smooth" })}>{completed ? t("cash.viewCheckout") : data?.checkout ? t("cash.continueCheckout") : t("cash.startCheckout")}<ChevronRight size={15} /></button></article>
      <article><HandCoins size={21} /><span><small>{t("cash.depositBalance")}</small><strong>{money(data?.deposit?.current_balance)}</strong></span><button type="button" onClick={() => document.getElementById("crew-cash-ledger")?.scrollIntoView({ behavior: "smooth" })}>{t("cash.viewLedger")}<ChevronRight size={15} /></button></article>
    </section>

    {data?.pending_receipts?.length > 0 && <PendingReceipts rows={data.pending_receipts} token={token} onChanged={load} />}

    <div id="crew-cash-checkout-flow">{!data?.can_perform ? <CrewEmptyState title={t("cash.notAssigned")} body={t("cash.notAssignedBody")} /> : completed ? <CompletedCheckout checkout={data.checkout} /> : <>
      <StepBar step={step} />
    ...
    <section className="crew-cash-ledger" id="crew-cash-ledger"><header><div><h2>{t("cash.depositLedger")}</h2><p>{t("cash.ledgerHelp")}</p></div>{data?.can_record_collection && Number(data?.deposit?.current_balance || 0) > 0 && <button type="button" onClick={() => setCollectionOpen(true)}>{t("cash.recordCollection")}</button>}</header>{data?.deposit?.recent?.length ? <div>{data.deposit.recent.map((row) => <article key={row.id}><span><strong>{row.activity}</strong><small>{time(row.occurred_at)}{row.receiver_name ? ` · ${row.receiver_name}` : ""}</small></span><em className={Number(row.signed_amount) < 0 ? "is-out" : "is-in"}>{Number(row.signed_amount) > 0 ? "+" : "−"}{money(Math.abs(row.signed_amount))}</em></article>)}</div> : <CrewEmptyState title={t("cash.noLedger")} body={t("cash.noLedgerBody")} />}</section>
...
function CompletedCheckout({ checkout }) { const { t } = useTranslation(); return <section className="crew-cash-complete"><span><Check size={23} /></span><h2>{t("cash.checkoutComplete")}</h2><p>{t("cash.completedAt", { time: time(checkout.completed_at) })}</p><dl><div><dt>{t("cash.countedCash")}</dt><dd>{money(checkout.counted_cash)}</dd></div><div><dt>{t("cash.variance")}</dt><dd>{money(checkout.variance)}</dd></div><div><dt>{t("cash.forDeposit")}</dt><dd>{money(checkout.amount_for_deposit)}</dd></div></dl></section>; }
