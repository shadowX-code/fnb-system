...
import { ArrowLeft, ArrowRight, Check, ChevronRight, ClipboardCheck, History, MapPin, PackagePlus, PackageSearch, Pencil, SlidersHorizontal } from "lucide-react";
...
import CrewAssetMasterPhotoField, { CrewAssetMasterImage } from "./CrewAssetMasterPhotoField.jsx";
...
export default function CrewAssetsMobile({ token, onBack, onFlowChange, management = false, outletId = null }) {
  ...
  if (inspection) return <InspectionFlow token={token} outletId={management ? outletId : null} data={data} initial={inspection} onBack={() => setInspection(null)} onDraftSaved={() => load()} onCancelled={async () => { await load(); setNotice(t("assets.inspectionDraftCancelled")); setInspection(null); }} onSaved={async (result) => { const refreshed = await load(); const updates = new Map((result?.asset_updates || []).map((item) => [item.id, item])); if (updates.size) setData((previous) => previous ? { ...previous, assets: previous.assets.map((item) => ({ ...item, ...(updates.get(item.id) || {}) })) } : previous); if (asset) { const latest = refreshed?.assets?.find((item) => item.id === asset.id) || asset; setAsset({ ...latest, ...(updates.get(asset.id) || {}) }); } setNotice(t("assets.inspectionComplete")); setInspection(null); }} />;
  if (asset) return <><AssetDetail asset={asset} data={data} onBack={() => setAsset(null)} onAdjust={() => setAdjusting(true)} onInspect={() => setInspection({ scope: "specific", assets: [asset] })} onActivity={() => setActivity({ assetId: asset.id, title: asset.name })} onEdit={() => setAdding("edit")} />
    {adjusting ? <AdjustSheet token={token} outletId={management ? outletId : null} asset={asset} onClose={() => setAdjusting(false)} onSaved={async () => { const refreshed = await load(); setAsset(refreshed?.assets?.find((item) => item.id === asset.id) || null); setAdjusting(false); }} /> : null}
    {adding === "edit" ? <EditAssetDetailsSheet token={token} outletId={management ? outletId : null} asset={asset} onClose={() => setAdding(false)} onSaved={async () => { const refreshed = await load(asset.id); setAsset(refreshed?.assets?.find((item) => item.id === asset.id) || asset); setAdding(false); }} /> : null}
  ...
  return <section className="crew-assets-page"><CrewMobileDetailHeader title={t("assets.title")} onBack={onBack} /><State loading={loading} error={error} retry={load}><>
    ...
    <div className="crew-assets-list">{visible.map((item) => <button type="button" key={item.id} onClick={() => setAsset(item)}><span className="crew-assets-thumb">{item.thumbnail_url || item.image_url || item.original_image_url ? <CrewAssetMasterImage asset={item} /> : <PackageSearch size={22} />}</span><span><strong>{item.name}</strong><small>{item.category_name}{item.location ? ` · ${item.location}` : ""}</small><em>{pluralize(item.current_quantity, item.unit, t)}</em></span><CrewStatusBadge tone={tone(item.condition)}>{t(`assets.${conditionKeys[item.condition]}`)}</CrewStatusBadge><ChevronRight size={18} /></button>)}{!visible.length ? <CrewEmptyState title={t("assets.noAssets")} body={t("assets.noAssetsBody")} /> : null}</div>
    {adding ? <AddAssetSheet token={token} outletId={management ? outletId : null} categories={data?.categories || []} onClose={() => setAdding(false)} onSaved={async (assetId) => { const refreshed = await load(assetId); setAdding(false); setAsset(refreshed?.assets?.find((item) => item.id === assetId) || null); }} /> : null}{activity ? <ActivitySheet data={data} title={activity.title} onClose={() => setActivity(false)} /> : null}
  </></State></section>;
}
...
function AssetDetail({ asset, data, onBack, onAdjust, onInspect, onActivity, onEdit }) {
  ...
  return <section className="crew-assets-page"><CrewMobileDetailHeader title={t("assets.assetDetail")} onBack={onBack} action={data?.can_manage_asset_details ? <button className="crew-mobile-header-text-action crew-assets-edit-action" type="button" onClick={onEdit}><Pencil size={17} />{t("assets.edit")}</button> : null} /><article className="crew-asset-detail">
    <div className="crew-asset-detail-lead"><button className="crew-asset-detail-image" type="button" disabled={!display} onClick={() => setViewer(true)} aria-label={display ? t("assets.viewPhoto") : undefined}>{display ? <CrewAssetMasterImage asset={{ ...asset, thumbnail_url: "" }} /> : <PackageSearch size={30} />}</button><header><span><small>{asset.asset_code || asset.category_name}</small><h1>{asset.name}</h1>{asset.location ? <em><MapPin size={14} />{asset.location}</em> : null}</span><CrewStatusBadge tone={tone(asset.condition)}>{t(`assets.${conditionKeys[asset.condition]}`)}</CrewStatusBadge></header></div>
...
function AdjustSheet({ token, outletId, asset, onClose, onSaved }) {
  const { t } = useTranslation();
  ...
  async function save() { setSaving(true); setError(""); try { await crewService.adjustAsset(token, { requestId: requestId(), assetId: asset.id, adjustmentType: type, quantity, reason, note }, outletId); await onSaved(); } catch (cause) { setError(cause.message || t("assets.unableAdjust")); } finally { setSaving(false); } }
...
function AddAssetSheet({ token, outletId, categories, onClose, onSaved }) {
  const { t } = useTranslation();
  ...
  function handlePhotoChange(nextPhoto) {
    setPhoto((current) => {
      const resolved = typeof nextPhoto === "function" ? nextPhoto(current) : nextPhoto;
      if (!resolved) createRequestId.current = requestId();
      return resolved;
    });
  }
  async function save() {
    ...
    try {
      ...
      const result = photo
        ? await crewService.createAssetWithPhoto(token, { requestId: createRequestId.current, asset: assetPayload, preparedPhoto: await crewService.prepareAssetMasterPhoto(photo.file, photo.crop) }, outletId)
        : await crewService.createAsset(token, { requestId: createRequestId.current, asset: assetPayload }, outletId);
...
function EditAssetDetailsSheet({ token, outletId, asset, onClose, onSaved }) {
  ...
  async function save() { setSaving(true); setError(""); try { await crewService.updateAssetDetails(token, { requestId: requestId(), assetId: asset.id, details: { name, description, location, remove_photo: removePhoto }, preparedPhoto: photo ? await crewService.prepareAssetMasterPhoto(photo.file, photo.crop) : null }, outletId); await onSaved(); } catch (cause) { setError(localizeAssetCreateError("update", cause, t)); } finally { setSaving(false); } }
  return <CrewBottomSheet title={t("assets.editDetails")} description={t("assets.editDetailsHelp")} onClose={onClose} closeDisabled={saving} footer={<><button className="crew-mobile-ghost" type="button" disabled={saving} onClick={onClose}>{t("common.cancel")}</button><button className="crew-mobile-primary" type="button" disabled={saving || preparingPhoto || !name.trim()} onClick={() => void save()}>{saving ? t("common.saving") : t("assets.saveChanges")}</button></>}><div className="crew-assets-form"><label>{t("assets.assetName")}<input value={name} autoComplete="off" enterKeyHint="next" onChange={(event) => setName(event.target.value)} /></label><label>{t("assets.location")} <small>{t("common.optional")}</small><input value={location} enterKeyHint="next" onChange={(event) => setLocation(event.target.value)} /></label><CrewAssetMasterPhotoField asset={asset} photo={photo} onPhotoChange={setPhoto} removeExisting={removePhoto} onRemoveExistingChange={setRemovePhoto} onPreparingChange={setPreparingPhoto} onError={setError} />{preparingPhoto ? <small role="status">{t("assets.preparingPreview")}</small> : null}<label>{t("assets.description")} <small>{t("common.optional")}</small><textarea value={description} enterKeyHint="done" onChange={(event) => setDescription(event.target.value)} /></label>{error ? <p className="crew-v2-error" role="alert">{error}</p> : null}</div></CrewBottomSheet>;
}
...
function InspectionFlow({ token, outletId, data, initial, onBack, onDraftSaved, onSaved, onCancelled }) {
  const { t } = useTranslation();
  ...
  async function save(status) { setSaving(true); setError(""); try {
    ...
    if (status === "completed") for (const [assetId, pending] of Object.entries(pendingPhotos)) { const evidence = await crewService.uploadAssetInspectionEvidence(token, assetId, pending.file, outletId); evidenceByAsset[assetId] = { image_url: evidence.image_url }; }
    const result = await crewService.submitAssetInspection(token, { requestId: requestId(), ...payload(status, evidenceByAsset) }, outletId);
  ...
  async function cancelDraft() { if (!draft?.id) return; setCancelling(true); setError(""); try { await (outletId ? crewService.archiveAssetInspectionDraft(token, draft.id, crypto.randomUUID(), outletId) : crewService.archiveAssetInspectionDraft(token, draft.id)); releasePendingPhotos(); setCancelOpen(false); await onCancelled(); } catch { setError(t("assets.unableCancelInspection")); setCancelOpen(false); } finally { setCancelling(false); } }
