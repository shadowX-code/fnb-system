...
export default function CrewAssetsMobile({ token, onBack }) {
  ...
  if (asset) return <><AssetDetail asset={asset} data={data} onBack={() => setAsset(null)} onAdjust={() => setAdjusting(true)} onInspect={() => setInspection({ scope: "specific", assets: [asset] })} />
    {adjusting && <AdjustSheet token={token} asset={asset} onClose={() => setAdjusting(false)} onSaved={async () => { setAdjusting(false); setAsset(null); await load(); }} />}
  </>;
...
function InspectionFlow({ token, data, initial, onBack, onSaved }) {
  ...
  async function save(status) { setSaving(true); setError(""); try { await crewService.submitAssetInspection(token, { requestId: requestId(), ...payload(status) }); await onSaved(); } catch (cause) { setError(cause.message || "Unable to save inspection."); } finally { setSaving(false); } }
