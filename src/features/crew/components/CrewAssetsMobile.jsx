...
function EditAssetDetailsSheet({ token, asset, onClose, onSaved }) {
  ...
  async function choosePhoto(file) { if (!file) return; setPreparingPhoto(true); setError(""); try { const prepared = await crewService.prepareAssetMasterPhoto(file); setPhoto({ ...prepared, crop: { zoom: 1, x: 0, y: 0 }, sourceUrl: URL.createObjectURL(file) }); setRemovePhoto(false); } catch (cause) { setError(cause.message || "That photo could not be prepared. Try another image."); } finally { setPreparingPhoto(false); } }
  async function save() { setSaving(true); setError(""); try { await crewService.updateAssetDetails(token, { requestId: requestId(), assetId: asset.id, details: { name, description, location, remove_photo: removePhoto }, preparedPhoto: photo ? await crewService.prepareAssetMasterPhoto(photo.file, photo.crop) : null }); await onSaved(); } catch (cause) { setError(assetCreateErrorMessage("update", cause)); } finally { setSaving(false); } }
  const hasExistingPhoto = Boolean(asset.image_url || asset.original_image_url);
  return <CrewBottomSheet title="Edit Asset Details" description="Update the Asset presentation details." onClose={onClose} closeDisabled={saving} footer={<><button className="crew-mobile-ghost" type="button" disabled={saving} onClick={onClose}>Cancel</button><button className="crew-mobile-primary" type="button" disabled={saving || preparingPhoto || !name.trim()} onClick={() => void save()}>{saving ? "Saving…" : "Save changes"}</button></>}><div className="crew-assets-form"><label>Asset name<input value={name} autoComplete="off" enterKeyHint="next" onChange={(event) => setName(event.target.value)} /></label><label>Location <small>Optional</small><input value={location} enterKeyHint="next" onChange={(event) => setLocation(event.target.value)} /></label><fieldset className="crew-assets-photo-field"><legend>Photo <small>Optional</small></legend>{photo ? <div className="crew-assets-photo-preview"><AssetPhotoCropper src={photo.sourceUrl} crop={photo.crop} onChange={(crop) => setPhoto((current) => ({ ...current, crop }))} /><div><label><Camera size={17} />Replace<input type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" capture="environment" onChange={(event) => void choosePhoto(event.target.files?.[0])} /></label><button type="button" onClick={() => { setPhoto(null); setRemovePhoto(true); }}><Trash2 size={17} />Remove</button></div></div> : removePhoto || !hasExistingPhoto ? <div className="crew-assets-photo-actions"><label><Camera size={18} />Take Photo<input type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" capture="environment" onChange={(event) => void choosePhoto(event.target.files?.[0])} /></label><label><ImagePlus size={18} />Choose from Library<input type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" onChange={(event) => void choosePhoto(event.target.files?.[0])} /></label></div> : <div className="crew-assets-photo-preview"><AssetMasterImage asset={{ ...asset, thumbnail_url: "" }} /><div><label><Camera size={17} />Replace<input type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" capture="environment" onChange={(event) => void choosePhoto(event.target.files?.[0])} /></label><button type="button" onClick={() => setRemovePhoto(true)}><Trash2 size={17} />Remove</button></div></div>}{preparingPhoto ? <small role="status">Preparing 4:3 preview…</small> : null}</fieldset><label>Description <small>Optional</small><textarea value={description} enterKeyHint="done" onChange={(event) => setDescription(event.target.value)} /></label>{error ? <p className="crew-v2-error" role="alert">{error}</p> : null}</div></CrewBottomSheet>;
}
