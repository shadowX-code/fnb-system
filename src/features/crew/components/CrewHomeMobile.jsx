...
import CrewPeerReviewHome from "./CrewPeerReviewHome.jsx";
...
export default function CrewHomeMobile({ session, attendance, context, roster, operations, clock, navigate, onOpenTask, theme, onToggleTheme, notificationUnreadCount = 0, management = false, inventoryOutletId, inventoryGrants, onOpenInventoryList, onOpenInventoryTarget }) {
  ...
  return <section className="crew-v2-home">
      ...
      {(!management || context?.clock_eligible || openShift) && <section className={`crew-home-attendance is-${attendanceMode}`} aria-label={t("locationEvidence.attendanceStatus")}>
        ...
        <button type="button" className="crew-home-attendance-footer" onClick={() => navigate("schedule")}><i className="crew-ui-icon-container crew-ui-icon-container--compact"><CalendarCheck size={18} /></i><span className="crew-home-attendance-shift"><small>{t("home.todayShift")}</small><strong>{shiftLabel}</strong></span><em><span>{t("home.viewSchedule")}</span><ChevronRight size={16} /></em></button>
      </section>}
      ...
      <CrewPeerReviewHome token={session.token} />
