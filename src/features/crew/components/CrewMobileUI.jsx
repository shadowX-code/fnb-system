...
export function CrewPageSection({ className = "", children }) {
  return <div className={`crew-ui-page-section ${className}`.trim()}>{children}</div>;
}
...
export function CrewSectionHeader({ title, meta, trailing = null, action, actionLabel, onAction, density = "default" }) {
  return <div className={`crew-ui-section-head is-${density}`}><h2 className="crew-type-section-title">{title}{meta !== undefined && <span>{meta}</span>}</h2>{trailing}{action && <button type="button" aria-label={actionLabel} onClick={onAction}>{action}</button>}</div>;
}
