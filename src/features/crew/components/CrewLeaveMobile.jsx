...
export default function CrewLeaveMobile({ token, onBack, onChanged }) {
  ...
  if (step) return <section className="crew-v3-leave">
    ...
    {step === 4 && <section className="crew-v3-leave-form"><CrewSectionHeader title="Review Request" /><div className="crew-v3-review-list"><div><span>Leave Type</span><strong>{typeLabel[form.leave_type]}</strong></div><div><span>Dates</span><strong>{dateRange(form)}</strong></div><div><span>Duration</span><strong>{days} day{days === 1 ? "" : "s"}{form.duration_type === "half_day" ? ` · ${form.half_day_period.toUpperCase()}` : ""}</strong></div><div><span>Available Before</span><strong>{selectedBalance?.balance_enforced ? `${selectedBalance.available} days` : "No limit"}</strong></div><div><span>After Request</span><strong>{afterRequest == null ? "No limit" : `${afterRequest} days`}</strong></div><div><span>Reason</span><strong>{form.reason}</strong></div></div></section>}
    {error && <div className="crew-v2-error" role="alert">{error}</div>}
    <footer className="crew-v3-leave-footer"><button type="button" className="crew-v2-secondary" onClick={() => step === 1 ? setStep(0) : setStep(step - 1)}>Back</button>{step < 4 ? <button type="button" className="crew-v2-primary" disabled={(step === 2 && (!days || insufficient)) || (step === 3 && form.reason.trim().length < 2)} onClick={() => setStep(step + 1)}>Continue</button> : <button type="button" className="crew-v2-primary" disabled={saving || insufficient} onClick={submit}>{saving ? "Submitting…" : "Submit Request"}</button>}</footer>
  </section>;
  ...
  return <section className="crew-v3-leave">
    ...
    {!loading && <section className="crew-v3-balance-grid" aria-label="Leave balances">{data.balances?.filter((item) => item.leave_type !== "other").map((item) => <article key={item.entitlement_id}><span>{typeLabel[item.leave_type]}</span><strong>{item.balance_enforced ? item.available : "∞"}</strong><small>{item.balance_enforced ? `${item.pending} pending · ${item.used} used` : "No balance limit"}</small></article>)}</section>}
...
function BalancePreview({ balance, requested, after, insufficient }) { return <div className={`crew-v3-leave-balance-preview ${insufficient ? "is-insufficient" : ""}`}><CalendarDays size={18} /><div><span><small>Available</small><strong>{balance?.balance_enforced ? balance.available : "No limit"}</strong></span><span><small>Requested</small><strong>{requested}</strong></span><span><small>After</small><strong>{after == null ? "No limit" : after}</strong></span></div>{insufficient && <p>Insufficient leave balance. Choose fewer days or contact your manager.</p>}</div>; }
