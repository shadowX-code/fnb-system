...
export default function CrewLeaveMobile({ token, onBack, onChanged }) {
  ...
  return <section className="crew-leave-page">
    ...
    {error && <div className="crew-ui-error" role="alert">{error}</div>}
    {loading ? <div className="crew-ui-loading">{t("leave.loading")}</div> : <div className="crew-ui-list-stack">{rows.length ? rows.map((item) => <article className="crew-ui-list-card crew-leave-request" key={item.id}><header><span className="crew-ui-list-card-copy"><strong>{typeLabel[item.leave_type]}</strong><small>{dateRange(item)} · {item.requested_days} {dayUnit(item.requested_days, t)}</small></span><CrewStatusBadge tone={statusTone[item.status]}>{translateStatus(item.status, t)}</CrewStatusBadge></header><p>{t("leave.submitted", { date: formatCrewDate(item.submitted_at) })}</p>{item.status === "rejected" && <div className="crew-leave-rejection"><strong>{t("leave.reason")}</strong>{item.rejection_reason}</div>}{item.status === "approved" && <p className="crew-leave-help">{t("leave.approvedChangeHelp")}</p>}{item.can_cancel && <button type="button" className="crew-leave-cancel" disabled={saving} onClick={() => cancel(item.id)}><X size={15} /> {t("leave.cancelRequest")}</button>}</article>) : <CrewEmptyState title={t("leave.noLeave", { tab: t(`leave.${tab}`, { defaultValue: tab }) })} body={tab === "upcoming" ? t("leave.noUpcoming") : tab === "pending" ? t("leave.noPending") : t("leave.noHistory")} />}</div>}
  </section>;
}
