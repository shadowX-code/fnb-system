...
export default function CrewLeaveMobile({ token, onBack, onChanged }) {
  ...
  if (step) return <section className="crew-leave-page">
    ...
    {step === 1 && <section className="crew-leave-form"><CrewSectionHeader title={t("leave.leaveType")} /><div className="crew-ui-choice-list crew-ui-choice-list--mint">{Object.entries(typeLabel).map(([value, label]) => <button type="button" className={form.leave_type === value ? "is-selected" : ""} key={value} onClick={() => update("leave_type", value)}><span><strong>{label}</strong>{value === "medical" && <small>{t("leave.medicalDocumentFuture")}</small>}</span>{form.leave_type === value ? <span className="crew-ui-icon-container crew-ui-icon-container--small is-selected"><Check size={15} /></span> : <ChevronRight size={18} />}</button>)}</div></section>}
    {step === 2 && <section className="crew-leave-form"><CrewSectionHeader title={t("leave.dates")} /><div className="crew-ui-field-grid crew-leave-date-grid"><CrewDatePicker label={t("leave.startDate")} value={form.start_date} min={today()} onChange={(value) => update("start_date", value)} /><CrewDatePicker label={t("leave.endDate")} value={form.end_date} min={form.start_date} disabled={form.duration_type === "half_day"} onChange={(value) => update("end_date", value)} /></div><div className="crew-ui-segmented"><button type="button" className={form.duration_type === "full_day" ? "is-active" : ""} onClick={() => update("duration_type", "full_day")}>{t("leave.fullDay")}</button><button type="button" className={form.duration_type === "half_day" ? "is-active" : ""} onClick={() => update("duration_type", "half_day")}>{t("leave.halfDay")}</button></div>{form.duration_type === "half_day" && <div className="crew-ui-segmented"><button type="button" className={form.half_day_period === "am" ? "is-active" : ""} onClick={() => update("half_day_period", "am")}>AM</button><button type="button" className={form.half_day_period === "pm" ? "is-active" : ""} onClick={() => update("half_day_period", "pm")}>PM</button></div>}<BalancePreview balance={selectedBalance} requested={days} after={afterRequest} insufficient={insufficient} /></section>}
    {step === 3 && <section className="crew-leave-form"><CrewSectionHeader title={t("leave.reason")} /><label className="crew-ui-form-field">{t("leave.reasonNote")}<textarea rows="6" maxLength="1000" value={form.reason} onChange={(event) => update("reason", event.target.value)} placeholder={t("leave.reasonPlaceholder")} /></label>{form.leave_type === "medical" && <div className="crew-ui-note crew-ui-note--mint"><span className="crew-ui-icon-container crew-ui-icon-container--small"><FileText size={16} /></span><span><strong>{t("leave.documentNotUploaded")}</strong><small>{t("leave.documentOptionalV1")}</small></span></div>}</section>}
  ...
  return <section className="crew-leave-page">
    <CrewMobileDetailHeader title={t("leave.title")} onBack={onBack} action={data.can_apply === false ? null : <button type="button" className="crew-mobile-primary crew-leave-header-action" onClick={() => setStep(1)}><Plus size={16} aria-hidden="true" /> <span>{t("leave.apply")}</span></button>} />
    {!loading && data.can_apply === false && <div className="crew-ui-note"><Info size={16} /><span>{t("leave.managementReadOnly")}</span></div>}
    {!loading && <section className="crew-ui-kpi-grid crew-leave-balance-grid" aria-label={t("leave.balances")}>{data.balances?.filter((item) => item.leave_type !== "other").map((item) => <article className="crew-ui-kpi crew-leave-balance-card" key={item.entitlement_id}><span>{typeLabel[item.leave_type]}</span><strong>{item.balance_enforced ? `${item.available} ${dayUnit(item.available, t)}` : t("leave.unlimited")}</strong><small className="crew-leave-balance-unit">{item.balance_enforced ? t("leave.available") : t("leave.noBalanceLimit")}</small>{item.balance_enforced && <small className="crew-leave-balance-meta">{t("leave.pendingUsed", { pending: item.pending, used: item.used })}</small>}</article>)}</section>}
...
function BalancePreview({ balance, requested, after, insufficient }) { const { t } = useTranslation(); return <div className={`crew-ui-note crew-ui-note--mint crew-leave-balance-preview ${insufficient ? "is-insufficient" : ""}`}><span className="crew-ui-icon-container crew-ui-icon-container--small"><CalendarDays size={16} /></span><div><span><small>{t("leave.available")}</small><strong>{balance?.balance_enforced ? balance.available : t("common.noLimit")}</strong></span><span><small>{t("leave.requested")}</small><strong>{requested}</strong></span><span><small>{t("leave.after")}</small><strong>{after == null ? t("common.noLimit") : after}</strong></span></div>{insufficient && <p>{t("leave.insufficient")}</p>}</div>; }
