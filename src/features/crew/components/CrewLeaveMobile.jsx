...
export default function CrewLeaveMobile({ token, onBack, onChanged }) {
  ...
  if (step) return <section className="crew-v3-leave">
    ...
    {step === 3 && <section className="crew-v3-leave-form"><CrewSectionHeader title={t("leave.reason")} /><label>{t("leave.reasonNote")}<textarea rows="6" maxLength="1000" value={form.reason} onChange={(event) => update("reason", event.target.value)} placeholder={t("leave.reasonPlaceholder")} /></label>{form.leave_type === "medical" && <div className="crew-v3-document-note"><FileText size={18} /><span><strong>{t("leave.documentNotUploaded")}</strong><small>{t("leave.documentOptionalV1")}</small></span></div>}</section>}
    {step === 4 && <section className="crew-v3-leave-form"><CrewSectionHeader title={t("leave.review")} /><div className="crew-v3-review-list"><div><span>{t("leave.leaveType")}</span><strong>{typeLabel[form.leave_type]}</strong></div><div><span>{t("leave.dates")}</span><strong>{dateRange(form)}</strong></div><div><span>{t("leave.duration")}</span><strong>{days} {t("common.days")}{form.duration_type === "half_day" ? ` · ${form.half_day_period.toUpperCase()}` : ""}</strong></div><div><span>{t("leave.availableBefore")}</span><strong>{selectedBalance?.balance_enforced ? `${selectedBalance.available} ${t("common.days")}` : t("common.noLimit")}</strong></div><div><span>{t("leave.afterRequest")}</span><strong>{afterRequest == null ? t("common.noLimit") : `${afterRequest} ${t("common.days")}`}</strong></div><div><span>{t("leave.reason")}</span><strong>{form.reason}</strong></div></div></section>}
    {error && <div className="crew-v2-error" role="alert">{error}</div>}
    <footer className="crew-v3-leave-footer"><button type="button" className="crew-v2-secondary" onClick={() => step === 1 ? setStep(0) : setStep(step - 1)}>{t("common.back")}</button>{step < 4 ? <button type="button" className="crew-v2-primary" disabled={(step === 2 && (!days || insufficient)) || (step === 3 && form.reason.trim().length < 2)} onClick={() => setStep(step + 1)}>{t("common.continue")}</button> : <button type="button" className="crew-v2-primary" disabled={saving || insufficient} onClick={submit}>{saving ? t("leave.submitting") : t("leave.submitRequest")}</button>}</footer>
  </section>;
  ...
  return <section className="crew-v3-leave">
    ...
    {error && <div className="crew-v2-error" role="alert">{error}</div>}
    {loading ? <div className="crew-v3-mobile-loading">{t("leave.loading")}</div> : <div className="crew-v3-row-group">{rows.length ? rows.map((item) => <article className="crew-v3-leave-card" key={item.id}><div><span><strong>{typeLabel[item.leave_type]}</strong><small>{dateRange(item)} · {item.requested_days} {t("common.days")}</small></span><CrewStatusBadge tone={statusTone[item.status]}>{translateStatus(item.status, t)}</CrewStatusBadge></div><p>{t("leave.submitted", { date: formatCrewDate(item.submitted_at) })}</p>{item.status === "rejected" && <div className="crew-v3-rejection"><strong>{t("leave.reason")}</strong>{item.rejection_reason}</div>}{item.status === "approved" && <p className="is-help">{t("leave.approvedChangeHelp")}</p>}{item.can_cancel && <button type="button" disabled={saving} onClick={() => cancel(item.id)}><X size={15} /> {t("leave.cancelRequest")}</button>}</article>) : <CrewEmptyState title={t("leave.noLeave", { tab: t(`leave.${tab}`, { defaultValue: tab }) })} body={tab === "upcoming" ? t("leave.noUpcoming") : tab === "pending" ? t("leave.noPending") : t("leave.noHistory")} />}</div>}
  </section>;
}

function BalancePreview({ balance, requested, after, insufficient }) { const { t } = useTranslation(); return <div className={`crew-v3-leave-balance-preview ${insufficient ? "is-insufficient" : ""}`}><CalendarDays size={18} /><div><span><small>{t("leave.available")}</small><strong>{balance?.balance_enforced ? balance.available : t("common.noLimit")}</strong></span><span><small>{t("leave.requested")}</small><strong>{requested}</strong></span><span><small>{t("leave.after")}</small><strong>{after == null ? t("common.noLimit") : after}</strong></span></div>{insufficient && <p>{t("leave.insufficient")}</p>}</div>; }
