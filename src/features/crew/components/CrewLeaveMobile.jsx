...
export default function CrewLeaveMobile({ token, onBack, onChanged }) {
  ...
  if (step) return <section className="crew-v3-leave">
    ...
    {step === 4 && <section className="crew-v3-leave-form"><CrewSectionHeader title={t("leave.review")} /><div className="crew-v3-review-list"><div><span>{t("leave.leaveType")}</span><strong>{typeLabel[form.leave_type]}</strong></div><div><span>{t("leave.dates")}</span><strong>{dateRange(form)}</strong></div><div><span>{t("leave.duration")}</span><strong>{days} {dayUnit(days, t)}{form.duration_type === "half_day" ? ` · ${form.half_day_period.toUpperCase()}` : ""}</strong></div><div><span>{t("leave.availableBefore")}</span><strong>{selectedBalance?.balance_enforced ? `${selectedBalance.available} ${dayUnit(selectedBalance.available, t)}` : t("common.noLimit")}</strong></div><div><span>{t("leave.afterRequest")}</span><strong>{afterRequest == null ? t("common.noLimit") : `${afterRequest} ${dayUnit(afterRequest, t)}`}</strong></div><div><span>{t("leave.reason")}</span><strong>{form.reason}</strong></div></div></section>}
