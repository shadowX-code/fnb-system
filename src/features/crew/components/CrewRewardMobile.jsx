...
function RewardHero({ data, onOpenModal }) {
  ...
  return <article className="crew-reward-hero">
    ...
    <div className="crew-reward-hero-metrics">
      ...
      <div className="is-contribution"><span><small>{t("reward.contribution")} <InfoButton label={t("common.aboutNamed", { title: t("reward.contributionShare").toLowerCase() })} onClick={() => onOpenModal("contribution")} /></small><strong>{rate(contribution, 2)}</strong><p>{t("reward.eligibleHours", { eligible: Number(data.eligible_hours || 0).toFixed(1), total: Number(data.total_eligible_hours || 0).toFixed(1) })}</p><i aria-hidden="true"><b style={{ width: `${contribution * 100}%` }} /></i></span></div>
...
function PerformanceOverview({ data, onOpenSheet, onViewPerformance }) {
  const { t } = useTranslation();
  ...
  return <section className="crew-reward-section crew-reward-performance">
    ...
    <div className="crew-reward-performance-main">
      <div className="crew-reward-score-summary">
        ...
        <div><small>{t("reward.performanceScore", { score: Math.round(score) })}</small><h3>{translateRewardLevel(data.performance_level, t) || translateStatus("ready_for_review", t)}</h3><span className="crew-reward-rate"><small>{t("reward.currentRate")} <InfoButton label={t("common.aboutNamed", { title: t("reward.currentRate").toLowerCase() })} onClick={() => onOpenSheet("rates")} /></small><strong>{rate(data.earn_rate)}</strong></span></div>
      </div>
    </div>
...
export default function CrewRewardMobile({ data, loading, onRetry, onViewPerformance }) {
  ...
  return <section className="crew-reward-final">
    ...
    {unlocked ? <>
      ...
      <PerformanceOverview data={data} onOpenSheet={setSheet} onViewPerformance={onViewPerformance} />
    ...
    {sheet === "projection" && <Modal title={t("reward.aboutProjections")} onClose={() => setSheet(null)}><p>{t("reward.projectionDetail")}</p><div className="crew-reward-formula"><small>{t("reward.rewardPool")} × {t("reward.contributionShare")}</small><strong>{money(data.reward_pool ?? data.configured_pool)} × {rate(data.contribution_share, 2)} = {money(data.maximum_share)}</strong></div><div className="crew-reward-formula is-result"><small>{t("reward.maximumShare")} × {t("reward.performanceEarnRate")}</small><strong>{money(data.maximum_share)} × {rate(data.earn_rate)} = {money(data.reward_amount ?? data.estimated_reward)}</strong></div></Modal>}
