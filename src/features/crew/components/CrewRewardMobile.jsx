...
function RewardHero({ data, onOpenModal }) {
  ...
  return <article className="crew-reward-hero">
    ...
    <div className="crew-reward-hero-metrics">
      <div className="is-maximum"><small>{t("reward.maximum")} <InfoButton label={t("common.aboutNamed", { title: t("reward.maximumShare").toLowerCase() })} onClick={() => onOpenModal("maximum")} /></small><strong>{money(data.maximum_share)}</strong></div>
      <div className="is-contribution"><span><small>{t("reward.contribution")} <InfoButton label={t("common.aboutNamed", { title: t("reward.contributionShare").toLowerCase() })} onClick={() => onOpenModal("contribution")} /></small><strong>{rate(contribution, 2)}</strong></span></div>
      <div className="is-pool"><small>{t("reward.rewardPool")} <InfoButton label={t("common.aboutNamed", { title: t("reward.rewardPool").toLowerCase() })} onClick={() => onOpenModal("pool")} /></small><strong>{money(data.reward_pool ?? data.configured_pool)}</strong></div>
    </div>
    <button type="button" className="crew-reward-hero-insight" onClick={() => onOpenModal("help")}><Lightbulb size={15} /><span>{t("reward.motivation")}</span><ChevronRight size={16} /></button>
  </article>;
}
  ...
  return <section className="crew-reward-section crew-reward-performance">
    ...
    <div className="crew-reward-performance-main">
      <div className="crew-reward-score-summary">
        <div className="crew-reward-score-ring"><span><strong>{Math.round(score)}</strong><small>/ 100</small></span></div>
        <div><small>{t("reward.performanceScore", { score: Math.round(score) })}</small><h3>{translateRewardLevel(data.performance_level, t) || translateStatus("ready_for_review", t)}</h3><span className="crew-reward-rate"><small>{t("reward.currentRate")}</small><strong>{rate(data.earn_rate)}</strong></span></div>
      </div>
    </div>
  </section>;
}
...
function RewardProjection({ data, onOpenSheet }) {
  ...
  return <section className="crew-reward-section crew-reward-projection">
    <header><span><h2><TrendingUp size={19} /> {t("reward.projection")}</h2><p>{t("reward.projectionCaption")}</p></span><button type="button" onClick={() => onOpenSheet("help")}><Info size={15} /> {t("reward.howWorks")}</button></header>
    <div className="crew-reward-projection-track">
      {projections.map((item, index) => <div key={item.key || item.label} className={`is-${item.key || index}`}><i aria-hidden="true" /><span><small>{translateProjectionLabel(item, t)}</small><b>{t("reward.score", { score: item.key === "max" ? "95+" : Math.round(Number(item.score || 0)) })}</b></span><strong>{money(item.amount)}</strong><em>{t("reward.rateEarned", { rate: rate(item.earn_rate) })}</em></div>)}
    </div>
...
export default function CrewRewardMobile({ data, loading, onRetry, onViewPerformance }) {
  ...
  return <section className="crew-reward-final">
    ...
    {sheet === "help" && <Modal title={t("reward.about")} onClose={() => setSheet(null)}><div className="crew-reward-modal-section"><strong>{t("reward.maximumShare")}</strong><p>{t("reward.maximumShareHelp")}</p><div className="crew-reward-formula"><small>{t("reward.rewardPool")} × {t("reward.contributionShare")}</small><strong>{money(data.reward_pool ?? data.configured_pool)} × {rate(data.contribution_share, 2)} = {money(data.maximum_share)}</strong></div></div><div className="crew-reward-modal-section"><strong>{t("reward.performanceEarnRate")}</strong><p>{t("reward.performanceEarnRateHelp")}</p><div className="crew-reward-formula is-result"><small>{t("reward.maximumShare")} × {t("reward.performanceEarnRate")}</small><strong>{money(data.maximum_share)} × {rate(data.earn_rate)} = {money(data.reward_amount ?? data.estimated_reward)}</strong></div></div><TierTable tiers={tiers} /><div className="crew-reward-modal-note"><Lightbulb size={18} /><span>{t("reward.helpCallout")}</span></div></Modal>}
