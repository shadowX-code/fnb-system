...
export default function CrewAccessManagerModal({ employee, mode = "enable", onClose, onSaved }) {
  ...
  if (result) {
    return <Modal title={reset ? "Passcode Reset" : "Crew Access Activated"} description="Copy or share the temporary passcode now. It is shown once only and cannot be retrieved later." size="sm" onClose={onClose} footer={<button className="btn-primary" type="button" onClick={onClose}>Done</button>}>
      <div className="space-y-4">
        ...
        <p className="text-xs font-medium text-text-secondary">Share it securely before closing this window.</p>
      </div>
    </Modal>;
  }
  ...
  return <Modal title={reset ? "Reset Crew Passcode" : "Activate Crew Access"} description={`${employee.full_name} · ${employee.position || "No position"} · ${employee.workplace || "No workplace"}`} size="sm" onClose={saving ? undefined : onClose} footer={<><button className="btn-secondary" type="button" disabled={saving} onClick={onClose}>Cancel</button><button className="btn-primary" form="crew-access-form" type="submit" disabled={saving}>{saving ? "Saving..." : reset ? "Reset Passcode" : "Activate"}</button></>}>
    <form id="crew-access-form" className="space-y-4" onSubmit={submit}>
      ...
      <div><div className="text-sm font-bold text-text-primary">Passcode</div><label className="mt-2 flex items-center gap-2 text-sm"><input type="radio" checked={!manual} onChange={() => setManual(false)} /> Generate random</label><label className="mt-2 flex items-center gap-2 text-sm"><input type="radio" checked={manual} onChange={() => setManual(true)} /> Enter manually</label>{manual ? <input className="mt-3 w-full rounded-xl border border-border bg-surface px-3 py-2 text-center text-lg font-bold tracking-[0.3em] outline-none focus:border-primary focus:ring-2 focus:ring-primary/15" inputMode="numeric" maxLength="4" value={passcode} onChange={(event) => setPasscode(event.target.value.replace(/\D/g, ""))} placeholder="••••" autoComplete="off" /> : null}<p className="mt-2 text-xs leading-5 text-text-secondary">Generated and manually entered passcodes are shown only in the completion window.</p></div>
