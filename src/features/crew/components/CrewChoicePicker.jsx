...
export default function CrewChoicePicker({ label, value, options, onChange, placeholder, description, disabled = false }) {
  ...
  return <div className="crew-choice-picker">
    ...
    <button type="button" className="crew-choice-picker-trigger" aria-labelledby={label ? labelId : undefined} aria-label={label || resolvedPlaceholder} aria-haspopup="dialog" aria-expanded={open} disabled={disabled} onClick={() => setOpen(true)}>
      <strong>{selected?.label || resolvedPlaceholder}</strong><ChevronDown size={18} aria-hidden="true" />
    </button>
    {open ? <CrewBottomSheet title={label || resolvedPlaceholder} description={description} onClose={() => setOpen(false)}>
      <div className="crew-ui-choice-list crew-ui-choice-list--mint" role="listbox" aria-label={label || resolvedPlaceholder}>
