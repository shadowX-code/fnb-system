...
export default function CrewLearningAdminResetPage({ auth, ui, store }) {
  ...
  return <div className="crew-onboarding-admin-page">
    ...
    <CrewAdminToolbar outlet={<CrewAdminOutletField />} secondary={canManage ? <button className="btn-secondary" onClick={() => setCloneOpen(true)}><Copy size={15} /> Clone From Outlet</button> : null} primary={canManage ? <button className="btn-primary" disabled={saving} onClick={openEditor}>{draft ? "Edit Draft" : published ? "Create Draft" : "Create Onboarding"}</button> : null} />
