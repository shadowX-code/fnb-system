...
export default function CrewLearningAdminResetPage({ auth, ui, store }) {
  ...
  return <div className="crew-onboarding-admin-page">
    ...
    <AdminFilterToolbar ariaLabel="Onboarding controls" outlet={<CrewAdminOutletField />} secondaryActions={canManage ? <button className="btn-secondary" onClick={() => setCloneOpen(true)}><Copy size={15} /> Clone From Outlet</button> : null} primaryActions={canManage ? <button className="btn-primary" disabled={saving} onClick={openEditor}>{draft ? "Edit Draft" : published ? "Create Draft" : "Create Onboarding"}</button> : null} />
...
function OnboardingWorkspace({ outlet, journey, draft, progress, section, setSection, onViewModule, onViewEmployee }) {
  ...
  const started = progress.filter((item) => item.status !== "not_started").length;
  ...
  const hasPublishedDraft = Boolean(draft && journey.status === "published");
  return <div className="crew-onboarding-workspace is-unified"><section className="crew-onboarding-status-line"><div><PublicationState status={journey.status} version={journey.version} unpublishedChanges={hasPublishedDraft} /><span>{outlet?.name}</span></div><span>Automatic enrollment · Sequential learning</span></section><section className="crew-onboarding-summary" aria-label="Onboarding summary"><Summary label="Modules" value={journey.modules?.length || 0} /><Summary label="Lessons" value={lessonCount(journey)} /><Summary label="Completion" value={`${average}%`} /><Summary label="Crew started" value={started} /><Summary label="Crew completed" value={completed} /></section><AdminSegmentedControl className="crew-onboarding-section-switch" value={section} onChange={setSection} label="Onboarding sections" options={[{ value: "modules", label: "Modules", panelId: "onboarding-modules" }, { value: "progress", label: "Crew Progress", panelId: "onboarding-progress" }]} />{section === "modules" ? <ModuleList journey={journey} progress={progress} onView={onViewModule} /> : <OnboardingProgress rows={progress} onView={onViewEmployee} />}</div>;
}
...
function OnboardingProgress({ rows, onView }) {
  ...
  const activeFilters = [query && { key: "query", label: "Search", value: query, onRemove: () => setQuery("") }, status !== "all" && { key: "status", label: "Status", value: status.replace("_", " "), onRemove: () => setStatus("all") }].filter(Boolean);
  return <section className="crew-progress-panel" id="onboarding-progress"><AdminFilterToolbar ariaLabel="Crew Progress filters" search={<label className="crew-progress-search-control"><span>Search Crew</span><span><Search size={16} /><input aria-label="Search Crew" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search Crew" /></span></label>} filters={<SelectField label="Status" ariaLabel="Status" value={status} onChange={setStatus} options={[{ value: "all", label: "All" }, { value: "not_started", label: "Not Started" }, { value: "in_progress", label: "In Progress" }, { value: "completed", label: "Completed" }]} />} activeFilters={activeFilters} onClear={() => { setQuery(""); setStatus("all"); }} />{visible.length ? <DataTable rows={visible} getRowKey={(row) => row.employee?.id} onRowClick={(row) => onView(row.employee?.id)} columns={[{ key: "employee", header: "Employee", render: (row) => <span className="crew-progress-employee"><strong>{row.employee?.full_name}</strong><small>{row.employee?.position || row.employee?.employee_code}</small></span> }, { key: "progress", header: "Progress", render: (row) => `${row.progress_percentage || 0}%` }, { key: "module", header: "Current Module", render: (row) => row.current_module || (row.status === "completed" ? "Completed" : "Not Started") }, { key: "completed", header: "Completed Modules", render: (row) => `${row.completed_modules || 0} / ${row.total_modules || 0}` }, { key: "started", header: "Started", render: (row) => formatDate(row.started_at) }, { key: "finished", header: "Completed", render: (row) => formatDate(row.completed_at) }, { key: "status", header: "Status", render: (row) => <Badge tone={semanticStatusTone(row.status)}>{String(row.status).replace("_", " ")}</Badge> }]} /> : <EmptyState icon={Users} title="No Crew match these filters" description="Eligible active Crew appear automatically; no manual assignment is required." />}</section>;
}
