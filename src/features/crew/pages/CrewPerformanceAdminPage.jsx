...
function FeedbackAdmin({ data, summary, onModerate, onDetail, onCorrectAttribution, canModerate, canCorrectAttribution }) {
  const [query, setQuery] = useState(""); const [experience, setExperience] = useState("all"); const [status, setStatus] = useState("all");
  const rows = (data || []).filter((row) => {
    const haystack = `${row.employee_name} ${row.comment || ""} ${(row.positive_tags || []).join(" ")} ${(row.improvement_tags || []).join(" ")}`.toLowerCase();
    return (experience === "all" || row.experience === experience) && (status === "all" || row.scoring_status === status) && haystack.includes(query.toLowerCase());
  });
  const totals = summary || {}; const included = Number(totals.included_feedback || 0); const positive = Number(totals.positive_feedback || 0);
  return <div className="crew-growth-stack"><section className="crew-growth-metrics"><Metric icon={MessageSquareText} label="Total Feedback" value={Number(totals.total_feedback || 0)} detail="All submissions this period" /><Metric icon={CheckCircle2} label="Positive Ratio" value={included ? `${Math.round(positive * 100 / included)}%` : "—"} detail="Included feedback only" tone="success" /><Metric icon={AlertTriangle} label="Needs Improvement" value={Number(totals.needs_improvement_feedback || 0)} detail="Included feedback only" tone="warning" /><Metric icon={FileText} label="Excluded" value={Number(totals.excluded_feedback || 0)} detail="Retained in evidence history" /></section><section className="crew-growth-filterbar"><label className="crew-growth-search"><span>Search</span><div><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search Crew, comment or tag" /></div></label><SelectField label="Experience" ariaLabel="Feedback experience" value={experience} onChange={setExperience} options={[{ value: "all", label: "All" }, { value: "great", label: "Great" }, { value: "okay", label: "Okay" }, { value: "needs_improvement", label: "Needs Improvement" }]} /><SelectField label="Scoring" ariaLabel="Feedback scoring" value={status} onChange={setStatus} options={[{ value: "all", label: "All" }, { value: "included", label: "Included" }, { value: "excluded", label: "Excluded" }]} /></section><section className="crew-growth-table"><DataTable rows={rows} getRowKey={(row) => row.id} tableClassName="min-w-[1040px]" columns={[
    { key: "date", header: "Date", render: (row) => formatDate(row.submitted_at) }, { key: "crew", header: "Crew", render: (row) => <span><strong>{row.employee_name}</strong>{row.employee_position ? <small className="block text-text-secondary">{row.employee_position}</small> : null}</span> }, { key: "experience", header: "Experience", render: (row) => <Badge tone={row.experience === "great" ? "success" : row.experience === "needs_improvement" ? "warning" : "neutral"}>{row.experience.replaceAll("_", " ")}</Badge> },
    { key: "tags", header: "Tags", render: (row) => <FeedbackTags row={row} compact /> }, { key: "comment", header: "Comment", render: (row) => <span className="block max-w-56 truncate">{row.comment || "—"}</span> }, { key: "scoring", header: "Scoring", render: (row) => <span><Badge tone={row.scoring_status === "included" ? "info" : "neutral"}>{row.scoring_status}</Badge>{row.scoring_status === "excluded" ? <small className="mt-1 block max-w-40 truncate text-text-secondary" title={row.exclusion_reason || ""}>{row.exclusion_reason || "Reason retained"}</small> : null}</span> },
    { key: "action", header: "Action", align: "right", render: (row) => <span className="flex justify-end gap-2"><button type="button" className="icon-btn" aria-label={`View feedback details for ${row.employee_name}`} title="Feedback details" onClick={() => onDetail(row)}><FileText size={15} /></button>{canCorrectAttribution ? <button type="button" className="icon-btn" aria-label={`Correct Crew attribution for ${row.employee_name}`} title="Correct Crew attribution" onClick={() => onCorrectAttribution(row)}><UserRoundPen size={15} /></button> : null}{canModerate ? <button type="button" className="btn-secondary" onClick={() => onModerate({ ...row, exclude: row.scoring_status === "included" })}>{row.scoring_status === "included" ? "Exclude" : "Restore"}</button> : null}</span> },
  ]} /></section></div>;
}
...
function FeedbackDetail({ item, onClose }) {
  ...
  return <Modal title="Feedback Detail" description="Original guest evidence is retained separately from scoring decisions." size="xl" onClose={onClose} footer={<button className="btn-secondary" onClick={onClose}>Close</button>}><div className="space-y-5 text-sm"><dl className="grid gap-4 sm:grid-cols-2"><DetailItem label="Submitted" value={formatDateTime(item.submitted_at)} /><DetailItem label="Crew" value={item.employee_name} /><DetailItem label="Experience" value={item.experience.replaceAll("_", " ")} /><DetailItem label="Scoring" value={<Badge tone={item.scoring_status === "included" ? "info" : "neutral"}>{item.scoring_status}</Badge>} /></dl><section><h3 className="mb-2 font-semibold">Tags</h3><FeedbackTags row={item} /></section><section><h3 className="mb-2 font-semibold">Comment</h3><p className="whitespace-pre-wrap rounded-lg border border-border bg-surface px-3 py-2 text-text-secondary">{item.comment || "No comment provided."}</p></section>{item.scoring_status === "excluded" ? <section><h3 className="mb-2 font-semibold">Current Exclusion</h3><dl className="grid gap-3 rounded-lg border border-border bg-surface p-3 sm:grid-cols-3"><DetailItem label="Reason" value={item.exclusion_reason || "—"} /><DetailItem label="Excluded by" value={item.excluded_by_name || "—"} /><DetailItem label="Excluded at" value={formatDateTime(item.excluded_at)} /></dl></section> : null}<EvidenceHistory title="Scoring moderation" empty="No scoring moderation has been recorded." entries={moderation} render={(entry) => <><strong>{entry.previous_status} to {entry.next_status}</strong><span>{entry.reason}</span><small>{entry.changed_by || "Admin"} · {formatDateTime(entry.changed_at)}</small></>} /><EvidenceHistory title="Attribution corrections" empty="No attribution correction has been recorded." entries={attribution} render={(entry) => <><strong>{entry.previous_employee_name} to {entry.next_employee_name}</strong><span>{entry.reason}</span><small>{entry.changed_by || "Admin"} · {formatDateTime(entry.changed_at)}</small></>} /></div></Modal>;
}
