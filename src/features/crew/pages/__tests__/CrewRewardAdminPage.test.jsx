...
vi.mock("../../../../services/crewService.js", () => ({ crewService: {
  rewardAdminData: mocks.data, createRewardCycle: mocks.create, calculateRewardCycle: mocks.calculate,
...
const fixture = { cycles: [cycle], cycle, entries: [entry], adjustments: [] };
...
describe("Crew Reward Admin", () => {
  ...
  it("creates a cycle through the controlled authority", async () => {
    mocks.data.mockResolvedValue({ cycles: [], cycle: null, entries: [], adjustments: [] });
    render(<CrewRewardAdminPage auth={auth} ui={ui} store={{ outlets: [outlet] }} />);
    fireEvent.click(await screen.findByRole("button", { name: "New Reward Cycle" }));
    fireEvent.click(screen.getByRole("button", { name: "Create Draft" }));
    await waitFor(() => expect(mocks.create).toHaveBeenCalledWith(expect.objectContaining({ outletId: "outlet-1", configuredPool: 500, minimumPerformance: 60 })));
  });

  it("records adjustments with a required audit reason", async () => {
    render(<CrewRewardAdminPage auth={auth} ui={ui} store={{ outlets: [outlet] }} />);
    fireEvent.click(await screen.findByRole("button", { name: "Adjust" }));
    fireEvent.change(screen.getByPlaceholderText("Example: 10 or -5"), { target: { value: "5" } });
