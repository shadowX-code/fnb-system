...
vi.mock("../../../../services/crewService.js", () => ({ crewService: { tasksAdminPage: mocks.data, saveTask: mocks.save, ensureTaskDraft: mocks.ensure, activateOperationTemplate: mocks.activate, duplicateTask: mocks.duplicate, reviewTask: mocks.review, manageTaskSchedule: mocks.manage, taskAdminDetail: mocks.detail, taskAdminResult: mocks.result, localizedContentAdmin: mocks.localization, saveLocalizedContentUnits: mocks.saveLocalization, translateLocalizedContent: mocks.translate, editLocalizedTranslation: mocks.editTranslation, reviewLocalizedTranslation: mocks.reviewTranslation } }));
...
describe("Crew unified Tasks Admin", () => {
  it("renders one Tasks workspace with status filters and actions", async () => {
    ...
    expect(screen.getAllByRole("button", { name: /Create Task/ }).length).toBeGreaterThan(0);
  ...
  it("keeps a 16-block checklist compact and revision-safe", async () => {
    ...
    mocks.data.mockImplementation(({ listing }) => Promise.resolve({ rows: listing === "review_queue" ? fixture.review_queue : [{ ...fixture.definitions[0], blocks }], total_count: listing === "review_queue" ? fixture.review_queue.length : 1, page: 1, page_size: 20, summary: { published_sops: fixture.published_sops, employees: fixture.employees } }));
    render(<CrewOperationsAdminPage auth={auth} ui={ui} store={{ outlets: [outlet] }} />);
  ...
  it("reuses the existing Draft instead of creating another revision", async () => {
    ...
    mocks.data.mockImplementation(({ listing }) => Promise.resolve({ rows: listing === "review_queue" ? fixture.review_queue : [currentWithDraft], total_count: listing === "review_queue" ? fixture.review_queue.length : 1, page: 1, page_size: 20, summary: { published_sops: fixture.published_sops, employees: fixture.employees } }));
