...
export default function CrewCashCheckoutAdminPage({ auth, ui, store }) {
  ...
  return <div className="space-y-4">
    ...
    {collectionOpen && <CollectionForm outletId={outletId} employees={data.employees} balance={data.summary.current_balance} onClose={() => setCollectionOpen(false)} onSaved={async () => { setCollectionOpen(false); await refresh(); }} ui={ui} />}
  </div>;
}
...
function CollectionForm({ outletId, employees, balance, onClose, onSaved, ui }) {
  const [form, setForm] = useState({ request_id: crypto.randomUUID(), receiver_type: "external", receiver_employee_id: "", receiver_name: "", amount: "", purpose: "Cash deposit collection", note: "" }); const [saving, setSaving] = useState(false);
  async function submit(event) { event.preventDefault(); setSaving(true); try { await crewService.recordAdminCashCollection(outletId, form); await onSaved(); ui.notify({ title: form.receiver_type === "internal" ? "Handover submitted" : "Collection recorded", message: "The Cash Deposit Balance was updated immediately." }); } catch (cause) { ui.notify({ title: "Unable to record collection", message: cause.message, tone: "error" }); } finally { setSaving(false); } }
  const employeeOptions = employees.map((item) => ({ value: item.id, label: `${item.name} · ${item.position}` }));
  return <Modal title="Record Cash Collection" description={`Cash Deposit Balance: ${money(balance)}`} onClose={onClose} footer={<><button className="btn-secondary" onClick={onClose}>Cancel</button><button form="collection-form" type="submit" className="btn-primary" disabled={saving}>{saving ? "Saving…" : "Record Collection"}</button></>}><form id="collection-form" className="space-y-4" onSubmit={submit}><SelectField label="Receiver Type" value={form.receiver_type} options={receiverTypeOptions} onChange={(receiver_type) => setForm({ ...form, receiver_type, receiver_employee_id: "", receiver_name: "" })} />{form.receiver_type === "internal" ? <><SelectField label="Receiver" required searchable value={form.receiver_employee_id} options={employeeOptions} placeholder="Search Crew" onChange={(receiver_employee_id) => setForm({ ...form, receiver_employee_id })} /><p className="-mt-2 rounded-xl bg-primary/5 px-3 py-2 text-xs text-text-secondary">The balance updates on submission; the receiver confirmation is acknowledgement only.</p></> : <Field label="Receiver Name" required><input aria-label="Receiver Name" className="control h-10" required value={form.receiver_name} onChange={(event) => setForm({ ...form, receiver_name: event.target.value })} /></Field>}<MoneyField label="Amount (RM)" required value={form.amount} min="0.05" max={Number(balance || 0)} onChange={(amount) => setForm({ ...form, amount })} /><SelectField label="Purpose" value={form.purpose} options={collectionPurposeOptions} onChange={(purpose) => setForm({ ...form, purpose })} /><Field label="Note (optional)"><textarea className="control min-h-20" value={form.note} onChange={(event) => setForm({ ...form, note: event.target.value })} /></Field></form></Modal>;
}
