...
import { Award, ChevronDown, ChevronRight, ClipboardCheck, MoreVertical, Plus, RotateCcw, Search, ShieldCheck, Sparkles, Trash2, UsersRound, X } from "lucide-react";
...
function RequirementRow({ row, index, evidence, allowedTypes, expanded, onToggle, onChange, onMove, onDelete, canMoveUp, canMoveDown }) {
  ...
  return <article className={`crew-growth-requirement ${expanded ? "is-expanded" : ""}`}><div className="crew-growth-requirement-summary"><button type="button" className="crew-growth-requirement-toggle" onClick={onToggle} aria-expanded={expanded}><span className="crew-growth-requirement-order">{String(index + 1).padStart(2, "0")}</span><span className="min-w-0"><strong>{requirementLabel(row.type)}</strong><small>{summary}</small></span><Badge tone={row.required !== false ? "info" : "neutral"}>{row.required !== false ? "Required" : "Optional"}</Badge></button><button className="icon-btn" type="button" aria-label={`More actions for requirement ${index + 1}`} onClick={() => setMenuOpen((current) => !current)}><MoreVertical size={15} /></button><button className="icon-btn" type="button" aria-label={`${expanded ? "Collapse" : "Expand"} requirement ${index + 1}`} onClick={onToggle}><ChevronDown className={expanded ? "rotate-180" : ""} size={15} /></button></div>{menuOpen ? <div className="crew-growth-requirement-menu"><button className="btn-secondary" type="button" disabled={!canMoveUp} onClick={() => { onMove(index, -1); setMenuOpen(false); }}>Move Up</button><button className="btn-secondary" type="button" disabled={!canMoveDown} onClick={() => { onMove(index, 1); setMenuOpen(false); }}>Move Down</button><button className="btn-secondary text-rose-700" type="button" onClick={onDelete}><Trash2 size={13} /> Delete</button></div> : null}{expanded ? <div className="crew-growth-requirement-fields"><SelectField label="Requirement Type" value={row.type} onChange={(type) => onChange({ type, reference_id: "", label: requirementLabel(type), config: {} })} options={allowedTypes} />{needsEvidence ? <SelectField searchable label="Evidence" value={row.reference_id || ""} onChange={(reference_id) => onChange({ reference_id, label: evidence.find((item) => item.id === reference_id)?.label || "" })} options={options} placeholder="Select published evidence" /> : <Field label={row.type === "practical" ? "Assessment Name" : "Requirement Name"}><input className="control" value={row.label || ""} onChange={(event) => onChange({ label: event.target.value })} /></Field>}{row.type === "practical" ? <div className="crew-growth-checklist"><div><strong>Assessment Checklist</strong><small>Define observable standards for the practical assessment.</small></div>{checklist.map((item, itemIndex) => <div className="crew-growth-checklist-row" key={`${itemIndex}:${item}`}><input className="control" aria-label={`Checklist item ${itemIndex + 1}`} value={item} onChange={(event) => updateChecklist(checklist.map((entry, i) => i === itemIndex ? event.target.value : entry))} /><button className="icon-btn" type="button" aria-label={`Remove checklist item ${itemIndex + 1}`} onClick={() => updateChecklist(checklist.filter((_, i) => i !== itemIndex))}><X size={15} /></button></div>)}<button className="btn-secondary" type="button" onClick={() => updateChecklist([...checklist, ""])}><Plus size={14} /> Add checklist item</button></div> : null}<label className="crew-growth-required"><input type="checkbox" checked={row.required !== false} onChange={(event) => onChange({ required: event.target.checked })} /> Required</label></div> : null}</article>;
}
