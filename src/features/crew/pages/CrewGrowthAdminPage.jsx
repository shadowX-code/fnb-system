...
import { Award, ChevronDown, ChevronRight, ClipboardCheck, Eye, MoreVertical, Pencil, Plus, Search, ShieldCheck, Trash2, UsersRound, X } from "lucide-react";
...
function GrowthOverview({ data, outletSelect, onOpenReview, onOpenEmployee }) {
  ...
  return <div className="crew-growth-stack">
    <AdminFilterToolbar ariaLabel="Growth filters" outlet={outletSelect} search={<SearchField label="Search Crew" value={query} onChange={setQuery} placeholder="Search by name or employee code" />} filters={<><SelectField label="Position" value={position} onChange={setPosition} options={[allOption, ...positions.map((value) => ({ value, label: value }))]} /><SelectField label="Growth Status" value={status} onChange={setStatus} options={[allOption, { value: "on_track", label: "On Track" }, { value: "attention", label: "Needs Attention" }, { value: "not_started", label: "Not Started" }]} /></>} activeFilters={[query && { key: "query", label: "Search", value: query, onRemove: () => setQuery("") }, position !== "all" && { key: "position", label: "Position", value: position, onRemove: () => setPosition("all") }, status !== "all" && { key: "status", label: "Growth status", value: status.replaceAll("_", " "), onRemove: () => setStatus("all") }].filter(Boolean)} onClear={clearFilters} />
    <AdminSummaryGrid ariaLabel="Growth metrics" items={[{ label: "Active Crew", value: compactNumber(activeCrew), helper: "Selected outlet", icon: UsersRound }, { label: "Certified Crew", value: compactNumber(certified), helper: "At least one current certification", tone: "success", icon: Award }, { label: "Needs Review / Attention", value: compactNumber(attention), helper: "Actionable review or renewal risk", tone: attention ? "warning" : "success", icon: ClipboardCheck }]} />
    {reviewRows.length ? <NeedsReviewSection rows={reviewRows} onOpenReview={onOpenReview} /> : null}
    <section className="crew-growth-table crew-growth-crew-table"><p className="px-4 pt-3 text-sm font-semibold text-text-secondary">{crewRows.length} of {activeCrew} active Crew</p>{crewRows.length ? <DataTable rows={crewRows} getRowKey={(row) => row.employee.id} onRowClick={onOpenEmployee} tableClassName="min-w-[980px]" columns={crewColumns(onOpenEmployee, onOpenReview)} /> : <Empty title={data.crew.length ? "No Crew match these filters" : "No active Crew in this outlet"} detail={data.crew.length ? "Clear or adjust the filters to see more Crew." : "Active Crew capability profiles will appear here."} />}</section>
...
function crewColumns(onOpenEmployee, onOpenReview) { return [
  ...
  { key: "status", header: "Growth Status", render: (row) => <Badge tone={semanticStatusTone(growthStatus(row.summary) === "attention" ? "needs_attention" : growthStatus(row.summary) === "on_track" ? "active" : "not_started")}>{growthStatus(row.summary) === "attention" ? "Needs Attention" : growthStatus(row.summary) === "on_track" ? "On Track" : "Not Started"}</Badge> },
...
function SkillsLibrary({ data, canManage, onView, outletSelect, positionOptions }) {
  ...
  return <div className="crew-growth-stack">
    ...
    <section className="crew-growth-table"><DataTable rows={rows} getRowKey={(row) => row.id} onRowClick={onView} tableClassName="min-w-[960px]" columns={[
      ...
      { key: "status", header: "Status", render: (row) => <Badge tone={semanticStatusTone(row.status)}>{row.status === "active" ? "Active" : "Inactive"}</Badge> },
      { key: "actions", header: "Actions", align: "right", render: (row) => <button className="btn-secondary px-3 py-1.5 text-xs" type="button" onClick={() => onView(row)}>{canManage ? <><Pencil size={14} /> Edit</> : <><Eye size={14} /> View</>}</button> },
    ]} /></section>
  </div>;
}
...
function SkillEditor({ skill, evidence, outlet, positionOptions, saving, onClose, onSave }) {
  ...
  const addRequirement = (type) => {
    const row = blankRequirement(type);
    set({ requirements: [...form.requirements, row] });
    setExpandedRequirement(row.editor_key);
  ...
  const submit = () => {
    ...
    onSave({ ...form, validity_months: expiryMode === "none" ? "" : String(form.validity_months), requirements: form.requirements.map(({ editor_key, ...row }) => ({ ...row, label: row.label || evidence.find((item) => item.id === row.reference_id)?.label || requirementLabel(row.type) })) });
  };

  return <Modal title={skill.id ? "Edit Skill" : "New Skill"} description={`${outlet?.name || "Outlet"} · Certification configuration`} size="xl" onClose={onClose} bodyClassName="crew-growth-skill-editor" footer={<div className="crew-growth-editor-footer"><span>{validation || ""}</span><div><button className="btn-secondary" onClick={onClose}>Cancel</button><button className="btn-primary" disabled={saving || Boolean(validation)} onClick={submit}>{saving ? "Saving..." : "Save Skill"}</button></div></div>}>
    <section className="crew-growth-skill-setup"><div className="crew-growth-form-grid"><Field label="Skill Name" required><input aria-label="Skill Name" className="control" value={form.name} onChange={(event) => set({ name: event.target.value })} /></Field><SelectField label="Category" value={form.category} onChange={(category) => set({ category })} options={SKILL_CATEGORIES.map((value) => ({ value, label: value }))} required /><Field label="Description" className="is-wide"><textarea className="control" value={form.description} onChange={(event) => set({ description: event.target.value })} /></Field><div className="crew-growth-positions-field is-wide"><MultiSelectField label="Applicable Positions" value={form.positions} options={mergeExistingOptions(positionOptions, form.positions)} onApply={(positions) => set({ positions })} placeholder="All active Crew" /><p>Leaving this empty applies the skill to all active Crew in the selected outlet.</p></div><SelectField label="Status" value={form.status} onChange={(status) => set({ status })} options={SKILL_STATUSES} /><SelectField label="Certification Method" value={form.certification_method} onChange={(certification_method) => set({ certification_method, requirements: normalizeRequirementsForMethod(form.requirements, certification_method) })} options={CERTIFICATION_METHODS} /><SelectField label="Certification Validity" value={expiryMode} onChange={setExpiryMode} options={[{ value: "none", label: "No Expiry" }, { value: "months", label: "Expires after X months" }]} />{expiryMode === "months" ? <Field label="Expiry Months" required><input aria-label="Expiry Months" className="control" type="number" min="1" max="120" value={form.validity_months} onChange={(event) => set({ validity_months: event.target.value })} /></Field> : null}</div></section>
    <section className="crew-growth-requirements-editor"><header><div><h3>Certification Requirements</h3><p>Define what Crew must complete before certification.</p></div><button className="btn-secondary" type="button" onClick={() => setRequirementPickerOpen((current) => !current)}><Plus size={14} /> Add Requirement</button></header>{skill.id ? <p className="crew-growth-version-notice">Changes create a new requirements version. Existing certification evidence is preserved.</p> : null}{requirementPickerOpen ? <div className="crew-growth-requirement-picker" role="group" aria-label="Requirement type"><span>Choose requirement type</span><div>{allowedTypes.map((type) => <button key={type.value} className="btn-secondary" type="button" onClick={() => addRequirement(type.value)}>{type.label}</button>)}</div></div> : null}{form.requirements.map((row, index) => <RequirementRow key={row.editor_key || row.id || index} row={row} index={index} evidence={evidence} allowedTypes={allowedTypes} expanded={expandedRequirement === row.editor_key || expandedRequirement === row.id} onToggle={() => setExpandedRequirement((current) => current === (row.editor_key || row.id) ? null : (row.editor_key || row.id))} onChange={(patch) => changeRequirement(index, patch)} onMove={moveRequirement} onDelete={() => set({ requirements: form.requirements.filter((_, i) => i !== index) })} canMoveUp={index > 0} canMoveDown={index < form.requirements.length - 1} />)}{!form.requirements.length ? <Empty title="No requirements yet" detail="Add a requirement type to begin the certification path." /> : null}</section>
  </Modal>;
}

function RequirementRow({ row, index, evidence, allowedTypes, expanded, onToggle, onChange, onMove, onDelete, canMoveUp, canMoveDown }) {
  ...
  const summary = row.label || options.find((item) => item.value === row.reference_id)?.label || "Select evidence";
  const checklist = row.config?.items || [];
  const updateChecklist = (items) => onChange({ config: { ...(row.config || {}), items } });
  return <article className={`crew-growth-requirement ${expanded ? "is-expanded" : ""}`}><div className="crew-growth-requirement-summary"><button type="button" className="crew-growth-requirement-toggle" onClick={onToggle} aria-expanded={expanded}><span className="crew-growth-requirement-order">{String(index + 1).padStart(2, "0")}</span><span className="min-w-0"><strong>{requirementLabel(row.type)}</strong><small>{summary}</small></span><Badge tone={row.required !== false ? "info" : "neutral"}>{row.required !== false ? "Required" : "Optional"}</Badge></button><ActionMenu open={menuOpen} onOpenChange={setMenuOpen} width={150} ariaLabel={`More actions for requirement ${index + 1}`} trigger={({ toggle, ariaLabel }) => <button className="icon-btn" type="button" aria-label={ariaLabel} onClick={toggle}><MoreVertical size={15} /></button>}><div className="grid gap-1"><button className="btn-secondary justify-start" type="button" disabled={!canMoveUp} onClick={() => { onMove(index, -1); setMenuOpen(false); }}>Move Up</button><button className="btn-secondary justify-start" type="button" disabled={!canMoveDown} onClick={() => { onMove(index, 1); setMenuOpen(false); }}>Move Down</button><button className="btn-secondary justify-start text-rose-700" type="button" onClick={onDelete}><Trash2 size={13} /> Delete</button></div></ActionMenu><button className="icon-btn" type="button" aria-label={`${expanded ? "Collapse" : "Expand"} requirement ${index + 1}`} onClick={onToggle}><ChevronDown className={expanded ? "rotate-180" : ""} size={15} /></button></div>{expanded ? <div className="crew-growth-requirement-fields"><SelectField label="Requirement Type" value={row.type} onChange={(type) => onChange({ type, reference_id: "", label: requirementLabel(type), config: {} })} options={allowedTypes} />{needsEvidence ? <SelectField searchable label="Evidence" value={row.reference_id || ""} onChange={(reference_id) => onChange({ reference_id, label: evidence.find((item) => item.id === reference_id)?.label || "" })} options={options} placeholder="Select published evidence" /> : <Field label={row.type === "practical" ? "Assessment Name" : "Requirement Name"}><input className="control" value={row.label || ""} onChange={(event) => onChange({ label: event.target.value })} /></Field>}{row.type === "practical" ? <div className="crew-growth-checklist"><div><strong>Assessment Checklist</strong><small>Define observable standards for the practical assessment.</small></div>{checklist.map((item, itemIndex) => <div className="crew-growth-checklist-row" key={`${itemIndex}:${item}`}><input className="control" aria-label={`Checklist item ${itemIndex + 1}`} value={item} onChange={(event) => updateChecklist(checklist.map((entry, i) => i === itemIndex ? event.target.value : entry))} /><button className="icon-btn" type="button" aria-label={`Remove checklist item ${itemIndex + 1}`} onClick={() => updateChecklist(checklist.filter((_, i) => i !== itemIndex))}><X size={15} /></button></div>)}<button className="btn-secondary" type="button" onClick={() => updateChecklist([...checklist, ""])}><Plus size={14} /> Add checklist item</button></div> : null}<label className="crew-growth-required"><input type="checkbox" checked={row.required !== false} onChange={(event) => onChange({ required: event.target.checked })} /> Required</label></div> : null}</article>;
}

function CrewGrowthProfile({ row, onClose, onReview }) {
  const summary = summarize(row.skills);
  return <Modal title="Crew Growth Profile" description="Durable skill progress and certification history" size="xl" onClose={onClose} footer={<button className="btn-secondary" onClick={onClose}>Close</button>} panelClassName="crew-growth-profile-modal"><div className="crew-growth-profile"><section className="crew-growth-profile-identity"><InitialAvatar name={row.employee.full_name} size="lg" /><div><h3>{row.employee.full_name}</h3><p>{row.employee.position || "Crew"}</p><span>{row.employee.employee_code || "Active Crew"}</span></div><Badge tone="success">Active</Badge></section><section className="crew-growth-profile-summary"><ProfileMetric label="Certified" value={summary.certified} /><ProfileMetric label="In Progress" value={summary.inProgress} /><ProfileMetric label="Needs Review" value={summary.ready} /><ProfileMetric label="Not Started" value={row.skills.filter((state) => state.status === "not_started").length} /></section><div className="crew-growth-profile-list"><header><strong>Skill Progress</strong><span>{row.skills.length} skills</span></header>{row.skills.map((state) => <button key={state.skill_id} onClick={() => onReview({ employee_id: row.employee.id, employee_name: row.employee.full_name, position: row.employee.position, skill_id: state.skill_id, skill_name: state.skill?.name || "Skill", state })}><span><strong>{state.skill?.name || "Skill"}</strong><small>{state.status === "certified" ? `Certified ${formatDate(state.certification?.certified_at)}` : `${state.requirements_completed} / ${state.requirements_total} requirements complete`}</small></span><div className="crew-growth-skill-row-progress"><i style={{ width: `${state.requirements_total ? Math.round(state.requirements_completed * 100 / state.requirements_total) : 0}%` }} /></div><StateBadge state={state.status} /><ChevronRight size={15} /></button>)}</div></div></Modal>;
}
...
function blankRequirement(type) { return { editor_key: `new:${crypto.randomUUID()}`, type, reference_id: "", label: requirementLabel(type), required: true, config: type === "practical" ? { items: [] } : {} }; }
function normalizeRequirements(rows) { return rows.map((row, index) => normalizeRequirementPatch({ ...row, editor_key: row.editor_key || row.id || `existing:${index}`, type: row.type || "module", label: row.label || requirementLabel(row.type || "module"), required: row.required !== false, config: row.config || {} })); }
...
function StateBadge({ state }) { return <Badge tone={semanticStatusTone(state)}>{statusText(state)}</Badge>; }
...
function ProfileMetric({ label, value }) { return <article className="crew-growth-metric"><span><small>{label}</small><strong>{value}</strong></span></article>; }
...
function Field({ label, required = false, className = "", children }) { return <AdminFormField label={label} required={required} className={className}>{children}</AdminFormField>; }
