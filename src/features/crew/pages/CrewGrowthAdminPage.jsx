...
const stateTone = (state) => state === "certified" ? "success" : state === "ready_for_review" ? "info" : ["needs_renewal", "expired"].includes(state) ? "warning" : "neutral";
...
function GrowthOverview({ data, onOpenReview }) {
  ...
  return <div className="crew-growth-stack"><section className="crew-growth-metrics" aria-label="Growth metrics"><Metric icon={UsersRound} label="Active Crew" value={activeCrew} /><Metric icon={Award} label="Certified Skills" value={certified} /><Metric icon={ClipboardCheck} label="Certifications Pending" value={pending} /><Metric icon={UserRoundCheck} label="Crew Need Attention" value={attention} tone={attention ? "warning" : "success"} /></section><div className="crew-growth-overview-grid"><section className="crew-growth-section"><SectionHead title="Skill Coverage" detail="Certified active Crew by applicable skill" /><div className="crew-growth-coverage">{data.skills.filter((skill) => skill.status === "active").map((skill) => { const eligible = states.filter((row) => row.skill_id === skill.id); const count = eligible.filter((row) => row.status === "certified").length; const percent = eligible.length ? Math.round(count * 100 / eligible.length) : 0; return <article key={skill.id}><div><strong>{skill.name}</strong><span>{count} / {eligible.length} Certified</span></div><div><span style={{ width: `${percent}%` }} /></div><b>{percent}%</b></article>; })}{!data.skills.length ? <Empty title="No skills configured" detail="Create the first outlet skill in Skills." /> : null}</div></section><section className="crew-growth-section"><SectionHead title="Needs Attention" detail="Ready reviews and renewal risks" />{data.reviews.length ? <DataTable density="compact" rows={data.reviews.slice(0, 8)} getRowKey={(row) => `${row.employee_id}:${row.skill_id}`} onRowClick={onOpenReview} tableClassName="min-w-[640px]" columns={[{ key: "employee", header: "Employee", render: (row) => <NameCell title={row.employee_name} detail={row.position} /> }, { key: "skill", header: "Skill", render: (row) => row.skill_name }, { key: "issue", header: "Issue", render: (row) => attentionIssue(row.state.status) }, { key: "status", header: "Status", render: (row) => <StateBadge state={row.state.status} /> }, { key: "open", header: "", align: "right", render: () => <ChevronRight size={16} /> }]} /> : <Empty title="No Crew need attention" detail="Ready reviews and renewal risks will appear here." />}</section></div><section className="crew-growth-section"><SectionHead title="Recent Certifications" detail="Immutable certification history" />{data.recent_certifications.length ? <DataTable density="compact" rows={data.recent_certifications.slice(0, 10)} getRowKey={(row) => row.id} columns={[{ key: "employee", header: "Employee", render: (row) => row.employee_name }, { key: "skill", header: "Skill", render: (row) => row.skill_name }, { key: "date", header: "Certified Date", render: (row) => formatDate(row.certified_at) }, { key: "by", header: "Certified By", render: (row) => row.certified_by }, { key: "expiry", header: "Expiry", render: (row) => row.expires_at ? formatDate(row.expires_at) : "No Expiry" }]} /> : <Empty title="No certifications yet" detail="Completed certifications will form the permanent activity record." />}</section></div>;
}
...
function CrewGrowthProfile({ row, onClose, onReview }) {
  ...
  return <Modal title={row.employee.full_name} description={`${row.employee.position || "Crew"} · Growth profile`} size="xl" onClose={onClose} footer={<button className="btn-secondary" onClick={onClose}>Close</button>}><div className="crew-growth-profile"><section className="crew-growth-profile-summary"><Metric label="Certified" value={summary.certified} /><Metric label="In Progress" value={summary.inProgress} /><Metric label="Ready for Review" value={summary.ready} /></section><div className="crew-growth-profile-list">{row.skills.map((state) => <button key={state.skill_id} onClick={() => onReview({ employee_id: row.employee.id, employee_name: row.employee.full_name, position: row.employee.position, skill_id: state.skill_id, skill_name: state.skill?.name || "Skill", state })}><span><strong>{state.skill?.name || "Skill"}</strong><small>{state.status === "certified" ? formatDate(state.certification?.certified_at) : `${state.requirements_completed} / ${state.requirements_total} requirements complete`}</small></span><StateBadge state={state.status} /><ChevronRight size={16} /></button>)}</div></div></Modal>;
}
...
function CertificationReview({ item, canAssess, canCertify, onClose, onAssess, onCertify }) {
  ...
  return <Modal title={item.skill_name} description={`${item.employee_name} · ${item.position || "Crew"}`} size="xl" onClose={onClose} footer={<><button className="btn-secondary" onClick={onClose}>Close</button>{canAssess && state.requirements.some((row) => row.type === "practical") ? <button className="btn-secondary" disabled={saving} onClick={assess}>{passPractical ? "Pass Assessment" : "Record Improvement"}</button> : null}{canCertify ? <button className="btn-primary" disabled={saving || !eligibleToCertify(state)} onClick={certify}>Certify</button> : null}</>}><div className="crew-growth-review"><header><StateBadge state={state.status} /><span>{state.requirements_completed} / {state.requirements_total} requirements complete</span></header><section><h3>Requirement Evidence</h3>{state.requirements.map((row) => <article key={row.requirement_id}><span className={row.completed ? "is-done" : ""}>{row.completed ? "✓" : "○"}</span><div><strong>{row.label}</strong><small>{row.detail}</small></div>{row.score != null ? <Badge tone="info">{row.score}%</Badge> : null}</article>)}</section>{state.requirements.some((row) => row.type === "practical") ? <section><h3>Manager Practical Assessment</h3>{checklist.map((row, index) => <div className="crew-growth-assessment-row" key={row.label}><strong>{row.label}</strong><SelectField ariaLabel={`${row.label} rating`} value={row.rating} onChange={(rating) => setChecklist((current) => current.map((item, i) => i === index ? { ...item, rating } : item))} options={[{ value: "meets_standard", label: "Meets Standard" }, { value: "needs_improvement", label: "Needs Improvement" }, { value: "not_observed", label: "Not Observed" }]} /></div>)}</section> : null}<label>Manager Note<textarea className="control" value={note} onChange={(event) => setNote(event.target.value)} placeholder="Optional assessment or certification note" /></label></div></Modal>;
}
...
function NameCell({ title, detail }) { return <span className="crew-growth-name"><strong>{title}</strong><small>{detail || "—"}</small></span>; }
function Metric({ icon: Icon = Sparkles, label, value, tone = "neutral" }) { return <article className={`crew-growth-metric is-${tone}`}>{Icon ? <Icon size={18} /> : null}<span><strong>{value}</strong><small>{label}</small></span></article>; }
