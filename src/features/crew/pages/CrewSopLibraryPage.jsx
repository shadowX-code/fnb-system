...
function SopLibrary({ outletControl, outlet, sops, categories, loading, error, onRetry, canManage, onOpen, onEdit, onCreate, onUsage, onNewVersion, onDeleteDraft }) {
  ...
  const emptyTitle = sops.length ? "No SOPs match these filters" : "No SOPs yet";
...
function SopRowActions({ row, canManage, onOpen, onEdit, onNewVersion, onDeleteDraft }) {
  ...
  return <div className="crew-sop-row-actions">
    {primaryAction}
    <ActionMenu open={menuOpen} onOpenChange={setMenuOpen} ariaLabel={`More actions for ${row.title}`} trigger={({ toggle, ariaLabel }) => <button className="icon-btn crew-sop-compact-more" aria-label={ariaLabel} aria-expanded={menuOpen} type="button" onClick={toggle}><MoreHorizontal size={16} /></button>}><div role="menu"><button role="menuitem" type="button" onClick={() => { setMenuOpen(false); onOpen(row.id); }}>View</button>{canManage && draft ? <button role="menuitem" className="is-danger" type="button" onClick={() => { setMenuOpen(false); onDeleteDraft(row); }}>Delete Draft</button> : null}</div></ActionMenu>
  </div>;
}
...
function PublishedDocument({ version, showOutline = true }) {
  ...
  return <div className={`crew-sop-document-shell ${showOutline ? "" : "is-reader"}`}>
    ...
    <main className={showOutline ? "" : "crew-sop-document-scroll"}><div className="crew-sop-document-meta"><div><Badge tone={semanticStatusTone(version.status)}>{version.status === "published" ? "Published" : "Draft preview"}</Badge><span>v{version.version}</span></div></div><CrewSopDocument sections={sections} admin sectionRefs={refs} /></main>
  </div>;
}
...
function SopEditor({ sop, outlet, version, saving, onBack, onRefresh, onConfirm, onPublish, onDeleteDraft }) {
  ...
  function moveSectionTo(sectionId, targetId) {
    setSections((current) => moveById(current, sectionId, targetId));
    setDirty(true);
  }
  ...
  return <Modal title={sop.title} description={`Draft v${version.version} · ${outlet?.name}`} size="2xl" panelClassName="crew-sop-editor-popout" bodyClassName={`crew-sop-editor-popout-body ${pane === "preview" ? "is-preview" : ""}`} onClose={requestClose} headerActions={<span className={`crew-sop-save-state ${dirty ? "is-dirty" : "is-saved"}`}>{dirty ? "Unsaved changes" : <><Check size={13} /> Saved</>}</span>} footer={footer} footerClassName="block">
    {pane === "edit" ? <div className="crew-sop-draft-workspace">
      <aside><div><strong>Section Outline</strong><span>{sections.length}</span></div>{sections.map((section, index) => <div key={section.id} className="crew-sop-outline-row" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { const drag = readSortableDrag(event); if (drag?.scope === "sop-section" && drag.itemId !== section.id) moveSectionTo(drag.itemId, section.id); }}><AdminSortableHandle label={`Reorder section ${index + 1}`} scope="sop-section" itemId={section.id} index={index} count={sections.length} onMove={(direction) => moveSection(section.id, direction)} /><button className={selected?.id === section.id ? "is-active" : ""} onClick={() => setSelectedId(section.id)}><span>{String(index + 1).padStart(2, "0")}</span><strong>{section.title || "Untitled Section"}</strong><ChevronRight size={15} /></button></div>)}<button className="crew-sop-add-section" onClick={addSection}><Plus size={15} /> Add Section</button></aside>
...
function UsageView({ sopId, onNavigate }) {
  ...
  if (!usage) return <LoadingSkeleton rows={3} />;
...
function CreateSopModal({ categories, targetOutlet, sourceOutlets, saving, onClose, onCreate, onCloned }) {
  ...
  function moveSection(sectionId, direction) { const index = sections.findIndex((row) => row.id === sectionId); const target = index + direction; if (target < 0 || target >= sections.length) return; const next = [...sections]; [next[index], next[target]] = [next[target], next[index]]; setSections(next); }
  function moveSectionTo(sectionId, targetId) { setSections((current) => moveById(current, sectionId, targetId)); }
  ...
  return <Modal title="Create SOP" description={`${targetOutlet?.name || "Outlet"} · Start blank or clone an existing published SOP`} size="xl" panelClassName={`crew-sop-create-popout ${mode === "clone" ? "is-clone" : ""}`} bodyClassName="crew-sop-create-popout-body" onClose={onClose} footer={footer} footerClassName="block">
    ...
    {mode === "blank" ? <div className="crew-sop-create-workspace">
      <aside className="crew-sop-create-meta">
        ...
        <div className="crew-sop-create-outline"><header><strong>Sections</strong><span>{sections.length}</span></header>{sections.map((section, index) => <div key={section.id} className="crew-sop-outline-row" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { const drag = readSortableDrag(event); if (drag?.scope === "create-sop-section" && drag.itemId !== section.id) moveSectionTo(drag.itemId, section.id); }}><AdminSortableHandle label={`Reorder section ${index + 1}`} scope="create-sop-section" itemId={section.id} index={index} count={sections.length} onMove={(direction) => moveSection(section.id, direction)} /><button className={section.id === selected.id ? "is-active" : ""} onClick={() => setSelectedId(section.id)}><span>{String(index + 1).padStart(2, "0")}</span><strong>{section.title || "Untitled Section"}</strong><ChevronRight size={15} /></button></div>)}<button className="btn-secondary crew-sop-add-section" onClick={addSection}><Plus size={15} /> Add Section</button></div>
      </aside>
      <main className="crew-sop-create-section"><div className="crew-sop-editor-form-head"><div><span>Section {sections.findIndex((row) => row.id === selected.id) + 1}</span><h2>{selected.title}</h2></div><button className="icon-btn is-danger" disabled={sections.length === 1} onClick={removeSection} aria-label="Delete section"><Trash2 size={16} /></button></div><AdminFormField label="Section Title" required><input className="control w-full" value={selected.title} onChange={(event) => updateSelected({ title: event.target.value })} /></AdminFormField><AdminFormField label="Content" as="div"><RichTextEditor value={selected.editorHtml} onChange={(editorHtml) => updateSelected({ editorHtml })} onImage={chooseImage} /></AdminFormField>{selected.pendingImage ? <div className="crew-sop-image-placeholder"><div><img src={selected.pendingImage.url} alt="SOP draft preview" /></div><AdminFormField label="Image caption"><input className="control w-full" value={selected.pendingImage.caption} onChange={(event) => updateSelected({ pendingImage: { ...selected.pendingImage, caption: event.target.value } })} /></AdminFormField><button className="btn-secondary is-danger" onClick={() => updateSelected({ pendingImage: null })}>Remove Image</button><p>Uploads securely when the complete draft is saved.</p></div> : null}<ToggleField label="Key Point" helper="Optional callout after the section content." checked={Boolean(selected.keyPointContent)} onChange={(checked) => updateSelected({ keyPointContent: checked ? "Add the key point…" : "" })} />{selected.keyPointContent ? <AdminFormField label="Key Point Content"><textarea className="control min-h-20 w-full py-3" value={selected.keyPointContent} onChange={(event) => updateSelected({ keyPointContent: event.target.value })} /></AdminFormField> : null}{error ? <p role="alert" className="crew-sop-editor-error">{error}</p> : null}</main>
      ...
      <div className="crew-sop-source-list" role="radiogroup" aria-label="Published SOPs">{loadingSource ? <p>Loading published SOPs…</p> : visibleSources.map((sop) => <button key={sop.id} type="button" role="radio" aria-checked={sourceSopId === sop.id} className={sourceSopId === sop.id ? "is-selected" : ""} onClick={() => setSourceSopId(sop.id)}><span><strong>{sop.title}</strong><small>{sop.category || "Other"} · Published v{currentVersion(sop)?.version} · {sourceOutlet?.name || "Source outlet"} · Updated {formatDate(sop.updated_at)}</small></span><Check size={16} aria-hidden="true" /></button>)}{!loadingSource && !visibleSources.length ? <p>No published SOPs match this selection.</p> : null}</div>
      <p className="crew-sop-clone-note">Sections, safe media, and category are copied into a new outlet-scoped draft. Future edits never change the source SOP.</p>{error ? <p role="alert" className="crew-sop-editor-error">{error}</p> : null}
    </div>}
  </Modal>;
}
