...
export default function CrewMobileApp({ onNotify }) {
  ...
  const homeTasks = (operations?.tasks || []).map((row) => {
    ...
    const deadline = row.due_at ? t("tasks.dueAt", { time: formatTime(row.due_at) }) : null;
