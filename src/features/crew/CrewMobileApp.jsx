...
import CrewAvailabilityMobile from "./components/CrewAvailabilityMobile.jsx";
...
export default function CrewMobileApp() {
  ...
  async function refresh(token = session?.token) {
    if (!token) return;
    setPageLoading(true);
    const [historyResult, contextResult, learningResult, growthResult, performanceResult, rewardResult, operationsResult, rosterResult, leaveResult, shiftRequestsResult] = await Promise.allSettled([
      ...
      crewService.shiftRequests(token),
    ]);
    ...
    setShiftRequests(shiftRequestsResult.status === "fulfilled" ? shiftRequestsResult.value : { requests: [], incoming: [], available_shifts: [] });
    setPageLoading(false);
  }
  ...
  return <main className="crew-v2-shell"><section className="crew-v2-app">
    {screen === "home" && <section className="crew-v2-home">
      ...
      {(shiftRequests?.available_shifts || []).some((item) => item.eligibility?.eligible) ? <section className="crew-v2-home-section"><CrewActionRow icon={Repeat2} tone="amber" title="Available Shift" subtitle={`${(shiftRequests.available_shifts || []).filter((item) => item.eligibility?.eligible).length} eligible cover opportunit${(shiftRequests.available_shifts || []).filter((item) => item.eligibility?.eligible).length === 1 ? "y" : "ies"}`} meta="View" onClick={() => setScreen("shiftRequests")} /></section> : null}
    ...
    <CrewBottomNav items={navItems} active={screen === "operations" || screen === "attendance" || screen === "schedule" ? "home" : ["leave", "availability", "shiftRequests", "shiftSwap"].includes(screen) ? "me" : screen} onChange={(next) => { setScreen(next); if (next === "me") setMeView("main"); }} />
  </section></main>;
}
