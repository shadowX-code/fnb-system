...
const eligibleFamily = { id: "family-eligible", name_en: "New Sauce", status: "active" };
...
describe("FactoryProductionSopPage smoke", () => {
  ...
  it("only offers Finished Goods without any SOP family when creating a first SOP", () => {
    renderPage(["factory_production_sop.view", "factory_production_sop.create"]);
    fireEvent.click(screen.getByRole("button", { name: "Create SOP" }));
    fireEvent.click(screen.getByRole("button", { name: "Finished Good *" }));
    expect(screen.getByText("New Sauce")).not.toBeNull();
    expect(screen.getAllByText("Sambal")).toHaveLength(1);
  });

  it("explains the empty first-SOP eligibility state before opening the builder", () => {
    renderPage(["factory_production_sop.view", "factory_production_sop.create"], { productFamilies: [family] });
    fireEvent.click(screen.getByRole("button", { name: "Create SOP" }));
    expect(screen.getAllByText("All Finished Goods already have an SOP. Create a new version from an existing SOP instead.")).toHaveLength(2);
  });
});
