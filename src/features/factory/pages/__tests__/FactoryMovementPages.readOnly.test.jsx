...
vi.mock("../../hooks/useProductMovementsQuery.js", () => ({ default: () => ({ filters: { dateFrom: "", dateTo: "", search: "", category: "", movementType: "" }, listing: { rows: [
...
describe("Movement ledger read-only presentation", () => {
  it("executes Receiving, Production Usage, and Stock Check row renderers without mutation controls", () => {
    ...
    expect(screen.getAllByRole("button", { name: "RM-CHI-260809-01" })).toHaveLength(3);
  ...
  it("executes Production, Dispatch, and Stock Check Product Movement row renderers without mutation controls", () => {
    ...
    expect(screen.getByPlaceholderText("Product, SKU, batch or source")).not.toBeNull();
    expect(screen.queryByPlaceholderText("Search batch or source")).toBeNull();
    expect(screen.queryByRole("button", { name: /create|edit|delete/i })).toBeNull();
  });
});
