...
export default function FactoryBatchTraceabilityPage({ onNotify }) {
  ...
  const batchTypeOptions = [{ value: "", label: "All batch types" }, { value: "production", label: "Production" }, { value: "adjustment", label: "Adjustment" }, { value: "legacy_unallocated", label: "Legacy / unallocated" }];
  const expiryOptions = [{ value: "", label: "All expiry states" }, { value: "expired", label: "Expired" }, { value: "expiring_30", label: "Expiring in 30 days" }, { value: "valid", label: "Valid beyond 30 days" }, { value: "no_expiry", label: "No expiry recorded" }];
  const reconciliationOptions = [{ value: "", label: "All statuses" }, { value: "reconciled", label: "Reconciled" }, { value: "legacy_unallocated", label: "Legacy / unallocated" }, { value: "review_required", label: "Review required" }, { value: "mismatch", label: "Mismatch" }];
  ...
  const activeFilters = useMemo(() => {
    ...
    if (filters.batchNo) filtersForDisplay.push({ key: "batchNo", label: "Batch no.", value: filters.batchNo, onRemove: () => updateFilters({ batchNo: "" }) });
  ...
  const columns = [
    ...
    { key: "action", label: "Actions", className: "sticky right-0 w-14 bg-white shadow-[-1px_0_0_#dce3e8]", align: "right", render: (row) => <FactoryRowAction onClick={() => loadDetail(row)} /> },
  ];
  return <div className="space-y-5">
    ...
    <FactoryFilterBar
      ...
      moreFilters={<><Field label="Packaging SKU"><SearchableSelect value={filters.finishedGood} options={[{ value: "", label: "All packaging SKUs" }, ...finishedGoodOptions]} placeholder="All packaging SKUs" onChange={(finishedGood) => updateFilters({ finishedGood })} /></Field><Field label="Batch type"><SearchableSelect value={filters.batchType} options={batchTypeOptions} placeholder="All batch types" onChange={(batchType) => updateFilters({ batchType })} /></Field><Field label="Storage location"><SearchableSelect value={filters.storageLocation} options={[{ value: "", label: "All storage locations" }, ...storageOptions]} placeholder="All storage locations" onChange={(storageLocation) => updateFilters({ storageLocation })} /></Field><Field label="Expiry status"><SearchableSelect value={filters.expiryStatus} options={expiryOptions} placeholder="All expiry states" onChange={(expiryStatus) => updateFilters({ expiryStatus })} /></Field><Field label="Batch no."><input className={inputClass()} value={filters.batchNo} onChange={(event) => updateFilters({ batchNo: event.target.value })} placeholder="Search batch number" /></Field></>}
    >
      ...
      <Field label="Date"><div className="grid grid-cols-2 gap-2"><FeedXDatePicker value={filters.dateFrom} placeholder="From" onChange={(dateFrom) => updateFilters({ dateFrom })} /><FeedXDatePicker value={filters.dateTo} placeholder="To" onChange={(dateTo) => updateFilters({ dateTo })} /></div></Field>
      <Field label="Status"><SearchableSelect value={filters.reconciliationStatus} options={reconciliationOptions} placeholder="All statuses" onChange={(reconciliationStatus) => updateFilters({ reconciliationStatus })} /></Field>
    </FactoryFilterBar>
    <div className="overflow-hidden rounded-xl border border-border bg-white"><FactoryTableLoadState state={listing} label="Batch Traceability" onRetry={retry} permissionMessage="Batch traceability is hidden by your current role." /><FactoryTable columns={columns} rows={rows} loading={listing.loading} density="compact" headerStyle="sentence" rowHover="mint" emptyTitle="No records match these filters" emptyDescription="Clear filters to view authoritative batch records." />{listing.hasLoaded ? <FactoryPagination page={listing.loadedPage} pageSize={listing.loadedPageSize} total={listing.loadedTotal} loading={listing.loading} noun="" onPageChange={requestPage} onPageSizeChange={requestPageSize} /> : null}</div>
