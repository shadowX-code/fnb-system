...
export function PettyCashCategoryModal({ categories, onClose, onSave }) {
  ...
  const columns = [
    { key: "name", label: "Category", render: (row) => <FactoryCellText primary={row.name} /> },
    { key: "description", label: "Description", render: (row) => row.description ? <span className="text-sm text-text-secondary">{row.description}</span> : <FactoryCellMuted /> },
