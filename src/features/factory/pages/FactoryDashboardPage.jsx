...
import FactoryProductionIntelligenceRanking from "../components/dashboard/FactoryProductionIntelligenceRanking.jsx";
...
import { canonicalAdminUrlForLegacyPath } from "../../../app/routeOwnership.js";
...
export default function FactoryDashboardPage({ onRefreshFactoryData }) {
  ...
  const dashboardActions = <button className="btn-secondary" type="button" disabled={dashboardAnalytics.loading} onClick={() => Promise.all([onRefreshFactoryData?.(), loadDashboardAnalytics()])}><RefreshCw size={15} className={dashboardAnalytics.loading ? "animate-spin" : ""} /> Refresh</button>;
    ...
    const plannedOutputByProductUom = new Map(plannedRows.map((row) => [
      `${row.finished_good_id || "legacy"}-${row.uom}`,
      Number(row.planned_qty || 0),
    ]));
    const productionRankingRows = productionRows.map((row) => ({
      ...row,
      target_qty: plannedOutputByProductUom.get(`${row.finished_good_id || "legacy"}-${row.uom}`) || null,
    }));
      ...
      <div className="space-y-5">
        ...
        {dashboardAnalytics.hasLoaded ? (
          <>
            ...
            {permissions.production ? <FactoryProductionIntelligenceRanking rows={productionRankingRows} metric={dashboardProductionMeasure} onMetricChange={setDashboardProductionMeasure} /> : null}
            ...
            <div className="grid gap-4 xl:grid-cols-3">
              {(permissions.production || permissions.dispatch) ? <Card className="xl:col-span-2" title="Production vs Dispatch Trend" description="Separate UOM-safe series for the six months ending in the selected month.">
                <div className="grid gap-4 p-4 lg:grid-cols-2">
                  {permissions.production ? <div><div className="mb-3 flex flex-wrap items-start justify-between gap-3"><div><div className="text-sm font-black text-text-primary">Production Output ({selectedProductionUom || "UOM"})</div><div className="text-xs text-text-secondary">Selected month: {quantity(productionTrendSummary.current, selectedProductionUom)}</div></div><div className="flex items-center gap-2"><FactoryDashboardUomSelect uoms={productionUoms} value={selectedProductionUom} onChange={setDashboardProductionUom} ariaLabel="Production trend UOM" /><Badge tone={productionTrendSummary.change !== null && productionTrendSummary.change < 0 ? "warning" : "neutral"}>{productionTrendSummary.change === null ? "MoM —" : `${productionTrendSummary.change >= 0 ? "+" : ""}${productionTrendSummary.change.toFixed(1)}% MoM`}</Badge></div></div>{hasProductionTrendActivity ? <div className="h-[230px]" role="img" aria-label={`Six month Production output trend in ${selectedProductionUom}`}><ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1} initialDimension={{ width: 1, height: 1 }}><LineChart data={productionTrend}><CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" /><XAxis dataKey="month" tick={{ fontSize: 11 }} /><YAxis tick={{ fontSize: 11 }} /><Tooltip contentStyle={chartTooltipStyle} formatter={(value) => [quantity(value, selectedProductionUom), "Production"]} /><Line type="monotone" dataKey="quantity" stroke="#167d5a" strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 5 }} /></LineChart></ResponsiveContainer></div> : <EmptyState title="No production activity for this period." description="Completed Production output will appear when activity is recorded." />}</div> : null}
