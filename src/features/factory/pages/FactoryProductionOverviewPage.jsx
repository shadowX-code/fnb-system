...
function activityDate(dateValue, timeValue) {
  ...
  return { sortValue: 0, dateLabel: "—", timeLabel: "—" };
}
...
export default function FactoryProductionOverviewPage({ route = "production-overview", auth, openJob, startJob, completeProduction, viewCompletedResult, releaseJob, cancelJob }) {
  ...
  const columns = [
    { key: "scheduled", title: "Schedule", helper: "Scheduled for future production", badge: "neutral", rail: "bg-slate-400 dark:bg-slate-500", headerTint: "bg-slate-500/5 dark:bg-slate-400/5" },
    { key: "released", title: "Released", helper: "Ready to start", badge: "info", rail: "bg-blue-500", headerTint: "bg-blue-500/5 dark:bg-blue-400/8" },
    { key: "in_progress", title: "In Progress", helper: "Currently running", badge: "warning", rail: "bg-amber-500", headerTint: "bg-amber-500/8 dark:bg-amber-400/10", active: true },
    { key: "completed", title: "Completed Today", helper: "Finished today", badge: "success", rail: "bg-emerald-500", headerTint: "bg-emerald-500/5 dark:bg-emerald-400/8" },
  ...
  const activityColumns = [
    ...
    { key: "result", label: "Result", render: (row) => <FactoryStatusBadge status={row.result} tone={row.tone} variant="emphasized">{row.result}</FactoryStatusBadge> },
  ];
