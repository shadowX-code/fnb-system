...
export default function FactoryMestiCleaningPage({ auth, onNotify }) {
  ...
  const monthlyRowKey = (row) => row.requirement_id || row.logical_requirement_id;
  const monthlyCells = new Map(monthlyRows.flatMap((row) => (row.days || []).map((cell) => [`${monthlyRowKey(row)}:${cell.due_date}`, cell])));
