...
import FactoryWorkspaceRoutes from "./FactoryWorkspaceRoutes.jsx";
...
import { beginFactoryRouteTrace, markFactoryRouteMeaningfulContent } from "../utils/factoryRuntimePerformance.js";
...
export default function FactoryWorkspaceRuntime({ initialTab = "dashboard", ui, auth }) {
  ...
  const markRouteReady = useCallback((route) => markFactoryRouteMeaningfulContent(route, "factory-route"), []);
  ...
  useEffect(() => {
    beginFactoryRouteTrace(initialTab);
    loadData();
  ...
  return (
    <FactoryPermissionsProvider permissionSet={auth?.permissions || []} can={can}>
      ...
      <FactoryOperationalJobsProvider route={initialTab} auth={auth} refreshKey={data} onPermissionDenied={clearOperationalPermission}>{(operationalJobs) => <>
        ...
        <FactoryWorkspaceRoutes
          initialTab={initialTab}
          auth={auth}
          data={data}
          ui={ui}
          operationalJobs={operationalJobs}
          onReady={markRouteReady}
          legacy={{
            can,
            loadData,
            clearPlanningPermission,
            clearJobOrdersListingPermission,
            jobOrdersListingBridge,
            jobFinishedGoodName,
            productionQcTone,
            productionQcDisplayLabel,
            openEquipment: () => setModal({ type: "equipment" }),
            editEquipment: (value) => setModal({ type: "equipment", value }),
            openEquipmentCategories: () => setModal({ type: "equipment-categories" }),
            openJob: (job, options) => setModal({ type: "job", value: job, readOnly: options?.readOnly }),
            createJob: () => setModal({ type: "job" }),
            viewJob: (job) => setModal({ type: "job", value: job, readOnly: true }),
            editJob: (job) => setModal({ type: "job", value: job, readOnly: false }),
            releaseJob: releaseJobOrder,
            deleteJob: deleteJobOrder,
            cancelJob: cancelJobOrder,
            startJob: (job) => setModal({ type: "start-production", job }),
            viewProcess: (job, readOnly) => setModal({ type: "production-process", job, readOnly }),
            completeJob: (job) => setModal({ type: "production", job }),
            completeProduction: (job, options) => setModal(options?.processOnly ? { type: "production-process", job, readOnly: Boolean(options.readOnly) } : { type: "production", job }),
            viewCompletedResult: viewCompletedJobOrder,
            openRawMaterialMovementDetail: (value) => setModal({ type: "raw-material-movement-detail", value }),
            closeRawMaterialMovementDetail: () => setModal((current) => current?.type === "raw-material-movement-detail" ? null : current),
            renderRawReceiving,
            renderRawStockCheck,
            renderProduction,
            renderReports,
            renderFinishedGoodsDispatch,
            renderProductStockCheck,
          }}
        />
