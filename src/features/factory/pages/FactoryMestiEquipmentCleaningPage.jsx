...
import { Check, ClipboardCheck, Plus, Search, XCircle } from "lucide-react";
...
const monthStatusOptions = [
  { value: "", label: "All" }, { value: "pending", label: "Pending" }, { value: "completed", label: "Awaiting Verification" },
  { value: "verified", label: "Verified" }, { value: "unsatisfactory", label: "Unsatisfactory" }, { value: "missed", label: "Missed" },
];
const inputClass = factoryInputClass();
...
function equipmentPrimary(equipment) { return equipment?.equipment_name || equipment?.name || equipment?.equipment_code || "Equipment"; }
function equipmentSecondary(equipment) {
  return [equipment?.location_name || equipment?.location?.location_name, equipment?.equipment_code].filter(Boolean).join(" · ") || "No location or equipment code";
}
...
function OccurrenceDetail({ occurrence, onClose }) {
  ...
  return <Modal title={`${equipmentPrimary(occurrence)} · ${occurrence.task_name}`} description={equipmentSecondary(occurrence)} onClose={onClose} size="lg">
    <div className="grid gap-4 text-sm md:grid-cols-2">
      ...
      <div><div className="text-xs font-semibold text-text-muted">Equipment</div><div className="mt-1 font-bold text-text-primary">{equipmentPrimary(occurrence)}</div><div className="text-xs text-text-secondary">{equipmentSecondary(occurrence)}</div></div>
      ...
      {occurrence.source_type === "after_production" ? <div className="md:col-span-2"><div className="text-xs font-semibold text-text-muted">Production Evidence</div><div className="mt-1 font-bold text-text-primary">{productionLabel(occurrence)}</div><div className="text-xs text-text-secondary">{occurrence.production_snapshot?.production_no || ""}{occurrence.production_snapshot?.sop_version ? ` · SOP ${occurrence.production_snapshot.sop_version}` : ""}{occurrence.production_snapshot?.production_end_date ? ` · Production end ${formatFactoryDate(occurrence.production_snapshot.production_end_date)}` : ""}{occurrence.production_snapshot?.completed_at ? ` · Recorded ${formatFactoryDateTime(occurrence.production_snapshot.completed_at)}` : ""}</div></div> : null}
...
function MonthlyCellDetail({ cell, equipment, onClose, onOpenOccurrence, occurrenceActions }) {
  if (!cell || !equipment) return null;
  return <Modal title={`${equipmentPrimary(equipment)} · ${formatFactoryDate(cell.due_date)}`} description={equipmentSecondary(equipment)} onClose={onClose} size="lg">
    <div className="divide-y divide-border">
      {(cell.occurrences || []).map((occurrence) => <div key={occurrence.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
        ...
        <div className="flex shrink-0 items-center gap-2"><StatusBadge status={occurrence.status} /><FactoryRowActions onView={() => onOpenOccurrence(occurrence)} viewLabel={`View ${occurrence.task_name} details`} {...occurrenceActions(occurrence)} /></div>
      </div>)}
    </div>
  </Modal>;
}
...
export default function FactoryMestiEquipmentCleaningPage({ onNotify }) {
  ...
  async function act(action, occurrence, result) {
    try {
      ...
      const refreshedDaily = await loadDaily();
      const refreshedMonthly = await loadMonthly();
      setDetail((current) => refreshedDaily?.find((item) => item.id === current?.id) || current);
      setMonthlyDetail((current) => {
        if (!current) return current;
        const equipment = refreshedMonthly?.find((item) => item.equipment_id === current.equipment.equipment_id);
        const cell = equipment?.days?.find((item) => item.due_date === current.cell.due_date);
        return equipment && cell ? { equipment, cell } : current;
      });
  ...
  function occurrenceActions(row) {
    const canComplete = (row.status === "pending" || row.status === "missed" || row.status === "unsatisfactory") && (can("factory_mesti_equipment_cleaning.complete") || canManage);
    const canReview = row.status === "completed" && (can("factory_mesti_equipment_cleaning.review") || canManage);
    return {
      primaryAction: canComplete ? { label: "Complete", onClick: () => act("complete", row) } : canReview ? { label: "Verify", onClick: () => act("verify", row, "verified") } : null,
      secondaryActions: canReview ? [{ label: "Mark unsatisfactory", destructive: true, onClick: () => act("verify", row, "unsatisfactory") }] : [],
    };
  }
  ...
  return <div className="space-y-5">
    <PageHeader section="MeSTI" title="Cleaning of Equipment" description="Scheduled and after-Production equipment cleaning evidence." />
    <FactoryViewTabs value={activeTab} onChange={setActiveTab} tabs={[{ value: "daily", label: "Daily" }, { value: "monthly", label: "Monthly" }, { value: "setup", label: "Setup" }]} />
    {error ? <div className="rounded-lg border border-rose-200 bg-rose-50 p-3 text-sm font-semibold text-rose-700">{error}</div> : null}
    {activeTab === "daily" ? <><FactoryOperationalSummary items={[{ label: "Due", value: summary.due }, { label: "Pending", value: summary.pending, tone: "warning" }, { label: "Awaiting Verification", value: summary.awaiting, tone: "info" }, { label: "Issues", value: summary.issues, tone: summary.issues ? "danger" : "neutral" }]} /><FactoryDailyToolbar><FactoryDailyDateField><FeedXDatePicker value={date} onChange={setDate} /></FactoryDailyDateField></FactoryDailyToolbar>{loading ? <div className="py-12 text-center text-sm font-semibold text-text-secondary">Loading Cleaning of Equipment...</div> : groups.length ? <FactoryDataSurface>{groups.map((group) => <FactoryOperationalGroup key={group.key} title={equipmentPrimary(group)} description={equipmentSecondary(group)} count={`${group.rows.length} obligation${group.rows.length === 1 ? "" : "s"}`}>{group.rows.map((row) => <FactoryOperationalRow key={row.id} primary={row.task_name} secondary={`${recurrenceLabel(row)}${row.source_type === "after_production" ? ` · ${productionLabel(row)}` : ""}`} evidence={<FactoryOperationalEvidence items={[row.completed_at ? { key: "completed", label: `Completed ${row.completed_by_name || "—"}` } : null, row.verified_at ? { key: "verified", label: `Verified ${row.verified_by_name || "—"}` } : null]} />} status={<StatusBadge status={row.status} variant="emphasized" />} actions={<FactoryRowActions onView={() => setDetail(row)} viewLabel={`View ${row.task_name} details`} {...occurrenceActions(row)} />} />)}</FactoryOperationalGroup>)}</FactoryDataSurface> : <EmptyState title="No equipment cleaning due" description="" />}</> : null}
    {activeTab === "monthly" ? <><FactoryDailyToolbar><div className="w-full sm:w-[170px]"><Field label="Month"><FactoryMonthPicker value={month} onChange={setMonth} /></Field></div><div className="w-full sm:w-[260px]"><Field label="Equipment"><div className="relative"><Search size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" /><input className={`${inputClass} pl-9`} value={monthFilters.query} placeholder="Search equipment" onChange={(event) => setMonthFilters((current) => ({ ...current, query: event.target.value }))} /></div></Field></div><div className="w-full sm:w-[220px]"><Field label="Location"><SearchableSelect value={monthFilters.location} options={[{ value: "", label: "All" }, ...monthLocations.map((location) => ({ value: location, label: location }))]} placeholder="All" onChange={(location) => setMonthFilters((current) => ({ ...current, location }))} /></Field></div><div className="w-full sm:w-[200px]"><Field label="Status"><SearchableSelect value={monthFilters.status} options={monthStatusOptions} placeholder="All" onChange={(status) => setMonthFilters((current) => ({ ...current, status }))} /></Field></div></FactoryDailyToolbar>{loading ? <div className="py-12 text-center text-sm font-semibold text-text-secondary">Loading monthly compliance...</div> : <FactoryDataSurface><FactoryComplianceMatrix rows={visibleMonthlyRows} days={monthDays(month)} rowKey={(row) => row.equipment_id} entityLabel="Equipment" frequencyLabel="Summary" renderEntity={equipmentPrimary} renderSecondary={equipmentSecondary} renderFrequency={(row) => `${row.summary?.total_count || 0} cleanings · ${row.summary?.verified_count || 0} verified · ${row.summary?.pending_count || 0} pending`} getCell={(row, day) => row.days?.find((cell) => cell.due_date === day) || null} cellLabel={cellLabel} cellTitle={(cell, row) => `${equipmentPrimary(row)} on ${formatFactoryDate(cell.due_date)}: ${cell.total_count} obligations; ${cell.verified_count} verified, ${cell.completed_count} awaiting verification, ${cell.pending_count} pending`} onCellClick={(cell, row) => setMonthlyDetail({ equipment: row, cell })} empty={<EmptyState title="No monthly cleaning occurrences" description="" />} /></FactoryDataSurface>}</> : null}
    {activeTab === "setup" ? <><div className="flex justify-end">{canSaveSetup ? <button className="btn-primary" type="button" onClick={() => { setEquipmentQuery(""); setRequirementDraft(emptyRequirementDraft()); setShowRequirementForm(true); }}><Plus size={16} />Create Requirement</button> : null}</div><FactoryDataSurface><FactoryTable rows={requirements} emptyTitle="No scheduled cleaning requirements" columns={[{ key: "task", label: "Task", render: (row) => <div className="font-bold text-text-primary">{row.task_name}</div> }, { key: "equipment", label: "Equipment", render: (row) => <span className="font-semibold text-text-secondary" title={(row.equipment_names || []).join(", ")}>{(row.equipment_ids || []).length} equipment{(row.equipment_ids || []).length === 1 ? "" : "s"}</span> }, { key: "recurrence", label: "Frequency", render: (row) => recurrenceLabel(row).replace("Scheduled · ", "") }, { key: "effective", label: "Effective From", render: (row) => formatFactoryDate(row.effective_from) }, { key: "status", label: "Status", render: (row) => <StatusBadge status={row.status} /> }, { key: "actions", label: "Actions", align: "right", render: (row) => canSaveSetup ? <FactoryRowActions directSingleSecondary secondaryActions={[{ label: "Edit", onClick: () => editRequirement(row) }]} /> : null }]} /></FactoryDataSurface></> : null}
    {showRequirementForm ? <Modal title={requirementDraft.id ? "Edit Cleaning Requirement" : "Create Cleaning Requirement"} onClose={() => setShowRequirementForm(false)} size="lg" footer={<><button className="btn-secondary" type="button" onClick={() => setShowRequirementForm(false)}>Cancel</button><button className="btn-primary" type="submit" form="equipment-cleaning-requirement">{requirementDraft.id ? "Save Changes" : "Create Requirement"}</button></>}><form id="equipment-cleaning-requirement" className="space-y-5" onSubmit={saveRequirement}><section className="space-y-3"><h3 className="text-sm font-bold text-text-primary">Cleaning Rule</h3><div className="grid gap-3 md:grid-cols-2"><Field label="Task Name"><input className={inputClass} value={requirementDraft.task_name} onChange={(event) => setRequirementDraft((current) => ({ ...current, task_name: event.target.value }))} required /></Field><Field label="Effective From"><FeedXDatePicker value={requirementDraft.effective_from} onChange={(value) => setRequirementDraft((current) => ({ ...current, effective_from: value }))} /></Field><Field label="Frequency"><select className={inputClass} value={requirementDraft.recurrence_type} onChange={(event) => setRequirementDraft((current) => ({ ...current, recurrence_type: event.target.value }))}><option value="daily">Daily</option><option value="weekly">Weekly</option></select></Field>{requirementDraft.id ? <Field label="Status"><select className={inputClass} value={requirementDraft.status} onChange={(event) => setRequirementDraft((current) => ({ ...current, status: event.target.value }))}><option value="active">Active</option><option value="inactive">Inactive</option></select></Field> : null}</div>{requirementDraft.recurrence_type === "weekly" ? <div><div className="mb-2 text-sm font-bold text-text-primary">Weekdays</div><div className="flex flex-wrap gap-2">{weekdays.map((day) => <label key={day.value} className="inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm font-semibold"><input type="checkbox" checked={requirementDraft.recurrence_weekdays.includes(day.value)} onChange={(event) => setRequirementDraft((current) => ({ ...current, recurrence_weekdays: event.target.checked ? [...new Set([...current.recurrence_weekdays, day.value])] : current.recurrence_weekdays.filter((value) => value !== day.value) }))} />{day.label}</label>)}</div></div> : null}</section><section className="border-t border-border pt-4"><div className="flex flex-wrap items-baseline justify-between gap-2"><h3 className="text-sm font-bold text-text-primary">Applies to Equipment</h3><span className="text-xs font-semibold text-text-secondary">{requirementDraft.equipment_ids.length} equipment selected</span></div><div className="relative mt-3"><Search size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" /><input className={`${inputClass} pl-9`} value={equipmentQuery} onChange={(event) => setEquipmentQuery(event.target.value)} placeholder="Search equipment, category, or location" /></div><div className="mt-3 max-h-72 divide-y divide-border overflow-y-auto rounded-lg border border-border">{visibleEquipment.map((equipment) => <label key={equipment.id} className="flex cursor-pointer items-center gap-3 px-3 py-2.5 hover:bg-slate-50"><input aria-label={`${equipmentPrimary(equipment)} ${equipment.equipment_code || ""}`.trim()} type="checkbox" checked={requirementDraft.equipment_ids.includes(equipment.id)} onChange={(event) => toggleEquipment(equipment.id, event.target.checked)} /><span className="min-w-0"><span className="block font-bold text-text-primary">{equipmentPrimary(equipment)}</span><span className="block text-xs text-text-secondary">{equipmentSecondary(equipment)}</span></span></label>)}{!visibleEquipment.length ? <div className="px-3 py-5 text-sm font-semibold text-text-secondary">No matching active Equipment.</div> : null}</div></section></form></Modal> : null}
    <MonthlyCellDetail cell={monthlyDetail?.cell} equipment={monthlyDetail?.equipment} onClose={() => setMonthlyDetail(null)} onOpenOccurrence={(occurrence) => { setMonthlyDetail(null); setDetail(occurrence); }} occurrenceActions={occurrenceActions} />
