...
function recurrenceLabel(row) {
  if (row.trigger_type === "after_operation") return "After operation";
...
function emptyRequirementDraft() {
  return { task_name: "", equipment_ids: [], trigger_type: "scheduled", recurrence_type: "daily", recurrence_weekdays: [1], status: "active", effective_from: malaysiaBusinessDateInput() };
}
...
function monthlyCellLabel(cell) {
  if (cell.status === "mixed" || cell.trigger_type === "after_operation") return `${cell.verified_count}/${cell.total_count}`;
...
function OccurrenceDetail({ occurrence, onClose }) {
  ...
  return <Modal title={`${occurrence.equipment_code} · ${occurrence.task_name}`} onClose={onClose} size="lg">
    <div className="grid gap-4 p-5 text-sm md:grid-cols-2">
      ...
      <div><div className="text-xs font-semibold text-text-muted">Trigger</div><div className="mt-1 font-bold text-text-primary">{recurrenceLabel(occurrence)}</div></div>
      ...
      {occurrence.production_equipment_usage_id ? <div className="md:col-span-2"><div className="text-xs font-semibold text-text-muted">Production Evidence</div><div className="mt-1 font-bold text-text-primary">{productionLabel(occurrence)}</div><div className="text-xs text-text-secondary">{occurrence.usage_completed_at ? formatFactoryDateTime(occurrence.usage_completed_at) : ""}</div></div> : null}
  ...
  return <div className="space-y-5">
    ...
    {activeTab === "daily" ? <>
      ...
      {loading ? <div className="py-12 text-center text-sm font-semibold text-text-secondary">Loading Cleaning of Equipment...</div> : groups.length ? groups.map((group) => <Card key={group.key} title={[group.equipment_code, group.equipment_name].filter(Boolean).join(" · ")} description={group.location_name}>
        <FactoryTable rows={group.rows} columns={[
          { key: "task", label: "Task", render: (row) => <div><div className="font-bold text-text-primary">{row.task_name}</div><div className="text-xs text-text-secondary">{recurrenceLabel(row)}{row.production_equipment_usage_id ? ` · ${productionLabel(row)}` : ""}</div></div> },
    ...
    {activeTab === "setup" ? <>
      ...
      <Card title="Cleaning Requirements" description=""><FactoryTable rows={requirements} columns={[
        ...
        { key: "trigger", label: "Trigger", render: (row) => recurrenceLabel(row) },
    ...
    {showRequirementForm ? <Modal title={requirementDraft.id ? "Edit Cleaning Requirement" : "Create Cleaning Requirement"} onClose={() => setShowRequirementForm(false)} size="xl"><form className="space-y-5 p-5" onSubmit={saveRequirement}>
      ...
      <Field label="Trigger Type"><select className={inputClass} value={requirementDraft.trigger_type} onChange={(event) => setRequirementDraft((current) => ({ ...current, trigger_type: event.target.value, recurrence_type: event.target.value === "after_operation" ? "" : (current.recurrence_type || "daily"), recurrence_weekdays: event.target.value === "after_operation" ? [] : current.recurrence_weekdays }))}><option value="scheduled">Scheduled</option><option value="after_operation">After Operation</option></select></Field>
      {requirementDraft.trigger_type === "scheduled" ? <><Field label="Frequency"><select className={inputClass} value={requirementDraft.recurrence_type} onChange={(event) => setRequirementDraft((current) => ({ ...current, recurrence_type: event.target.value }))}><option value="daily">Daily</option><option value="weekly">Weekly</option></select></Field>{requirementDraft.recurrence_type === "weekly" ? <div><div className="mb-2 text-sm font-bold text-text-primary">Weekdays</div><div className="flex flex-wrap gap-2">{weekdays.map((day) => <label key={day.value} className="inline-flex items-center gap-2 rounded border border-border px-3 py-2 text-sm font-semibold"><input type="checkbox" checked={requirementDraft.recurrence_weekdays.includes(day.value)} onChange={(event) => setRequirementDraft((current) => ({ ...current, recurrence_weekdays: event.target.checked ? [...new Set([...current.recurrence_weekdays, day.value])] : current.recurrence_weekdays.filter((value) => value !== day.value) }))} />{day.label}</label>)}</div></div> : null}</> : null}
