...
export default function useFactoryRawMaterialInventoryQuery({ filters }) {
  ...
  return useMemo(() => {
    ...
    const allRows = rawMaterials.map((material) => {
      const materialReceivings = receivings.filter((row) => row.raw_material_id === material.id);
      const movements = rawMaterialMovements.filter((row) => row.raw_material_id === material.id);
      ...
      const latest = latestCost(receivings, material); const balance = Number(material.current_balance || 0); const converted = latest.missing ? null : convert(balance, material.uom, latest.uom);
    ...
    return { rows, allRows, categories: rawMaterialCategories, loading: false, error: "", errorKind: "", retry: () => {} };
  }, [can, filters, rawMaterialCategories, rawMaterialMovements, rawMaterials, receivings]);
}
