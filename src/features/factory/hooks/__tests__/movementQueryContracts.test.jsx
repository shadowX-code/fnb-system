...
describe("Factory extracted query contracts", () => {
  ...
  it("maps Product, Batch Traceability, and Audit filters to their existing service contracts", async () => {
    const product = renderHook(() => useProductMovementsQuery({})); act(() => product.result.current.updateFilters({ search: "PB01", dateTo: "2026-08-31" })); await mocks.captured.loadPage({ page: 1, pageSize: 20 }); expect(mocks.service.listProductMovementsPage).toHaveBeenLastCalledWith(expect.objectContaining({ page: 1, pageSize: 20, filters: expect.objectContaining({ search: "PB01", dateTo: "2026-08-31" }) }));
