...
export default function FactoryProductFeedbackPublic() {
  const [entry, setEntry] = useState(null); const [error, setError] = useState(""); const [language, setLanguage] = useState("en"); const [step, setStep] = useState(0); const [answers, setAnswers] = useState({}); const [answerDetails, setAnswerDetails] = useState({}); const [contactStep, setContactStep] = useState(false); const [contact, setContact] = useState({ name: "", mobile: "" }); const [contactError, setContactError] = useState(""); const [submitting, setSubmitting] = useState(false); const [complete, setComplete] = useState(false);
  ...
  const setAdditionalText = (optionValue, textValue) => setAnswerDetails((current) => ({ ...current, [question.key]: { ...(current[question.key] || {}), [optionValue]: textValue } }));
  const clearAdditionalDetails = (nextValue) => setAnswerDetails((current) => {
    const allowed = new Set(Array.isArray(nextValue) ? nextValue : [nextValue]);
    const details = Object.fromEntries(Object.entries(current[question.key] || {}).filter(([option]) => allowed.has(option)));
    return { ...current, [question.key]: details };
  });
  ...
  async function submit(nextContact = contact) {
    ...
    try { await factoryService.submitPublicProductFeedback({ token, answers, answerDetails, language, sessionToken, contact: { name: nextContact.name.trim(), mobile: normalizedMobile || "" } }); setComplete(true); } catch (err) { setError(err.message || "submit"); } finally { setSubmitting(false); }
  }
...
function QuestionControl({ question, language, value, details, onChange, onAdditionalText, onAutoAdvance, selectAllLabel, selectedLabel, placeholder, specifyLabel }) {
  ...
  if (question.type === "multi_choice") { const selectedCount = Array.isArray(value) ? value.length : 0; return <><p className="feedback-select-all">{selectAllLabel}{selectedCount ? ` · ${selectedCount} ${selectedLabel}` : ""}</p><div className="feedback-multi-options">{options.map((option) => { const selected = Array.isArray(value) && value.includes(option.value); return <div key={option.value} className="feedback-option-detail"><button type="button" role="checkbox" aria-checked={selected} className={selected ? "selected" : ""} onClick={() => onChange(selected ? value.filter((item) => item !== option.value) : [...(value || []), option.value])}><span className="feedback-multi-checkbox" aria-hidden="true"><Check size={15} /></span><span>{optionLabel(option, language)}</span></button>{selected && option.allow_additional_text ? <input aria-label={specifyLabel} value={details?.[option.value] || ""} placeholder={specifyLabel} onChange={(event) => onAdditionalText(option.value, event.target.value)} /> : null}</div>; })}</div></>; }
  return <div className={question.type === "price_choice" ? "feedback-price-options" : "feedback-choice-options"}>{options.map((option) => <div key={option.value} className="feedback-option-detail"><button type="button" aria-pressed={value === option.value} className={value === option.value ? "selected" : ""} onClick={() => { if (option.allow_additional_text) onChange(option.value); else onAutoAdvance(option.value); }}><span>{optionLabel(option, language)}</span><Check size={18} /></button>{value === option.value && option.allow_additional_text ? <input aria-label={specifyLabel} value={details?.[option.value] || ""} placeholder={specifyLabel} onChange={(event) => onAdditionalText(option.value, event.target.value)} /> : null}</div>)}</div>;
}
