...
function hasMeaningfulStepDescription(step) {
  const description = String(step?.description || "").trim();
  const stepName = String(step?.step_name || step?.process_name || "").trim();
  const normalize = (text) => String(text || "").toLocaleLowerCase().replace(/[^\p{L}\p{N}]+/gu, " ").trim();
  return Boolean(description) && normalize(description) !== normalize(stepName);
}
            ...
            {steps.length ? steps.map((step) => {
              ...
              return (
                <article key={step.id} className="relative ml-10 rounded-xl border border-border bg-white p-4 sm:ml-12 sm:p-5">
                  ...
                  {hasMeaningfulStepDescription(step) ? <div className="mt-3 max-w-[75ch] text-sm font-semibold text-text-secondary">{step.description}</div> : null}
