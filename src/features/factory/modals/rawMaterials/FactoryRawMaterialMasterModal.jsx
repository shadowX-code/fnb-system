      ...
      <form id="factory-raw-material-form" className="space-y-4" onSubmit={submit}>
        ...
        <section className="space-y-3 rounded-lg border border-border bg-slate-50 p-3">
          <div className="flex items-center justify-between gap-3">
            ...
            <input aria-label="Package conversion" type="checkbox" checked={hasPackageConversion} onChange={(event) => setForm((current) => event.target.checked ? { ...current, conversion_package_uom: current.conversion_package_uom || (dimensionalFactoryUom(current.uom) ? "pack" : normalizeFactoryUom(current.uom)) || "pack", conversion_package_quantity: current.conversion_package_quantity || "", conversion_base_uom: current.conversion_base_uom || "kg" } : { ...current, conversion_package_uom: "", conversion_package_quantity: "", conversion_base_uom: "" })} />
          </div>
          {hasPackageConversion ? <div className="grid items-end gap-3 sm:grid-cols-[minmax(0,.8fr)_minmax(0,1fr)_minmax(0,.8fr)]">
            <Field label="1 Package UOM" error={fieldErrors.conversion_package_uom}><SearchableSelect value={form.conversion_package_uom || ""} options={packageOptions} placeholder="Package" searchPlaceholder="Search UOM" onChange={(conversionPackageUom) => setForm((current) => ({ ...current, conversion_package_uom: conversionPackageUom }))} /></Field>
            <Field label="Contains" error={fieldErrors.conversion_package_quantity}><input className={inputClass(fieldErrors.conversion_package_quantity)} type="number" min="0.0001" step="0.0001" value={form.conversion_package_quantity ?? ""} onChange={(event) => setForm((current) => ({ ...current, conversion_package_quantity: event.target.value }))} /></Field>
            <Field label="Base UOM" error={fieldErrors.conversion_base_uom}><SearchableSelect value={form.conversion_base_uom || ""} options={baseOptions} placeholder="Base UOM" searchPlaceholder="Search UOM" onChange={(conversionBaseUom) => setForm((current) => ({ ...current, conversion_base_uom: conversionBaseUom }))} /></Field>
          </div> : null}
        </section>
