...
export default function ProductGroupModal({ initialValue, categories = [], onClose, onSave, onArchive }) {
  ...
  return (
    <Modal
      ...
      footer={(
        <>
          {initialValue?.id && initialValue.status !== "archived" ? <button className="btn-secondary" type="button" disabled={saving} onClick={archive}>Archive Finished Good</button> : <span />}
    ...
    >
      <form id="factory-product-group-form" className="space-y-4" onSubmit={submit}>
        ...
        <section className="space-y-3 rounded-2xl border border-border bg-[var(--theme-subtle)] p-4">
          ...
          <label className="flex items-start gap-3 rounded-xl border border-border bg-[var(--theme-surface-raised)] px-3 py-3 text-sm text-text-primary">
