...
import { quantity, roundFactoryQuantity } from "../../utils/factoryFormatters.js";
...
export function allocateRawMaterialFefo(requiredQty, batches, reservedByBatch = {}) {
  let remaining = roundFactoryQuantity(requiredQty);
  const allocations = [];
  (Array.isArray(batches) ? batches : []).forEach((batch) => {
    const available = roundFactoryQuantity(Math.max(Number(batch.available_qty || 0) - Number(reservedByBatch[batch.batch_balance_id] || 0), 0));
    const allocatedQty = roundFactoryQuantity(Math.min(remaining, available));
    if (allocatedQty > 0) {
      allocations.push({ ...batch, allocated_qty: allocatedQty });
      reservedByBatch[batch.batch_balance_id] = roundFactoryQuantity(Number(reservedByBatch[batch.batch_balance_id] || 0) + allocatedQty);
      remaining = roundFactoryQuantity(remaining - allocatedQty);
    }
  });
