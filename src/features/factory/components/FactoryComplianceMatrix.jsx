...
export default function FactoryComplianceMatrix({ rows, days, getCell, renderEntity, renderSecondary, renderFrequency = (row) => row.frequency, entityLabel = "Task", frequencyLabel = "Frequency", rowKey = (row) => row.logical_requirement_id, onCellClick, cellLabel, cellTitle, empty, expandedCellKey = "", renderExpanded }) {
  if (!rows.length) return empty;
  return <div className="overflow-x-auto [container-type:inline-size]" data-testid="factory-compliance-matrix-scroll"><table className="min-w-[1120px] border-collapse text-left"><thead><tr className="border-b border-border text-[11px] font-semibold uppercase tracking-[0.06em] text-text-muted"><th className="sticky left-0 z-10 min-w-64 bg-surface px-4 py-2.5">{entityLabel}</th><th className="min-w-36 px-3 py-2.5">{frequencyLabel}</th>{days.map((day) => <th key={day} className="w-12 px-1 py-2.5 text-center">{Number(day.slice(-2))}</th>)}</tr></thead><tbody>{rows.flatMap((row) => {
    const key = rowKey(row);
    const expanded = expandedCellKey && renderExpanded ? days.map((day) => ({ cell: getCell(row, day), day })).find(({ cell, day }) => cell && `${key}:${day}` === expandedCellKey) : null;
    return [<tr key={key} className="border-b border-border last:border-b-0"><td className="sticky left-0 z-10 bg-surface px-4 py-3"><div className="text-sm font-semibold text-text-primary">{renderEntity(row)}</div>{renderSecondary ? <div className="mt-0.5 text-xs text-text-secondary">{renderSecondary(row)}</div> : null}</td><td className="whitespace-nowrap px-3 py-3 text-sm font-semibold text-text-secondary">{renderFrequency(row)}</td>{days.map((day) => { const cell = getCell(row, day); return <td key={day} className="p-1 text-center"><FactoryMatrixCell cell={cell} label={cell ? cellLabel(cell) : ""} title={cell ? cellTitle(cell, row) : "Not applicable"} onClick={cell ? () => onCellClick(cell, row) : undefined} /></td>; })}</tr>, expanded ? <tr key={`${key}:expanded`} className="border-b border-border bg-surface-muted/40"><td colSpan={days.length + 2} className="p-0"><div className="sticky left-0 box-border w-[100cqw] p-3" data-testid="factory-compliance-matrix-expansion">{renderExpanded(expanded.cell, row)}</div></td></tr> : null].filter(Boolean);
  })}</tbody></table></div>;
}
