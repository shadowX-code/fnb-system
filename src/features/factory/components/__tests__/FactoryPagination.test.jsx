...
import { useAdminPagedQuery } from "../../../../components/tables/AdminPagination.jsx";
...
describe("Admin paged query", () => {
  it("keeps the Factory entry point as a compatibility adapter", () => {
    expect(useFactoryPagedQuery).toBe(useAdminPagedQuery);
  });

  it("loads authoritative rows and summary", async () => {
    const loadPage = vi.fn().mockResolvedValue(payload([{ id: "one" }], 1));
    const { result } = renderHook(() => useAdminPagedQuery({ storageKey: "test-load", querySignature: "initial", loadPage }));
  ...
  it("accepts canonical snake-case RPC page metadata", async () => {
    ...
    const { result } = renderHook(() => useAdminPagedQuery({ storageKey: "test-snake-case", querySignature: "initial", loadPage }));
    await waitFor(() => expect(result.current[0].hasLoaded).toBe(true));
  ...
  it("never lets an older filter request overwrite the newer result", async () => {
    ...
    const { result, rerender } = renderHook(({ signature }) => useAdminPagedQuery({ storageKey: "test-stale", querySignature: signature, loadPage }), { initialProps: { signature: "old" } });
  ...
  it("keeps a sanitized transient error and replaces it after retry", async () => {
    ...
    const { result } = renderHook(() => useAdminPagedQuery({ storageKey: "test-retry", querySignature: "x", loadPage, mapError: () => ({ kind: "load", message: "Unable to load records." }) }));
  ...
  it("clears protected data on an authoritative permission denial", async () => {
    ...
    const { result } = renderHook(() => useAdminPagedQuery({ storageKey: "test-permission", querySignature: "one", loadPage, shouldClearOnError: (error) => error.code === "42501", mapError: () => ({ kind: "permission", message: "Data hidden." }) }));
