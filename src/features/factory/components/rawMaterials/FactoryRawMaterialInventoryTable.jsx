import { Archive, Check, Package, Pencil, X } from "lucide-react";
...
export default function FactoryRawMaterialInventoryTable({ rows, canEdit, materialLabel, formatQuantity, formatDate, formatCost, normalizedCostUnit, onPreviewImage, onOpenCost, onOpenDetail, onEdit, onArchive, onRestore, onSaveParLevel }) {
  ...
  const columns = [
      ...
      return (
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="text-xs text-text-secondary">{[row.material_code, row.category, secondaryNames].filter(Boolean).join(" · ") || "—"}</div>
          </div>
        </div>
      );
    } },
    ...
    { key: "status", label: "Status", render: (row) => <FactoryStatusBadge status={row.status}>{row.status === "archived" ? "Archived" : "Active"}</FactoryStatusBadge> },
    { key: "actions", label: "Actions", align: "right", render: (row) => <FactoryRowActions onView={() => onOpenDetail(row)} primaryAction={row.status === "archived" && canEdit && onRestore ? { label: "Restore", onClick: () => onRestore(row) } : null} directActions={canEdit && row.status !== "archived" ? [{ label: "Edit", icon: Pencil, onClick: () => onEdit(row) }, { label: "Archive", icon: Archive, disabled: Number(row.current_balance || 0) > 0, onClick: () => onArchive?.(row) }] : []} /> },
  ];
