...
export default function StockCheckModal({ stockType, title, initialValue, stockItems, rawMaterialCategories = [], finishedGoodCategories = [], readOnly = false, onConfirmSubmit, onClose, onSave }) {
  ...
  const [positiveBatchEditor, setPositiveBatchEditor] = useState(null);
  ...
  async function openPositiveBatchSelection(row) {
    setPositiveBatchEditor({ rowId: row.id, loading: true, error: "", batches: [] });
    try {
      const batches = await factoryService.listProductStockCheckPositiveBatches(row.finished_good_id);
      setPositiveBatchEditor((current) => current?.rowId === row.id ? { ...current, loading: false, batches } : current);
    } catch (loadError) {
      setPositiveBatchEditor((current) => current?.rowId === row.id ? { ...current, loading: false, error: loadError.message || "Unable to load batches." } : current);
    }
  }
  ...
  function countStatusControl(row, isSkipped) {
    return (
      <div className="inline-flex rounded-lg border border-border bg-white p-1">
        ...
        <button className={`rounded-md px-2 py-1 text-xs font-semibold ${isSkipped ? "bg-amber-500 text-white" : "text-text-secondary hover:bg-slate-50"}`} type="button" disabled={isLocked} onClick={() => updateRow(row.id, { count_status: "skip", physical_qty: "", batch_allocations: [], positive_adjustment_confirmed: false, positive_adjustment_batch_balance_id: "" })}>Skip</button>
      </div>
    );
  }
  ...
  function updatePhysicalCount(row, value) {
    ...
    updateRow(row.id, {
      ...
      // A positive count is a proposed reconciliation, never a fabricated Production Batch.
      positive_adjustment_confirmed: nextVariance > 0,
      positive_adjustment_batch_balance_id: nextVariance > 0 ? row.positive_adjustment_batch_balance_id || "" : "",
    });
  }
  ...
  return (
                ...
                <div key={row.id} className={`space-y-3 rounded-2xl border border-border bg-white p-3 ${showReasonError ? "ring-1 ring-amber-200" : ""}`}>
                  ...
                  {!isRaw && variance.variance !== 0 ? (
                    <div className="rounded-xl border border-border bg-slate-50 px-3 py-2 text-xs text-text-secondary">
                      ...
                      {variance.variance > 0 ? <div className="mt-1">{row.positive_adjustment_batch_balance_id ? "Existing batch selected" : "Reconciliation Batch to be created on approval"}: <span className="font-semibold text-text-primary">{rowSku?.storage_location || "Missing"}</span> · +{quantity(variance.variance, row.uom)}</div> : null}
                  ...
                  <div className="space-y-2">
                    ...
                    {!isRaw && variance.variance > 0 ? <div className="rounded-lg border border-primary/15 bg-primary/5 px-3 py-2 text-xs text-text-secondary"><div className="font-semibold text-text-primary">{row.positive_adjustment_batch_balance_id ? "Existing batch selected" : "Reconciliation Batch"}</div><div className="mt-0.5">{row.positive_adjustment_batch_balance_id ? "The approved variance will be posted to the selected batch." : "Created only on approval; inventory is unchanged while Draft or Submitted."}</div></div> : null}
                    {!isRaw && variance.variance > 0 ? <button className="btn-secondary w-full" type="button" disabled={isLocked} onClick={() => openPositiveBatchSelection(row)}>Choose batch treatment</button> : null}
                  </div>
                </div>
              );
            })}
                  ...
                  return (
                    <tr key={row.id} className={`border-b border-border last:border-0 ${showReasonError ? "bg-amber-50" : ""}`}>
                      ...
                      <td className="px-4 py-3">
                        ...
                        />
                        {showReasonError ? <div className="mt-1 text-xs font-semibold text-amber-700">{isSkipped ? "Required when skipped." : "Required for variance rows."}</div> : null}
                        {!isRaw && variance.variance > 0 ? <div className="mt-2 text-xs text-text-secondary">{row.positive_adjustment_batch_balance_id ? "Existing batch selected" : "Reconciliation Batch to be created on approval"}: <span className="font-bold text-text-primary">{rowSku?.storage_location || "Missing"}</span> · +{quantity(variance.variance, "Packs")}</div> : null}
                        ...
                        {!isRaw && variance.variance > 0 ? <button className="mt-2 text-xs font-bold text-primary hover:underline" type="button" disabled={isLocked} onClick={() => openPositiveBatchSelection(row)}>Choose batch treatment</button> : null}
                      </td>
                    </tr>
                  );
                })}
    ...
    {positiveBatchEditor && form.items.find((row) => row.id === positiveBatchEditor.rowId) ? (() => {
      const row = form.items.find((candidate) => candidate.id === positiveBatchEditor.rowId);
      return <Modal title="Positive Variance Batch" description="Choose an existing valid batch when the counted stock is known to belong to it. Otherwise, approval creates a traceable Reconciliation Batch." size="md" onClose={() => setPositiveBatchEditor(null)} footer={<button className="btn-secondary" type="button" onClick={() => setPositiveBatchEditor(null)}>Close</button>}>
        <div className="space-y-3">
          <button className={`w-full rounded-xl border px-3 py-3 text-left text-sm ${!row.positive_adjustment_batch_balance_id ? "border-primary bg-primary/5" : "border-border bg-white"}`} type="button" onClick={() => { updateRow(row.id, { positive_adjustment_confirmed: true, positive_adjustment_batch_balance_id: "" }); setPositiveBatchEditor(null); }}><div className="font-bold text-text-primary">Create Reconciliation Batch on approval</div><div className="mt-1 text-xs text-text-secondary">No Production Batch is created. Draft and Submitted checks leave inventory unchanged.</div></button>
          {positiveBatchEditor.loading ? <div className="text-sm text-text-secondary">Loading valid batches...</div> : null}
          {positiveBatchEditor.error ? <div className="text-sm font-semibold text-rose-700">{positiveBatchEditor.error}</div> : null}
          {!positiveBatchEditor.loading && !positiveBatchEditor.error && positiveBatchEditor.batches.map((batch) => <button key={batch.batch_balance_id} className={`w-full rounded-xl border px-3 py-3 text-left text-sm ${row.positive_adjustment_batch_balance_id === batch.batch_balance_id ? "border-primary bg-primary/5" : "border-border bg-white"}`} type="button" onClick={() => { updateRow(row.id, { positive_adjustment_confirmed: true, positive_adjustment_batch_balance_id: batch.batch_balance_id }); setPositiveBatchEditor(null); }}><div className="font-bold text-text-primary">{batch.batch_no}</div><div className="mt-1 text-xs text-text-secondary">{batch.source_type} · {quantity(batch.current_balance, "Packs")} · {batch.storage_location}</div></button>)}
        </div>
      </Modal>;
    })() : null}
    </>
