...
function ProductionRow({ row, metric, maximum }) {
  ...
  return (
    <div className="grid gap-3 border-b border-border px-4 py-4 last:border-b-0 sm:px-5 lg:grid-cols-[minmax(190px,0.9fr)_minmax(220px,1.5fr)_minmax(92px,0.42fr)_minmax(84px,0.38fr)_minmax(100px,0.48fr)_minmax(120px,0.54fr)] lg:items-center lg:gap-5">
      <div className="flex min-w-0 items-start gap-3"><div className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-surface-muted text-[11px] font-medium tabular-nums text-text-secondary">{row.rank}</div><div className="min-w-0"><div className="truncate text-sm font-semibold text-text-primary">{row.product || "Finished Good"}</div><div className="mt-0.5 truncate text-xs text-text-secondary">{row.packaging_sku || "—"}</div></div></div>
...
function FooterMetric({ icon: Icon, label, value, helper }) {
  return <div className="min-w-0 py-2.5 sm:px-4 sm:first:pl-0 sm:last:pr-0 sm:border-r sm:border-border sm:last:border-r-0"><div className="flex items-center gap-1.5 text-[11px] font-semibold text-text-muted"><Icon size={14} aria-hidden="true" />{label}</div><div className="mt-1 truncate text-base font-semibold tabular-nums text-text-primary">{value}</div>{helper ? <div className="mt-0.5 truncate text-[11px] text-text-secondary">{helper}</div> : null}</div>;
}
