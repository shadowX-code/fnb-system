...
export default function FactoryRowActions({ onView, viewLabel = "View details", primaryAction, secondaryActions = [], directSingleSecondary = false }) {
  ...
  return <div className="flex h-8 items-center justify-end gap-1.5 whitespace-nowrap" onClick={(event) => event.stopPropagation()}>
    ...
    {(!directSingleSecondary || actions.length > 1 || actions[0]?.destructive) && actions.length ? <ActionMenu open={open} onOpenChange={setOpen} width={196} ariaLabel="More row actions" trigger={({ toggle, ariaLabel }) => <button className="icon-btn h-8 w-8" type="button" aria-label={ariaLabel} title="More actions" onClick={toggle}><Ellipsis size={16} /></button>}>
