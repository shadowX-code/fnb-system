...
import { CalendarDays, ChevronLeft, ChevronRight, ClipboardCopy, Download, LockKeyhole, Send, UnlockKeyhole } from "lucide-react";
...
function ShiftBlock({ roster, canDeleteShift, locked, onDelete }) {
  if (!roster?.template) {
    return <span className="text-xs font-semibold text-text-muted">No shift</span>;
  }
  const template = roster.template;
  const isNonWorking = nonWorkingCodes.has(template.code);
  return (
    <div className={`group rounded-2xl border px-2.5 py-2 text-left shadow-sm ${templateTone(template)}`}>
...
export default function DutyRosterPage({ store, ui, auth }) {
  ...
  useEffect(() => {
    ...
    async function loadRosterData() {
      ...
      try {
        ...
        if (!ignore) setError(loadError.message || "Unable to load duty roster.");
      } finally {
  ...
  async function assignShift(employee, date) {
    if (locked) {
    ...
    try {
      ...
      ui.notify({ title: "Shift saved", message: `${employee.nickname || employee.full_name} · ${selectedTemplate.name}` });
    } catch (saveError) {
  ...
  return (
    <div className="space-y-4">
                      ...
                      {weekDates.map((date, index) => {
                        ...
                        return (
                          <th key={dateValue} className="min-w-[128px] px-3 py-3 text-left">
                            <div className="font-bold text-text-primary">{dayLabels[index]} {formatDay(date)}</div>
                            <div className="mt-1 text-[11px] leading-4 text-text-secondary">Kitchen: {coverage.Kitchen}</div>
                            <div className="text-[11px] leading-4 text-text-secondary">Floor: {coverage.Floor}</div>
                          </th>
                        );
                      })}
                    </tr>
                    ...
                    {groupedEmployees.map((group) => (
                      <Fragment key={group.group}>
                        <tr className="bg-primary/5">
                          <td colSpan={8} className="px-3 py-2 text-xs font-bold uppercase tracking-wide text-primary">{group.group}</td>
                        </tr>
                        {group.employees.map((employee) => (
                          <tr key={employee.id} className="table-row">
                            <td className="table-sticky-cell sticky left-0 z-10 bg-surface px-3 py-3">
                            ...
                            {weekDateValues.map((dateValue) => {
                              ...
                              return (
                                <td key={dateValue} className="border-l border-border px-2 py-2 align-top">
                                  <div
                                    className={`min-h-[72px] w-full rounded-2xl border border-dashed border-border bg-background/60 p-1.5 text-left transition ${!readOnly ? "hover:border-primary/40 hover:bg-primary/5" : ""}`}
                                    role="button"
              ...
              <div className="space-y-4 p-4 lg:hidden">
                {weekDateValues.map((dateValue, index) => (
                  <section key={dateValue} className="rounded-2xl border border-border bg-background p-3">
                    <div className="font-bold text-text-primary">{dayLabels[index]} {formatDay(weekDates[index])}</div>
                    <div className="mt-3 space-y-3">
                      {groupedEmployees.map((group) => (
                        <div key={`${dateValue}-${group.group}`}>
                          <div className="mb-2 text-xs font-bold uppercase tracking-wide text-primary">{group.group}</div>
          ...
          <Card className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="mt-2"><Badge tone={statusTone}>{period?.status === "locked" ? "Locked" : period?.status === "published" ? "Published" : "Draft"}</Badge></div>
              </div>
              {canManageRoster ? (
          ...
          <Card title="Quick Shift Templates" description="Select a template, then click a roster cell.">
            <div className="flex flex-wrap gap-2 p-4">
              {templates.map((template) => (
                <button
                  key={template.id}
                  className={`rounded-full border px-3 py-2 text-xs font-bold transition ${selectedTemplateId === template.id ? "border-primary bg-primary text-white" : `${templateTone(template)} hover:shadow-sm`}`}
                  type="button"
                ...
                return (
                  <div key={bucket} className="rounded-2xl border border-border bg-background p-3">
                    ...
                    <div className="mt-2 grid grid-cols-7 gap-1">
                      {weekDateValues.map((date) => (
                        <div key={date} className="rounded-lg bg-primary/10 py-1 text-center text-[11px] font-bold text-primary">
                          {coverageByDate.get(date)?.[bucket] ?? 0}
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        {[
          ["Total Staff Scheduled", summary.staff],
          ["Total Working Hours", hoursLabel(summary.hours)],
          ["Off Days", summary.off],
          ["Annual Leave", summary.al],
          ["MC", summary.mc],
        ].map(([label, value]) => (
