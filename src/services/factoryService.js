  ...
  const allocations = (row.allocations || []).map((allocation) => {
    ...
    const locationIssue = allocation.location_valid === true
      ...
      : String(currentLocation.status || "").toLowerCase() !== "active"
        ...
        : currentLocation.is_storage_location !== true
...
function normalizeStockCheckItem(row, stockType) {
  ...
  const productFamily = row.finished_good?.product_family || {};
  const systemQty = normalizeNumber(row.system_qty);
  const physicalQty = normalizeNumber(row.physical_qty);
  const positiveAdjustmentBatch = Array.isArray(row.positive_adjustment_batch)
    ? row.positive_adjustment_batch[0]
    : row.positive_adjustment_batch;
  ...
  return {
    ...
    positive_adjustment_batch_balance_id: row.positive_adjustment_batch_balance_id || "",
    positive_adjustment_batch_no: positiveAdjustmentBatch?.batch_no || "",
    positive_adjustment_batch_source_type: positiveAdjustmentBatch?.source_type || "",
...
const productStockCheckSelect = `id,check_no,check_date,status,notes,created_by,submitted_by,submitted_at,approved_by,approved_at,created_at,updated_at,created_by_employee:employees!factory_product_stock_checks_created_by_fkey(id,nickname,full_name,email),submitted_by_employee:employees!factory_product_stock_checks_submitted_by_fkey(id,nickname,full_name,email),approved_by_employee:employees!factory_product_stock_checks_approved_by_fkey(id,nickname,full_name,email),items:factory_product_stock_check_items(id,stock_check_id,finished_good_id,system_qty,physical_qty,variance_qty,variance_percent,count_status,variance_status,variance_reason,uom,adjustment_storage_location_id,positive_adjustment_confirmed,positive_adjustment_batch_balance_id,positive_adjustment_batch:factory_finished_good_batch_balances!factory_product_stock_check_i_positive_adjustment_batch_ba_fkey(batch_no,source_type),created_at,updated_at,finished_good:factory_finished_goods(product_code,product_name,packaging_type,pack_size_qty,pack_size_uom,base_qty,base_uom,uom,product_family:factory_product_families(name_en)),batch_allocations:factory_product_stock_check_batch_adjustments(id,batch_balance_id,quantity,batch:factory_finished_good_batch_balances(batch_no,manufacturing_date,expiry_date,storage_location_id,storage_location,storage_location_type,storage_location_ref:factory_storage_locations(location_name,location_type,status))))`;
...
const factoryServiceDefinition = {
  ...
  async submitPublicProductFeedback({ token, answers, language, sessionToken, contact = {}, answerDetails = {} }) {
    const { data, error } = await supabase.rpc("factory_product_feedback_public_submit", { p_token: token, p_answers: answers, p_language: language || "en", p_session_token: sessionToken || null, p_contact: contact, p_answer_details: answerDetails });
  ...
  async saveRawMaterial(material, employeeId) {
    ...
    if (!payload.uom) throw new Error("Storage UOM is required.");
  ...
  async saveProductRecipe(recipe, employeeId) {
    ...
    const items = (recipe.items ?? [])
      .map((item, index) => ({
        ...
        update_recipe_usage_uom: Boolean(item.update_recipe_usage_uom),
        wastage_percent: normalizeNumber(item.wastage_percent),
  ...
  async verifyProductionRecord(production) {
    const { data, error } = await supabase.rpc("factory_verify_production_record", { p_production_id: production.id });
    throwSupabaseError("factory.production.verify", error);
    return data || {};
  },

  async listMestiFoodProcessingControl(filters = {}) {
    const { data, error } = await supabase.rpc("factory_mesti_food_processing_control", { p_date_from: filters.dateFrom || null, p_date_to: filters.dateTo || null, p_finished_good_id: databaseUuid(filters.product), p_qc_status: filters.qcStatus || null, p_verification_status: filters.verificationStatus || null, p_search: String(filters.search || "").trim() || null });
    throwSupabaseError("factory.mesti_food_processing_control", error);
    return Array.isArray(data) ? data : [];
  },
  ...
  async listProductStockCheckPositiveBatches(finishedGoodId) {
    const { data, error } = await supabase.rpc("factory_list_product_stock_check_positive_batches", {
      p_finished_good_id: finishedGoodId,
    });
    throwSupabaseError("factory.product_stock_check.positive_batches", error);
    return (data || []).map((row) => ({
      batch_balance_id: row.batch_balance_id,
      batch_no: row.batch_no || "",
  ...
  async saveStockCheck(stockType, stockCheck, employeeId) {
    ...
    const items = (stockCheck.items ?? []).map((item) => {
      ...
      return {
        ...
        positive_adjustment_batch_balance_id: item.positive_adjustment_batch_balance_id || null,
